/**
 * Every user-facing string the translated surfaces draw, in both languages.
 *
 * ## Why there is no i18n library here
 *
 * The usual ones (i18next and friends) are 40–60 kB of runtime to solve
 * problems this app does not have: no lazy-loaded namespaces, no plural rules
 * beyond what `Intl.PluralRules` already does natively, no translator tooling
 * in the loop, two languages. The whole mechanism needed is a lookup and a
 * token substitution, and shipping a framework for that on a bundle this
 * carefully chunked is a poor trade.
 *
 * ## Flat keys, and why `en` is the schema
 *
 * `en` is declared `as const` and every other locale is typed as
 * `Record<TranslationKey, string>`. That inverts the usual failure mode: a
 * missing Portuguese string is a **compile error**, not a screen that silently
 * renders English to somebody who does not read it. Adding a key to `en` breaks
 * the build until `ptBR` answers for it, which is exactly the moment to write
 * the translation rather than the moment to forget.
 *
 * Keys are dotted by surface (`auth.*`, `nav.*`, `chat.*`) so a screen's strings
 * sit together and an unused one is easy to spot.
 *
 * ## Placeholders
 *
 * `{name}` style, substituted by `t()`. Kept deliberately dumb — no nesting, no
 * formatting directives. Anything that needs real locale-aware formatting
 * (dates, numbers, plurals) should use `Intl` at the call site, where the value
 * still has its type, rather than being smuggled through a string table.
 */

export const LOCALES = ['en', 'pt-BR'] as const;
export type Locale = (typeof LOCALES)[number];

/** What the language picker draws. `native` is what a speaker calls it. */
export const LOCALE_META: Record<Locale, { native: string; english: string; flag: string }> = {
  en: { native: 'English', english: 'English', flag: '🇺🇸' },
  'pt-BR': { native: 'Português (BR)', english: 'Portuguese (Brazil)', flag: '🇧🇷' },
};

const en = {
  // ---- Language picker ---------------------------------------------------
  'lang.label': 'Language',
  'lang.change': 'Change language',

  // ---- Auth: the desk behind the card ------------------------------------
  'auth.hero.title': 'Work that behaves like paper.',
  'auth.hero.tagline': 'Personal & collaborative project studio',
  'auth.desk.tickIt': 'Go on, tick it',
  'auth.desk.today': 'Today',
  'auth.desk.designIt': 'Design the whole thing',
  'auth.desk.dragMe': 'Drag me',
  'auth.desk.shipFriday': 'Ship Friday ✓',
  'auth.desk.halfDone': 'A list, half done',
  'auth.desk.thisWeek': 'This week',
  'auth.desk.miniTask': 'A task, in miniature',
  'auth.desk.rollOut': 'Roll out the new board',
  'auth.desk.onTime': 'On time',
  'auth.desk.clipOn': 'Clip something on',
  'auth.desk.connect': 'Connect two ideas',
  'auth.desk.deadlines': 'Deadlines that speak up',

  // ---- Auth: shared ------------------------------------------------------
  'auth.email': 'Email',
  'auth.password': 'Password',
  'auth.emailPlaceholder': 'you@company.com',

  // ---- Auth: sign in -----------------------------------------------------
  'auth.signIn.title': 'Sign in',
  'auth.signIn.subtitle': 'Your projects, tasks and boards are waiting.',
  'auth.signIn.submit': 'Sign in',
  'auth.signIn.waking': 'Waking the server…',
  'auth.signIn.wakingHint':
    'The API sleeps when nobody is using it. Starting it up takes a few seconds — your sign-in goes through as soon as it answers.',
  'auth.signIn.noAccount': 'No account yet?',
  'auth.signIn.createOne': 'Create one',
  'auth.signIn.forgot': 'Forgot your password?',
  'auth.signIn.welcomeBack': 'Welcome back, {name}.',
  'auth.signIn.failed': 'Could not sign you in.',

  // ---- Auth: sign up -----------------------------------------------------
  'auth.signUp.title': 'Create your studio',
  'auth.signUp.subtitle': 'One account for personal notes and team projects.',
  'auth.signUp.submit': 'Create account',
  'auth.signUp.haveAccount': 'Already have an account?',
  'auth.signUp.signIn': 'Sign in',
  'auth.signUp.displayName': 'Display name',
  'auth.signUp.namePlaceholder': 'Ana Ribeiro',
  'auth.signUp.passwordHint': 'At least 8 characters',
  'auth.signUp.passwordError': 'Add a letter and a number.',
  'auth.signUp.checkInbox': 'Check your inbox to confirm your address.',
  'auth.signUp.confirmNote':
    'We send one confirmation email. Accounts stay locked until the address is verified.',
  'auth.signUp.failed': 'Could not create the account.',

  // ---- Auth: signing in with a provider ---------------------------------
  'auth.oauth.divider': 'or',
  'auth.oauth.waking':
    'Waking the server before handing you over, so you are not left on a blank page. This only happens on the first visit in a while.',
  'auth.oauth.continueWith': 'Continue with {provider}',
  'auth.oauth.signUpWith': 'Sign up with {provider}',
  'auth.oauth.finishing': 'Signing you in…',
  'auth.oauth.finishingBody': 'One moment — finishing up with your provider.',
  'auth.oauth.failedTitle': 'That sign-in did not finish',
  'auth.oauth.failed': 'That sign-in could not be completed.',
  'auth.oauth.missingCode': 'This link is missing its sign-in code. Start again from the sign-in screen.',
  'auth.oauth.backToSignIn': 'Back to sign in',

  // ---- Auth: password reset ---------------------------------------------
  'auth.backToSignIn': 'Back to sign in',
  'auth.forgot.title': 'Reset your password',
  'auth.forgot.subtitle': 'We email a single-use link that expires in one hour.',
  'auth.forgot.submit': 'Send reset link',
  'auth.forgot.sent': 'If an account exists for',
  'auth.forgot.sentSuffix': ', the reset link is on its way.',
  'auth.reset.title': 'Choose a new password',
  'auth.reset.subtitle': 'Every existing session is signed out once you save.',
  'auth.reset.submit': 'Save new password',
  'auth.reset.newPassword': 'New password',
  'auth.reset.confirmPassword': 'Confirm password',
  'auth.reset.needLink': 'This page needs a reset link. Request a new one from the sign-in screen.',
  'auth.reset.hint': '8+ characters, with at least one letter and one number.',
  'auth.reset.mismatch': 'Passwords do not match.',

  // ---- Auth: email verification -----------------------------------------
  'auth.verify.validating': 'Validating your link',
  'auth.verify.confirmingTitle': 'Confirming your email',
  'auth.verify.oneMoment': 'One moment…',
  'auth.verify.confirmedTitle': 'Email confirmed',
  'auth.verify.youAreIn': 'You are in',
  'auth.verify.redirecting': 'Redirecting to your dashboard.',
  'auth.verify.welcome': 'Email confirmed. Welcome to Task Studio.',
  'auth.verify.linkInvalid': 'This confirmation link is no longer valid.',
  'auth.verify.wrongAddress': 'Wrong address?',
  'auth.verify.startOver': 'Start over',
  'auth.verify.sentTo': 'Sent to',
  'auth.verify.resend': 'Resend confirmation link',
  'auth.verify.pendingTitle': 'Confirm your email',
  'auth.verify.pendingSubtitle': 'We sent a confirmation link. Open it and your workspace unlocks.',
  'auth.verify.expiredTitle': 'Link expired',
  'auth.verify.expiredSubtitle': 'Confirmation links last 24 hours. Send yourself a fresh one.',
  'auth.verify.spamNote': '. Check the spam folder if it has not arrived.',
  'auth.verify.enterAddress': 'Enter your address below and we will send the link again.',

  // ---- Error surfaces ----------------------------------------------------
  'error.notFound.title': 'This board does not exist',
  'error.notFound.body': 'The page may have been deleted, or the link is wrong.',
  'error.notFound.back': 'Back to dashboard',
  'error.boundary.title': 'This page could not be opened',
  'error.boundary.retry': 'Try again',
  'error.boundary.reload': 'Reload the app',
  'error.boundary.chunk': 'Part of the app failed to download. Reloading picks up the current version.',

  // ---- Navigation --------------------------------------------------------
  'nav.groupWorkspace': 'Workspace',
  'nav.groupManage': 'Manage',
  'nav.dashboard': 'Dashboard',
  'nav.dashboardHint': 'Projects & metrics',
  'nav.taskMenu': 'Task menu',
  'nav.taskMenuHint': 'Your day, by hour',
  'nav.notesBoard': 'Notes board',
  'nav.notesBoardHint': 'Your Post-it desk',
  'nav.meetings': 'Meetings',
  'nav.meetingsHint': 'Everywhere you are expected',
  'nav.organizations': 'Organizations',
  'nav.returnToMenu': 'Return {label} to the menu',
  'nav.home': 'Home',
  'nav.projects': 'Projects',
  'nav.tasks': 'Tasks',
  'nav.board': 'Board',
  'nav.notes': 'Notes',
  'nav.themes': 'Themes',
  'nav.settings': 'Settings',
  'nav.recycleBin': 'Recycle bin',
  'nav.invitations': 'Invitations',
  'nav.openMenu': 'Open menu',
  'nav.closeMenu': 'Close menu',
  'nav.account': 'Account',
  'nav.signOut': 'Sign out',
  'nav.signingOut': 'Signing out…',
  'nav.notifications': 'Notifications',
  'nav.search': 'Search',
  'nav.newProject': 'New project',
  'nav.accountMenu': 'Account menu',
  'nav.profileSettings': 'Profile & settings',
  'nav.realtimeConnected': 'Realtime connected',
  'nav.realtimeOffline': 'Realtime offline',
  'nav.live': 'Live',
  'nav.offline': 'Offline',

  'dash.allProjects': 'All projects',
  'dash.yourProjects': 'Your projects',
  'dash.newProject': 'New project',
  'dash.showArchived': 'Archived',
  'dash.greeting': 'Good to see you, {name}.',
  'dash.greetingFallback': 'there',
  'agenda.nothingCompleted': 'Nothing completed yet',
  'agenda.nothingScheduled': 'Nothing scheduled',
  'agenda.noNotesBody': 'No one has attached a note to your tasks yet.',
  'agenda.scheduledBody': 'Tasks with a start time or a deadline appear here, ordered by hour.',
  'bin.tasks': 'Tasks',
  'bin.notes': 'Notes',
  'bin.noDeletedTasks': 'No deleted tasks',
  'bin.noDeletedNotes': 'No deleted notes',
  'bin.deletedTasksBody': 'Deleted tasks land here first, so nothing is lost by accident.',
  'bin.deletedNotesBody':
    'Post-its you remove from a board wait here before they are gone for good.',
  'bin.projectWhiteboard': 'Project whiteboard',
  'bin.pinnedToTask': 'Pinned to a task',
  'bin.confirmPurgeNote': 'Delete this note for good?',
  'invites.invitedYou': 'invited you',
  'rail.noDeadline': 'No deadline',
  'rail.openCount': 'open',
  'projectDash.totalTasks': 'Total tasks',
  'projectDash.dueThisWeek': 'Due this week',
  'projectDash.noDate': 'No date',
  // ---- Roster / invite --------------------------------------------------
  'roster.title': 'Roster',
  'roster.memberCount': '{count} member(s) on this project.',
  'roster.loadingMembers': 'Fetching the roster…',
  'roster.invite': 'Invite',
  'roster.you': '(you)',
  'roster.remove': 'Remove {name}',
  'roster.pending': 'Pending invitations',
  'roster.invitedRelative': 'invited',
  'roster.inviteTitle': 'Invite to the roster',
  'roster.inviteSubtitle': 'Search registered users by name, or paste an exact email address.',
  'roster.search': 'Search',
  'roster.searchPlaceholder': 'Name or email',
  'roster.role': 'Role',
  'roster.roleAdmin': 'Admin',
  'roster.roleAdminHint': 'Can invite people, post meetings and manage tasks',
  'roster.roleMember': 'Member',
  'roster.roleMemberHint': 'Can see the project and work on what they are assigned',
  'roster.searching': 'Searching…',
  'roster.noMatch': 'No match',
  'roster.noMatchBody': 'Only verified Task Studio accounts can be invited.',

  // ---- Create project ----------------------------------------------------
  'project.newTitle': 'New project',
  'project.pin': 'Pin project',
  'project.unpin': 'Unpin project',
  'project.doneCount': '{count} done',
  'project.openCount': '{count} open',
  'project.newSubtitle': 'You become the owner and can invite your roster right after.',
  'project.create': 'Create project',
  'project.name': 'Name',
  'project.namePlaceholder': 'Product launch',
  'project.description': 'Description',
  'project.descriptionPlaceholder': 'What is this project about?',
  'project.accentColour': 'Accent colour',
  'project.settingsTitle': 'Project settings',
  'project.settingsSubtitle': 'Rename it, re-colour it, or move it to the recycle bin.',
  'project.saveChanges': 'Save changes',
  'project.modeBlank': 'Blank project',

  // ---- The planned window -------------------------------------------------
  'project.window': 'Planned window',
  'project.startsAt': 'Starts',
  'project.endsAt': 'Finishes',
  'project.windowHint':
    'Both optional. A finish date is a ceiling: no task can be scheduled past it, and it cannot be pulled back over work that is already there.',
  'project.endsBeforeTasks': 'Work is already scheduled after this date.',
  'project.windowRange': '{from} – {to}',
  'project.windowFrom': 'From {from}',
  'project.windowUntil': 'Until {to}',
  'project.windowDaysLeft': '{count} days left',
  'project.windowOverrun': 'past its finish date',
  'project.modeGithub': 'From GitHub',

  // ---- Importing a project from a GitHub repository -----------------------
  'github.repository': 'Repository',
  'github.look': 'Look up',
  'github.hint':
    'Paste a public repository — github.com/owner/name, or just owner/name. Nothing is created until you have seen what it found.',
  'github.noDescription': 'No description on GitHub.',
  'github.openIssues': '{count} open issues',
  'github.archivedWarning':
    'This repository is archived on GitHub. It imports fine — the project just starts life as a record of finished work.',
  'github.pagesTitle': 'Pages it would create',
  'github.noPages': 'No README or changelog found — the project starts with an empty Documents tab.',
  'github.contributorsTitle': '{count} contributors can be invited',
  'github.contributorsHint': 'Only contributors who sign in here with GitHub can be matched.',
  'github.willInvite': '{name} will be invited',
  'github.noAccount': '{login} has no Task Studio account linked to GitHub',
  'github.useAssistant': 'Let the assistant read the repository',
  'github.import': 'Create the project',
  'github.backgroundHint':
    'This carries on in the background — close this and keep working. A card in the corner tracks it, and you can cancel at any point.',
  'github.importedSummary': '{tasks} tasks · {documents} pages · {invited} invited',
  'github.previewFailed': 'Could not read that repository.',
  'github.importFailed': 'Could not import that repository.',
  'github.cancelFailed': 'Could not cancel that import.',

  // ---- The import tracker: the card that watches a background import ------
  'importTracker.title': 'Importing a repository',
  'importTracker.aRepository': 'the repository',
  'importTracker.dismiss': 'Hide this',
  'importTracker.collapse': 'Roll up',
  'importTracker.expand': 'Open',
  'importTracker.openProject': 'Open project',
  'importTracker.cancelling': 'Cancelling…',
  'importTracker.cancelled': 'Cancelled. Nothing was created.',
  'importTracker.step.queued': 'Waiting to start…',
  'importTracker.step.resolving': 'Finding the repository…',
  'importTracker.step.reading': 'Reading the repository…',
  'importTracker.step.analysing': 'The assistant is reading it…',
  'importTracker.step.writing': 'Writing the project…',
  'importTracker.step.inviting': 'Inviting contributors…',
  'importTracker.step.done': 'Done',

  'project.filedUnderTitle': 'Filed under',
  'project.unfileExplain':
    'Takes the project out of this company and leaves everything else where it is — the roster, the tasks, the pages and the teams all stay. You can file it again from the company page at any time.',
  'project.unfile': 'Remove from organization',
  'project.unfileConfirm': 'Confirm — remove',
  'project.archiveTitle': 'Put this project away',
  'project.archivedTitle': 'This project is archived',
  'project.archiveExplain':
    'Takes it off the dashboard and leaves everything inside it untouched — the roster, the board, the pages and the history all stay exactly as they are. Nothing is deleted and you can take it back out whenever you like.',
  'project.unarchiveExplain':
    'It is off the dashboard until you take it back out. Everything inside it is untouched.',
  'project.archive': 'Archive project',
  'project.archivedChip': 'Archived',
  'project.unarchive': 'Take out of the archive',
  'project.dangerZone': 'Ending this project',

  // Finishing: keeps the record, clears the work. See the settings dialog.
  'project.finishProject': 'Finish project',
  'project.finishExplain':
    'Keeps the name, description, colour, people and teams as a record, and clears everything else: tasks, pages, notes, the whiteboard, the conversation, the meetings and the banner. Nothing cleared can be recovered.',
  'project.finishConfirmBody':
    'This permanently deletes every task, page, note, whiteboard stroke, message, meeting and uploaded file on this project. Only the name, description, colour, roster and teams stay. Enter your password to confirm.',
  'project.finishConfirmLabel': 'Your password',
  'project.finishConfirmAction': 'Finish it',
  'project.finishedToast':
    'Project finished. {items} item(s) cleared, including {tasks} task(s) and {documents} page(s).',
  'project.finished': 'Finished',
  'project.finishedOn': 'Finished {date}',
  'project.finishedBanner':
    'This project is finished. It is read-only until somebody reopens it.',
  'project.reopen': 'Reopen project',
  'project.reopenExplain':
    'Puts the project back into service with its people and teams. Nothing that was cleared comes back.',
  'project.reopenedToast': 'Project reopened.',
  'project.archiveWhileFinished':
    'A finished project is already off the board — reopen it first if you want to archive it instead.',

  'project.deleteExplain':
    'Everything in it — tasks, notes, documents and the conversation — goes with it. It lands in your recycle bin, where it can be restored for 30 days before it is destroyed.',
  'project.deleteProject': 'Delete project',
  'project.deleteConfirmBody': 'Type {name} to confirm.',
  'project.deleteConfirmLabel': 'Project name',
  'project.deleteConfirmAction': 'Delete it',

  // ---- Settings ----------------------------------------------------------
  'settings.title': 'Settings',
  'settings.heading': 'Profile & preferences',
  'settings.avatar': 'Avatar',
  'settings.uploading': 'Uploading…',
  'settings.uploadImage': 'Upload image',
  'settings.remove': 'Remove',
  'settings.identity': 'Identity',
  'settings.bio': 'Bio',
  'settings.bioPlaceholder': 'What are you working on?',
  'settings.saveProfile': 'Save profile',
  'settings.theme': 'Theme',
  'settings.appearance': 'Appearance',
  'settings.light': 'Light',
  'settings.dark': 'Dark',
  'settings.system': 'System',
  'settings.password': 'Password',
  'settings.integrations': 'Connected services',
  'settings.integrationsHint': 'Things Task Studio can reach on your behalf.',

  // ---- The command line ---------------------------------------------------
  'cli.title': 'Task Studio CLI',
  'cli.body':
    'Create projects, generate tasks and close them with a commit, from the repository you are already in.',
  'cli.projectBody':
    'Create tasks from this repository, switch to a task’s branch, and close one with a commit — without leaving your editor.',
  'cli.install': 'Install and sign in',
  'cli.thenInAnyRepo': 'Then, in any repository',
  'cli.inThisRepo': 'Then, in this repository',
  'cli.accountHint':
    'Signing in opens a browser to approve this machine — no password is typed into the terminal. What it keeps is a token you can revoke below.',
  'cli.projectHint':
    '“init” links the checkout you are in to this project. “hook install” lets an ordinary commit close the task on its branch.',
  'cli.showCommands': 'Commands',
  'cli.hideCommands': 'Hide',
  'cli.docs': 'Read the docs',
  // --- Approving a terminal ----------------------------------------------
  //
  // The device grant's browser half. Every string here is read by somebody
  // deciding whether to hand a machine a credential, so none of them is
  // reassuring: the warning says what is actually granted.
  'cliAuth.heading': 'Sign a terminal in',
  'cliAuth.subheading': 'Check the machine below is the one you are sitting at.',
  'cliAuth.codeLabel': 'The code your terminal is showing',
  'cliAuth.continue': 'Continue',
  'cliAuth.unnamedDevice': 'An unnamed machine',
  'cliAuth.grantWarning':
    'Approving gives this machine a token that can do everything you can, until you revoke it in Settings.',
  'cliAuth.approve': 'Approve',
  'cliAuth.deny': 'Not me',
  'cliAuth.approvedTitle': 'Signed in',
  'cliAuth.approvedBody': 'Go back to your terminal — it has what it needs.',
  'cliAuth.deniedTitle': 'Refused',
  'cliAuth.deniedBody': 'Nothing was given to that machine.',
  'cliAuth.expiredTitle': 'That code is no longer waiting',
  'cliAuth.expiredBody': 'Codes last ten minutes. Run taskstudio login again for a new one.',
  'cliAuth.tryAnother': 'Enter another code',
  'cli.machines': 'Signed-in machines',
  'cli.lastUsed': 'last used {when}',
  'cli.neverUsed': 'never used',
  'cli.expires': 'expires {when}',
  'cli.neverExpires': 'no expiry',
  'cli.noMachines': 'No machine is signed in',
  'cli.noMachinesBody': 'Nothing outside your browser can reach your account right now.',
  'cli.machinesError': 'Could not load your signed-in machines',
  'cli.machinesErrorBody': 'This list is the only place you can see what can reach your account, so it is worth another try.',
  'cli.revoke': 'Revoke',
  'cli.revokeNamed': 'Revoke {name}',
  'cli.revokeTitle': 'Revoke “{name}”?',
  'cli.revokeBody': 'This cannot be undone.',
  'cli.revokeDetail':
    '{name} will stop reaching your account immediately. Anyone using it will have to run “taskstudio login” again to get a new token.',
  'cli.revoked': 'That machine is signed out.',
  'cli.copy': 'Copy',
  'cli.copied': 'Copied',
  'cli.copyFailed': 'Could not copy. Select the line and copy it by hand.',
  'cli.commandsRegion': 'Task Studio CLI commands',
  'cli.commandScrollable': 'Command — scroll sideways to see the rest',

  // ---- Spotify ------------------------------------------------------------
  'spotify.title': 'Spotify',
  'spotify.pitch':
    'Put a small player on your screen: what is playing, skip and pause, volume, and a search box. Yours alone — nobody on your projects can see or touch it.',
  'spotify.connect': 'Connect Spotify',
  'spotify.connected': 'Spotify is connected.',
  'spotify.cancelled': 'No Spotify account was connected.',
  'spotify.failed': 'Spotify could not be connected. Try again.',
  'spotify.connectedAs': 'Connected as {name}',
  'spotify.disconnect': 'Disconnect',
  'spotify.disconnected': 'Spotify is disconnected.',
  'spotify.disconnectFailed': 'Could not disconnect Spotify.',
  'spotify.updateFailed': 'Could not change that.',
  'spotify.commandFailed': 'Spotify would not do that.',
  'spotify.showPlayer': 'Show the player',
  'spotify.showPlayerHint': 'Keeps the connection and hides the player from your screen.',
  'spotify.unavailable': 'Not enabled here',
  'spotify.unavailableHint':
    'This deployment has no Spotify application configured, so there is nothing to connect to.',
  'spotify.open': 'Open the player',
  'spotify.close': 'Close the player',
  'spotify.play': 'Play',
  'spotify.pause': 'Pause',
  'spotify.next': 'Next track',
  'spotify.previous': 'Previous track',
  'spotify.idle': 'Nothing is playing',
  'spotify.premiumOnly': 'Spotify allows skipping and volume on Premium only.',
  'spotify.searchPlaceholder': 'Search songs or artists',
  'spotify.queued': 'Queued “{name}” — it plays next.',
  'spotify.queueFailed': 'Spotify would not queue that.',
  'spotify.pin': 'Keep the player open',
  'spotify.unpin': 'Let the player close again',
  'spotify.noDevice': 'Open Spotify on a device to use this.',
  'spotify.notice':
    'The player controls whatever device Spotify is already playing on. Nothing you listen to is stored here.',

  // ---- Google Calendar ----------------------------------------------------
  'calendar.title': 'Google Calendar',
  'calendar.pitch':
    'Mirror the meetings you are expected at into your own calendar, and let changes you make there come back here.',
  'calendar.connect': 'Connect Google Calendar',
  'calendar.connected': 'Your calendar is connected.',
  'calendar.cancelled': 'No calendar was connected.',
  'calendar.connectFailed': 'Could not connect that calendar.',
  'calendar.noRefreshToken':
    'Google did not grant offline access, so background syncing is not possible. Try connecting again and accept every step.',
  'calendar.unavailable':
    'Calendar syncing is not configured on this deployment. Whoever runs it needs to set the Google credentials and an encryption key.',

  'calendar.enabled': 'Keep this calendar in sync',
  'calendar.push': 'Send my meetings to Google',
  'calendar.pull': 'Apply changes made in Google',
  'calendar.pushHint':
    'Meetings you are expected at appear in your Task Studio calendar, and leave it when they are completed or cancelled.',
  'calendar.pullHint':
    'Moving or cancelling a mirrored event in Google updates the meeting here — if you are allowed to change it.',

  'calendar.lastSynced': 'Last synced {when}',
  'calendar.neverSynced': 'Not synced yet',
  'calendar.syncNow': 'Sync now',
  'calendar.syncNowHint': 'Pull changes from Google and push anything missing.',
  'calendar.syncedChanges': '{count} meetings updated from your calendar.',
  'calendar.syncedClean': 'Everything is already up to date.',
  'calendar.syncFailed': 'Could not sync with Google.',
  'calendar.updateFailed': 'Could not save that setting.',

  'calendar.disconnect': 'Disconnect',
  'calendar.disconnectExplain':
    'Task Studio will stop syncing. The calendar it created in your Google account can go with it, or stay as a record.',
  'calendar.disconnectAndRemove': 'Disconnect and delete the calendar',
  'calendar.disconnectKeep': 'Disconnect and keep it',
  'calendar.disconnected': 'Your calendar was disconnected.',
  'calendar.disconnectFailed': 'Could not disconnect that calendar.',

  'calendar.badgeOn': 'Synced',
  'calendar.badgeOff': 'Sync Google Calendar',
  'calendar.badgeOnHint':
    'Your meetings and your Google Calendar are kept in step, both ways. Turn it off in settings.',
  'calendar.badgeOffHint': 'Keep these meetings and your Google Calendar in step, both ways.',

  // ---- The landing page ---------------------------------------------------
  //
  // The only surface in the app written for somebody who is not a user yet.
  // Every claim on it is checkable in the product; nothing here describes a
  // feature that does not exist, which is why there is no pricing section and
  // no testimonials.

  // ---- The repository a project stands for -------------------------------
  'repo.open': 'Open the repository',
  'repo.connectedTitle': 'The repository is connected',
  'repo.urlLabel': 'Repository address',
  'repo.publicOnly':
    'Public repositories only — this deployment reads GitHub with no credentials of yours.',
  'repo.connect': 'Connect a repository',
  'repo.connectTitle': 'Connect this project to GitHub',
  'repo.connectBody':
    'Paste a public repository. It becomes a link beside the project name, and tasks here can name the branch their work happens on.',
  'repo.placeholder': 'github.com/owner/name',
  'repo.connectAction': 'Connect',
  'repo.connected': 'Repository connected.',
  'repo.connectFailed': 'Could not connect that repository.',
  'repo.disconnect': 'Disconnect',
  'repo.disconnected': 'Repository disconnected.',
  'repo.disconnectFailed': 'Could not disconnect that repository.',
  'repo.disconnectHint': 'Branches already written on tasks are kept.',
  'repo.adminOnly': 'Only an owner or admin can connect a repository.',

  // ---- The branch a task's work happens on -------------------------------
  'task.branch': 'Branch',
  'task.branchHint': 'The branch this work happens on. It does not have to exist yet.',
  'task.branchPlaceholder': 'feature/billing-webhook',
  'task.openBranch': 'Open this branch on GitHub',
  'task.commitTitle': 'Finished on {branch}',
  'task.commitBody':
    'This task named a branch. Open it on GitHub to commit, open a pull request, or merge it.',
  'task.commitOpen': 'Open the branch',
  'task.commitLater': 'Not now',

  // ---- Connections hub ----------------------------------------------------
  'connections.broadcast': 'Broadcast',
  'connections.sync': 'Sync',
  'connections.editor': 'Your editor',
  'connections.features': 'Features',
  'connections.connect': 'Connect',
  'connections.connected': 'Connected',
  'connections.adminOnly': 'Connections are managed by the project’s owner and admins.',
  'connections.adminOnlyBody':
    'Everything here sends this project’s work somewhere outside it, so it sits at the same level as managing the roster.',
  'connections.personal': 'Personal',
  'connections.personalHint': 'Connected to your account, not to this project.',
  'connections.soon': 'Not yet',
  'connections.svc.discord': 'Discord',
  'connections.svc.slack': 'Slack',
  'connections.svc.webhook': 'Custom endpoint',
  'connections.svc.googleCalendar': 'Google Calendar',
  'connections.svc.github': 'GitHub',
  'connections.svc.trello': 'Trello',

  // ---- Reporting somebody -------------------------------------------------
  'report.action': 'Report this person',
  'report.title': 'Report {name}',
  'report.body':
    'This goes to whoever runs this deployment. They see what you write and your name; {name} is never told a report exists.',
  'report.reasonLabel': 'What happened',
  'report.placeholder': 'Describe what they did, and where.',
  'report.submit': 'Send report',
  'report.sent': 'Report sent. Thank you.',
  'report.failed': 'Could not send that report.',
  'report.already': 'Reported',
  'report.alreadyHint': 'You have reported this person. Reporting again replaces what you wrote.',
  'report.tooShort': 'Say a little more — one line is enough.',

  // ---- Steering the import assistant --------------------------------------
  'github.guidanceLabel': 'What should it focus on?',
  'github.guidanceHint':
    'Optional. Point it at a folder, or say what this project is for. It steers what the assistant reads — it cannot change what it produces.',
  'github.guidancePlaceholder': 'Focus on packages/api — this is the billing rewrite.',
  'github.overviewTitle': 'What would be imported',
  'github.overviewBack': 'Look up another',

  'landing.nav.docs': 'Docs',
  'docs.contents': 'Contents',
  'docs.backHome': 'Back to the front page',
  'landing.nav.skip': 'Skip to content',
  'landing.nav.how': 'How it works',
  'landing.nav.connects': 'Connections',
  'landing.nav.themes': 'Themes',
  'landing.nav.inside': 'What’s inside',
  'landing.nav.signIn': 'Sign in',
  'landing.nav.getStarted': 'Get started',
  'landing.nav.home': 'Task Studio — back to the top',
  /*
   * The maker's mark at the foot of the page.
   *
   * Split into `before` and `after` rather than being one string with a
   * placeholder, because the name does not sit in the same place in both
   * languages: English puts it before the noun ("A Pitico. solution") and
   * Portuguese after it ("Uma solução Pitico."). A `{name}` token would have
   * forced one word order onto both.
   *
   * `after` carries the full stop. It cannot live in the metal — the mask is
   * clipped from the right to reveal the word, so a stop inside it would have to
   * travel from behind the P to behind the o. See `PiticoMark`.
   */
  'landing.pitico.before': 'A',
  'landing.pitico.after': ' solution',
  'landing.pitico.letter': 'P.',
  'landing.pitico.word': 'Pico.',
  'landing.pitico.aria': 'Pico — the studio behind Task Studio',
  'landing.pitico.body':
    'Task Studio is built by a small studio that makes software the way it wants to use it.',

  'landing.word.tasks': 'tasks',
  'landing.word.meetings': 'meetings',
  'landing.word.notes': 'notes',
  'landing.word.projects': 'projects',
  'landing.word.work': 'work',

  'landing.hero.titleLead': 'Manage your',
  'landing.hero.titleTail': 'the way you would on paper.',
  'landing.hero.body':
    'Task Studio is a project studio that behaves like a desk. Drag work between columns, stick notes on a wall, book the room, and let it talk to the tools you already use.',
  'landing.hero.primary': 'Start for free',
  'landing.hero.secondary': 'See it working',


  // --- The feature carousel ----------------------------------------------
  'landing.how.page': 'Page {page} of {total}',
  'landing.how.goToPage': 'Go to page {page}',
  'landing.how.previous': 'Previous features',
  'landing.how.next': 'More features',

  'landing.demo.notesTab': 'Notes wall',
  'landing.demo.notesTitle': 'Stick it on the wall and tie it to the rest',
  'landing.notes.one': 'Pricing needs a decision',
  'landing.notes.two': 'Ask Bruno about the invoice',
  'landing.notes.three': 'Blocked on the contract',

  'landing.demo.meetTab': 'Meetings',
  'landing.demo.meetTitle': 'Book the room here, see it on your phone',
  'landing.meet.here': 'Task Studio',
  'landing.meet.there': 'Your calendar',
  'landing.meet.title': 'Billing review',
  'landing.meet.when': 'Thursday, 15:00',
  'landing.meet.onPhone': 'On every device',

  'landing.demo.pagesTab': 'Pages',
  'landing.demo.pagesTitle': 'Write it down where the work is',
  'landing.pages.title': 'Billing rewrite — brief',
  'landing.pages.saved': 'Saved',
  'landing.pages.hint': 'Or drop in a PDF and it stays exactly as it is.',

  'landing.demo.undoTab': 'Changelog',
  'landing.demo.undoTitle': 'Every change, and a way back from it',
  'landing.undo.task': 'Ship the billing page',
  'landing.undo.log': 'Changelog',
  'landing.undo.deleted': 'Ana moved a task to the bin',
  'landing.undo.restored': 'Ana moved a task to the bin',
  'landing.undo.button': 'Undo',
  'landing.undo.done': 'Undone',

  'landing.demo.commitTab': 'Your editor',
  'landing.demo.commitTitle': 'Commit as usual, and the board keeps up',
  'landing.commit.checking': 'taskstudio: reading the commit…',
  'landing.commit.task': 'Rate limit the import routes',
  'landing.commit.branch': 'on fix/import-rate-limit',
  'landing.commit.closed': 'Task closed.',

  'landing.demo.drawTab': 'Whiteboard',
  'landing.demo.drawTitle': 'Draw the thing you cannot describe',
  'landing.draw.hint': 'Pen, rubber, and notes on the same surface.',
  'landing.demo.boardTab': 'Board',
  'landing.demo.boardTitle': 'Pick a task up and put it where it belongs',

  'landing.demo.importTab': 'New project',
  'landing.demo.importTitle': 'Turn a repository into a project, in the background',

  'landing.demo.chatTab': 'Project chat',
  'landing.demo.chatTitle': 'Talk about the work, next to the work',

  'landing.board.todo': 'To do',
  'landing.board.doing': 'Doing',
  'landing.board.done': 'Done',
  'landing.board.mover': 'Ship the billing page',
  'landing.board.due': 'Due Friday',
  'landing.board.cardA': 'Draft the pricing copy',
  'landing.board.cardB': 'Review the API contract',
  'landing.board.cardC': 'Pick a payment provider',

  'landing.import.resolving': 'Finding the repository…',
  'landing.import.reading': 'Reading the repository…',
  'landing.import.analysing': 'The assistant is reading it…',
  'landing.import.writing': 'Writing the project…',
  'landing.import.done': 'Project ready',
  'landing.import.tasks': '3 tasks',
  'landing.import.pages': '4 pages',
  'landing.import.invited': '2 invited',

  'landing.chat.project': 'Billing rewrite',
  'landing.chat.online': '3 online',
  'landing.chat.typing': 'Typing',
  'landing.chat.placeholder': 'Write a message…',
  'landing.chat.one': 'The provider docs changed — do we still ship Friday?',
  'landing.chat.two': 'Only the webhook part. Half a day at most.',
  'landing.chat.three': 'Then Friday holds. Moving it to Doing.',


  'landing.svc.github': 'GitHub',
  'landing.svc.githubWhat': 'Turn a public repository into a project',
  'landing.svc.figma': 'Figma',
  'landing.svc.figmaWhat': 'Read a design file and export any frame from it',
  'landing.svc.googleCalendar': 'Google Calendar',
  'landing.svc.googleCalendarWhat': 'Two-way sync for your meetings',
  'landing.svc.feed': 'Calendar feed',
  'landing.svc.feedWhat': 'Subscribe from Apple, Outlook or anywhere',
  'landing.svc.discord': 'Discord',
  'landing.svc.discordWhat': 'Post to a channel when work lands',
  'landing.svc.slack': 'Slack',
  'landing.svc.slackWhat': 'The same, in your workspace',
  'landing.svc.trello': 'Trello',
  'landing.svc.trelloWhat': 'Import a whole board with its lists',
  'landing.svc.exports': 'Jira, Asana & CSV',
  'landing.svc.exportsWhat': 'Bring a board over from an export',
  'landing.svc.webhooks': 'Webhooks & API',
  'landing.svc.webhooksWhat': 'Signed events, and tokens for your scripts',

  // --- The themes shelf ---------------------------------------------------
  'landing.themes.title': 'Multiple ways to look at the same work',
  'landing.themes.pick': 'Choose a theme to preview',
  'landing.themes.light': 'Light',
  'landing.themes.dark': 'Dark',
  'landing.themes.hint': 'Drag across to compare',
  'landing.themes.apply': 'Try this theme',
  'landing.themes.applied': 'Applied',
  'landing.themes.applyHint': 'Applies it to this page — and to the app once you sign in.',
  'landing.themes.preview': 'Preview this theme',
  'landing.themes.previewing': 'Previewing',
  'landing.themes.previewHint': 'A preview on this page only — this theme is part of the paid plans.',
  'landing.themes.dragonVerse': 'Shān duì shān lái, yá duì yá\nMìfēng cǎihuā shēnshān lǐ lái\nMìfēng běn wèi cǎihuā sǐ\nLiángshānbo wèi Zhù Yīngtái',
  'theme.switchLabel': 'Colour scheme',
  'landing.themes.count': '{index} of {total}',

  'landing.inside.title': 'A studio, not a spreadsheet',

  'landing.note.boards': 'Two boards, one set of tasks',
  'landing.note.notes': 'A wall of Post-its',
  'landing.note.meetings': 'Meetings that reach your calendar',
  'landing.note.docs': 'Pages that live with the project',
  'landing.note.undo': 'A changelog you can undo',
  'landing.note.skins': 'Many ways to look',

  'landing.cta.primary': 'Create an account',
  'landing.cta.secondary': 'I already have one',

  'landing.footer.talk': "Let's talk.",
  'landing.footer.coffee': 'Pull up a chair.',
  'landing.footer.openSource': 'Open source',
  'landing.footer.themes': 'Themes',
  'landing.footer.startLabel': 'Start free',
  'landing.footer.startAction': 'Continue to sign up',
  'landing.footer.rights': '© 2026 Task Studio',
  'landing.footer.note1Title': 'Follow along',
  'landing.footer.note1Body': 'Every change, in the open.',
  'landing.footer.note2Title': 'Direct line',
  'landing.footer.note2Body': 'Found something broken? Say so.',
  'landing.footer.note3Title': 'Read the docs',
  'landing.footer.note3Body': 'Boards, projects and the CLI.',
  'landing.footer.tagline': 'Work that behaves like paper.',
  'landing.footer.source': 'Source',
  'landing.footer.by': 'by',
  'landing.footer.byWho': 'Instagram — opens in a new tab',

  // ---- Importing a board exported from another tool -----------------------
  'project.modeBoard': 'From a board',
  'boardImport.drop': 'Drop a board export here',
  'boardImport.formats': 'A Trello .json export, or a .csv from Jira, Asana or Trello.',
  'boardImport.choose': 'Choose a file',
  'boardImport.clear': 'Remove this file',
  'boardImport.readsTrello': 'read as a Trello board',
  'boardImport.readsCsv': 'read as a board export',
  'boardImport.whatComesTitle': 'What comes across',
  'boardImport.whatComes':
    'Lists become columns, cards become tasks, and due dates and completed cards carry over.',
  'boardImport.whatDoesNot':
    'Labels, checklists and assignees are written into each task’s description rather than dropped — this app has one set of columns and no way to represent the rest.',
  'boardImport.start': 'Create the project',
  'boardImport.summary': '{tasks} tasks · {documents} columns',
  'boardImport.failed': 'Could not read that export.',

  // ---- A calendar feed anything can subscribe to --------------------------
  'feed.title': 'Calendar feed',
  'feed.pitch':
    'A private link your calendar app subscribes to. Works with Apple Calendar, Outlook, Thunderbird and Google — no account to connect.',
  'feed.create': 'Create a feed link',
  'feed.copyNow': 'Copy this now — it is shown once and cannot be looked up again.',
  'feed.copy': 'Copy',
  'feed.copied': 'Copied',
  'feed.copyFailed': 'Could not copy. Select the link and copy it by hand.',
  'feed.howTo':
    'In your calendar app, look for “Subscribe to calendar”, “Add by URL”, or “From internet”, and paste it there.',
  'feed.lastFetched': 'Last fetched {when}',
  'feed.neverFetched': 'Nothing has subscribed to it yet',
  'feed.rotate': 'New link',
  'feed.rotateExplain':
    'A new link stops the old one working. Anywhere you have already subscribed will go quiet until you paste the new one.',
  'feed.rotateConfirm': 'Replace it',
  'feed.revoke': 'Delete the feed',
  'feed.revoked': 'The feed link was deleted.',
  'feed.issueFailed': 'Could not create a feed link.',
  'feed.revokeFailed': 'Could not delete the feed link.',

  // ---- Personal access tokens ---------------------------------------------
  'tokens.title': 'API tokens',
  'tokens.pitch':
    'For scripts, cron jobs and automations — anything that is not a browser and cannot sign in.',
  'tokens.new': 'New token',
  'tokens.name': 'What is it for',
  'tokens.namePlaceholder': 'Deploy script',
  'tokens.lifetime': 'Expires',
  'tokens.days30': 'In 30 days',
  'tokens.days90': 'In 90 days',
  'tokens.days365': 'In a year',
  'tokens.never': 'Never',
  'tokens.create': 'Create token',
  'tokens.scopeWarning':
    'A token can do everything you can. Give it an expiry, and revoke it the moment it is no longer needed.',
  'tokens.copyNow': 'Copy “{name}” now — it is shown once and cannot be looked up again.',
  'tokens.copy': 'Copy',
  'tokens.copied': 'Copied',
  'tokens.copyFailed': 'Could not copy. Select the token and copy it by hand.',
  'tokens.dismiss': 'Done',
  'tokens.lastUsed': 'used {when}',
  'tokens.neverUsed': 'never used',
  'tokens.expires': 'expires {when}',
  'tokens.revoke': 'Revoke',
  'tokens.revoked': 'That token no longer works.',
  'tokens.revokedTag': 'Revoked',
  'tokens.expiredTag': 'Expired',
  'tokens.createFailed': 'Could not create that token.',
  'tokens.revokeFailed': 'Could not revoke that token.',

  // ---- Outbound webhooks ---------------------------------------------------
  'project.tabConnections': 'Connections',
  'webhooks.title': 'Webhooks',
  'webhooks.subtitle': 'Post this project’s events to Discord, Slack or anywhere else.',
  'webhooks.none': 'Nothing is listening yet',
  'webhooks.noneBody':
    'Add a URL and this project will post there when a task is completed, a meeting is booked or somebody joins.',
  'webhooks.new': 'Add a webhook',
  'webhooks.add': 'Add it',
  'webhooks.url': 'Where to post',
  'webhooks.urlHint':
    'Paste an incoming webhook URL from Discord or Slack, or any HTTPS address that accepts a POST. Discord and Slack are formatted for automatically.',
  'webhooks.eventsTitle': 'What to post',
  'webhooks.allEvents': 'Everything',
  'webhooks.someEvents': 'Only the selected events',
  'webhooks.event.taskCreated': 'Task created',
  'webhooks.event.taskCompleted': 'Task completed',
  'webhooks.event.taskDeleted': 'Task deleted',
  'webhooks.event.meetingScheduled': 'Meeting booked',
  'webhooks.event.memberJoined': 'Member joined',
  'webhooks.event.projectCompleted': 'Project concluded',
  'webhooks.flavour.discord': 'Discord',
  'webhooks.flavour.slack': 'Slack',
  'webhooks.flavour.generic': 'JSON',
  'webhooks.enabled': 'Deliver to this endpoint',
  'webhooks.secretNow': 'Copy this signing secret now — it is shown once.',
  'webhooks.secretLabel': 'Signing secret',
  'webhooks.secretExplain':
    'Every delivery carries a signature made with this, so your endpoint can tell our requests from anybody else’s. Discord and Slack ignore it; your own server should not.',
  'webhooks.copy': 'Copy',
  'webhooks.copied': 'Copied',
  'webhooks.copyFailed': 'Could not copy. Select the secret and copy it by hand.',
  'webhooks.dismissSecret': 'Done',
  'webhooks.test': 'Send a test',
  'webhooks.testDelivered': 'The test arrived.',
  'webhooks.testFailed': 'The test could not be sent.',
  'webhooks.testFailedStatus': 'The endpoint answered {status}.',
  'webhooks.lastOk': 'Delivered ({status})',
  'webhooks.failures': '{count} failures in a row',
  'webhooks.remove': 'Remove',
  'webhooks.removeConfirm': 'Remove it',
  'webhooks.saveFailed': 'Could not save that webhook.',
  'webhooks.deleteFailed': 'Could not remove that webhook.',
  'webhooks.adminOnly': 'Only admins can see this',
  'webhooks.adminOnlyBody':
    'A webhook URL is a password for wherever it points, so it is kept to the project’s owner and admins.',
  'settings.currentPassword': 'Current password',
  'settings.changePassword': 'Change password',
  'settings.signsOutEverywhere': 'Signs out every device, including this one.',
  'settings.profileSaved': 'Profile saved.',
  'settings.avatarRemoved': 'Avatar removed.',
  'settings.avatarUpdated': 'Avatar updated.',
  'settings.uploadFailed': 'Could not upload the image.',

  // ---- Task composer -----------------------------------------------------
  'task.editTitle': 'Edit task',
  'task.newTitle': 'New task',
  'task.composerSubtitle': 'The type is derived from the schedule and the assignees.',
  'task.saveChanges': 'Save changes',
  'task.create': 'Create task',
  'task.titleLabel': 'Title',
  'task.titlePlaceholder': 'Ship the onboarding flow',
  'task.descriptionPlaceholder': 'Context, links, acceptance criteria…',
  'task.starts': 'Starts',
  'task.deadline': 'Deadline',
  'task.windowInvalid': 'The deadline must come after the start.',
  'task.dateInvalid': 'That is not a date this can use — check the year.',
  'task.dateOutOfRange': 'Pick a date within {years} years from today.',
  'task.colour': 'Colour',
  'task.priority': 'Priority',
  'task.addStep': 'Add a step and press Enter',
  'task.addChecklistItem': 'Add checklist item',
  'task.imageAttachment': 'Image attachment',
  'task.removeAttachment': 'Remove attachment',
  'task.attachImage': 'Attach an image (max 5 MB)',
  'task.imageAttached': 'Image attached.',

  // ---- Attached documents (a task's, a meeting's) ------------------------
  'file.attach': 'Attach a PDF or Word document',
  'file.attached': 'Document attached.',
  'file.remove': 'Remove document',
  'file.hint': 'PDF, DOCX or DOC, up to 10 MB. Uploaded as-is — nothing is re-encoded.',
  'file.uploadFailed': 'Could not upload that document.',
  'task.documentAttachment': 'Attached document',
  'task.hasDocument': 'Has a document attached',
  'meetings.documentAttachment': 'Attached document',

  // ---- Task detail -------------------------------------------------------
  'task.editTask': 'Edit task',
  'task.waiting': 'Waiting',
  'task.checklist': 'Checklist',
  'task.addAll': 'Add all',
  'task.dismiss': 'Dismiss',
  'task.markPending': 'Mark as pending',
  'task.markDone': 'Mark as done',
  'task.removeStep': 'Remove “{step}”',
  'task.addStepShort': 'Add a step',
  'task.addStepAction': 'Add step',
  'task.notes': 'Notes',
  'task.deleteNote': 'Delete note',
  'task.pinNote': 'Pin a note to this task',
  'task.addNote': 'Add note',

  // ---- AI assistant ------------------------------------------------------
  'ai.failed': 'The assistant could not finish',
  'ai.stillWriting': 'Still writing…',
  'ai.unavailable': 'The assistant is unavailable.',
  'ai.notConfigured': 'AI assistant not configured',
  'ai.notConfiguredBody':
    'Set GEMINI_API_KEY on the API to unlock workflow analysis and sub-task suggestions.',
  'ai.taskIdeas': 'Task ideas',
  'ai.taskIdeasBody':
    "Gemini reads the project's description and board, then proposes what could come next.",
  'ai.decideFirstHint':
    'Accept or dismiss what is already proposed before asking for more.',
  'ai.suggestTasks': 'Suggest tasks',
  'ai.noIdeas': 'No suggestions yet',
  'ai.noIdeasBody': 'Ask for a few ideas and accept the ones that fit.',
  'ai.accept': 'Accept',
  'ai.decline': 'Decline',
  'ai.taskAdded': '"{title}" added to the board.',
  'ai.addFailed': 'Could not add that task.',
  'ai.workflowReview': 'Workflow review',
  'ai.workflowReviewBody':
    "Gemini reads the project's completed and open work, then names the bottlenecks.",
  'ai.reanalyse': 'Re-analyse',
  'ai.analyse': 'Analyse project',
  'ai.noAnalysis': 'No analysis yet',
  'ai.noAnalysisBody': 'Run a review once there are a few completed tasks to learn from.',
  'ai.reading': 'Reading the board…',
  'ai.readingBody': 'Summarising completions, deadlines and assignment balance.',
  'ai.reviewReady': 'Workflow review ready.',
  'ai.generated': 'generated',

  // ---- Theme gallery -----------------------------------------------------
  'themes.heading': 'Theme gallery',
  'themes.searchPlaceholder': 'Search themes — try “dark”, “retro”, “toxic”…',
  'themes.searchLabel': 'Search themes',
  'themes.clearSearch': 'Clear the search',
  'themes.noMatch': 'No theme by that name',
  'themes.noMatchBody': 'Try a mood instead — “dark”, “loud”, “paper”, “retro”.',
  'themes.pages': 'Theme gallery pages',
  'themes.previousPage': 'Previous page',
  'themes.nextPage': 'Next page',

  // ---- Rich text editor --------------------------------------------------
  'editor.bold': 'Bold',
  'editor.italic': 'Italic',
  'editor.underline': 'Underline',
  'editor.strikethrough': 'Strikethrough',
  'editor.heading1': 'Heading 1',
  'editor.heading2': 'Heading 2',
  'editor.heading3': 'Heading 3',
  'editor.bodyText': 'Body text',
  'editor.quote': 'Quote',
  'editor.codeBlock': 'Code block',
  'editor.bulletedList': 'Bulleted list',
  'editor.numberedList': 'Numbered list',
  'editor.alignLeft': 'Align left',
  'editor.alignCentre': 'Align centre',
  'editor.alignRight': 'Align right',
  'editor.insertLink': 'Insert a link',
  'editor.removeLink': 'Remove the link',
  'editor.insertImage': 'Insert an image',
  'editor.insertVideo': 'Insert a video by URL',
  'editor.clearFormatting': 'Clear formatting',
  'editor.insert': 'Insert',
  'editor.documentBody': 'Document body',
  'editor.needsAbsoluteUrl': 'Links and media need a full http:// or https:// address.',
  'editor.uploadFailed': 'Could not upload that image.',
  'upload.storageBlocked':
    'The image storage refused this upload from this site. Try again — if it keeps happening, the storage settings are missing this address.',
  'upload.storageFailed': 'The image storage did not accept the file. Try again in a moment.',

  // ---- Text board --------------------------------------------------------
  'doc.untitled': 'Untitled document',
  'doc.created': 'Document created.',
  'doc.saved': 'Document saved.',
  'doc.discardChanges': 'Discard unsaved changes?',
  'doc.emptyPage': 'Empty page',
  'doc.yourBoard': 'Your text board',
  'doc.projectBoard': 'Project text board',
  'doc.storage': '{used} of {limit}',
  'doc.storageLabel': 'Board storage',
  'doc.storageHintProject':
    'Every page on this board counts towards one allowance of {limit}, shared by everyone on the project and set by the project owner’s plan.',
  'doc.storageHintPersonal':
    'Every page on this desk counts towards one allowance of {limit}, set by your plan.',
  'doc.storageFull': 'This board is full. Delete a page to make room, or upgrade the plan.',
  'doc.projectDocument': 'Project document',
  'doc.belongsToProject': 'Belongs to the whole project',
  'doc.pinnedToTask': 'Pinned to this task',
  'doc.pinnedToMeeting': 'Minutes for this meeting',
  'doc.meetings': 'Meetings',
  'doc.noOpenTasks': 'Every task here is finished',
  'doc.noOpenMeetings': 'No meeting still open',
  'doc.download': 'Download',
  'doc.format.docx': 'Word',
  'doc.format.docx.hint': 'Keeps editing, pictures included',
  'doc.formatUnavailable': 'Not available for this page',
  'doc.originalOnly': 'An uploaded file is handed back exactly as it arrived.',
  'doc.designOnly': 'A design is downloaded one object at a time, from the page itself.',
  'doc.downloadImages': 'Pictures in this page',
  'doc.downloadImagesHint': '{count} to save on their own',
  'doc.imageDownloadFailed': 'Could not save that picture.',
  'doc.imageDownloaded': 'Saved {name}.',
  'doc.imageUntitled': 'Picture {index}',
  'doc.downloadAllImages': 'Save all pictures',
  'doc.downloadAllImagesHint': 'One .zip with all {count}',
  'doc.imagesDownloaded': 'Saved the pictures.',
  'doc.imagesDownloadedCount': 'Saved {count} pictures.',
  'doc.imagesDownloadFailed': 'Could not save the pictures.',
  'doc.downloadAs': 'Download as',
  'doc.downloadFailed': 'Could not download that document.',
  'doc.format.pdf': 'PDF',
  'doc.format.pdf.hint': 'Typeset for printing and sharing, pictures included',
  'doc.format.txt': 'Plain text',
  'doc.format.txt.hint': 'The words only, no formatting',
  'doc.format.html': 'Web page',
  'doc.format.html.hint': 'Opens in a browser, keeps every link',
  'doc.originalFile': 'Original file',

  // ---- Importing an existing document ------------------------------------
  'doc.import': 'Import',
  'doc.importing': 'Uploading…',
  'doc.importHint': 'Bring in a PDF, a Word document (.docx) or a plain-text file',
  'doc.imported': 'Document imported.',
  'doc.importFailed': 'Could not import that document.',
  'doc.keptAsUploaded': 'Kept exactly as uploaded',
  'doc.keptAsUploadedHint':
    'An imported PDF or Word file is kept as it was uploaded and is not edited here. Download it, or write a new page beside it.',
  'doc.asUploaded': 'as uploaded',
  'doc.uploadedFile': 'Uploaded file',
  'doc.openOriginal': 'Open original',
  'doc.loadingPreview': 'Loading the document…',
  'doc.noPreview': 'No preview for {format} files',
  'doc.noPreviewHint':
    'Open it in whatever reads it on your machine, or download it from the menu above.',
  'doc.previewFailed': 'Could not load the file',
  'doc.previewFailedHint': 'It is still in storage — try downloading the original instead.',
  'doc.confirm': 'Confirm',
  'doc.pages': 'Pages',
  'doc.project': 'Project',
  'doc.nothingOpen': 'Nothing open',
  'doc.nothingOpenPersonal':
    'Somewhere to write more than fits on a Post-it — a plan, a draft, a list that got long.',
  'doc.nothingOpenProject':
    'Write the brief, the spec or the meeting notes — for the project, or pinned to one task.',
  'doc.documentTitle': 'Document title',
  'doc.tooLong': 'This page is too long to save. Trim it, or split it across two pages.',
  'doc.someone': 'Someone',
  'doc.linkedDocuments': 'Linked pages',
  'doc.openOnTextBoard': 'Open on the text board',
  'doc.readOnly': 'Read-only',
  'doc.readOnlyHint': 'Only {author} can edit this page. Ask them to share it with you.',
  'doc.whoCanEdit': 'Who can edit',
  'doc.whoCanEditHint':
    'You wrote this page, so only you can change it. Anybody you pick here can too — everybody on the project can already read it.',
  'doc.shareEditing': 'Share editing',
  'doc.editorsSaved': 'Editing rights updated.',
  'doc.editorsFailed': 'Could not update who may edit this page.',
  'doc.noOneElse': 'Nobody else yet',
  'doc.editorCount': '{count} can edit',
  'doc.authorOnly': 'Only you can edit this',

  // ---- Dropping files onto a text board ---------------------------------
  'doc.dropHere': 'Drop to add to this board',
  'doc.dropHint': 'PDF, Word, plain text, pictures or a .zip',
  'doc.dropRejectedType': 'Not a file this board takes. PDF, .docx, .txt, PNG, JPEG or .zip.',
  'doc.dropRejectedEmpty': 'That file is empty.',
  'doc.dropRejectedSize': 'Too large — the limit is {limit}.',
  'doc.dropOneAtATime': 'One file at a time. Adding {name}.',

  // ---- An imported picture ----------------------------------------------
  'doc.picture': 'Picture',
  'doc.optimised': 'Optimised from {from} to {to}',
  'doc.optimisedBadge': 'Optimised',
  'doc.optimisedHint':
    'Re-encoded in your browser before upload: smaller to store, faster to open, and the location metadata is gone.',
  'doc.imageFailed': 'Could not load that picture',

  // ---- An imported archive ----------------------------------------------
  'doc.archive': 'Archive',
  'doc.archiveContents': 'Inside this archive',
  'doc.archiveCount': '{files} files · {size} unpacked',
  'doc.archiveLoading': 'Reading the archive…',
  'doc.archiveEmpty': 'This archive is empty.',
  'doc.archiveTruncated': 'Showing the first {shown} of {total} entries.',
  'doc.archiveFailed': 'Could not read that archive',
  'doc.archiveHint': 'The listing is read without unpacking anything. Download the archive to get the files.',
  'doc.archiveSearch': 'Filter by name',
  'doc.archiveNoMatches': 'Nothing in here by that name',
  'doc.archiveFolder': 'Folder',
  'doc.archiveHeavy': 'Unpacks to {size} — {ratio}x its download size.',
  'doc.archiveHeavyHint': 'That is legitimate for folders of text or logs, but check you have the room before you unpack it.',

  // ---- Figma -------------------------------------------------------------
  'figma.name': 'Figma',
  'figma.connectedTitle': 'Figma is connected',
  'figma.securityLabel': 'About this token',
  /* The way out of the dialog for somebody who has neither half of the form
     yet. Opens the documentation in a new tab, deliberately: a step-by-step
     that navigated away would take the fields already filled in with it. */
  'figma.howTo': 'How to set this up',
  'figma.open': 'Open in Figma',
  'figma.connect': 'Connect Figma',
  'figma.connectTitle': 'Connect this project to Figma',
  'figma.connectBody':
    'Paste a Figma file link and a personal access token that can read it. The design then appears beside the project name, and can be added to the Documents tab as a live page.',
  'figma.urlLabel': 'File link',
  'figma.urlPlaceholder': 'figma.com/design/<key>/<name>',
  'figma.tokenLabel': 'Personal access token',
  'figma.tokenPlaceholder': 'figd_…',
  'figma.tokenHint':
    'Figma → Settings → Security → Personal access tokens. Read-only file access is enough.',
  'figma.sharedCredential':
    'Everybody on this project reads the design through this token. It is encrypted at rest and never shown again.',
  'figma.connectAction': 'Connect',
  'figma.connected': 'Figma file connected.',
  'figma.connectFailed': 'Could not connect that Figma file.',
  'figma.disconnect': 'Disconnect',
  'figma.disconnected': 'Figma disconnected.',
  'figma.disconnectFailed': 'Could not disconnect Figma.',
  'figma.disconnectHint':
    'The token is deleted. Design pages already on the board keep what they last synced and stop refreshing.',
  'figma.adminOnly': 'Only an owner or admin can connect Figma.',
  'figma.unavailable': 'Not enabled here',
  'figma.unavailableHint':
    'This deployment has no encryption key, so it cannot keep a Figma credential.',
  'figma.connectedFile': 'Connected to {file}',
  'figma.connectedBy': 'Using {name} token',

  // ---- A design page on the text board ----------------------------------
  'figma.addPage': 'Figma',
  'figma.addPageHint': 'Add the connected design file as a page',
  'figma.addPageNeedsConnection': 'Connect this project to Figma first, on the Connections tab.',
  'figma.pageAdded': 'Design added to the board.',
  'figma.pageFailed': 'Could not add that design.',
  'figma.design': 'Design file',
  'figma.sync': 'Sync',
  'figma.syncing': 'Checking Figma…',
  'figma.synced': 'Design updated from Figma.',
  'figma.alreadyCurrent': 'Already up to date.',
  'figma.syncFailed': 'Could not reach Figma.',
  'figma.syncedAt': 'Synced {when}',
  'figma.neverSynced': 'Never synced',
  'figma.pagesCount': '{count} pages',
  'figma.objectsCount': '{count} objects',
  'figma.noFrames': 'Nothing on this page yet',
  'figma.noPages': 'This file has no pages the board can read.',
  'figma.truncated': 'The file is larger than this — only the first objects are listed.',
  'figma.previewFailed': 'Figma could not render a preview.',
  'figma.selectObject': 'Pick an object to preview it',
  'figma.download': 'Download',
  'figma.downloadAs': 'Download this object as',
  'figma.downloaded': 'Downloaded {name}.',
  'figma.downloadFailed': 'Could not export that object.',
  'figma.format.png': 'PNG',
  'figma.format.png.hint': 'Raster, transparent background',
  'figma.format.jpg': 'JPEG',
  'figma.format.jpg.hint': 'Raster, smaller, no transparency',
  'figma.format.svg': 'SVG',
  'figma.format.svg.hint': 'Vector, scales to any size',
  'figma.format.pdf': 'PDF',
  'figma.format.pdf.hint': 'Vector, for printing and sharing',

  // ---- The assistant reading a design's structure ------------------------
  'figma.brief': 'Build brief',
  'figma.briefHint': 'Read the page and frame names and draft a brief',
  'figma.briefTitle': 'Build brief',
  'figma.briefLoading': 'Reading the structure…',
  'figma.briefFailed': 'The assistant could not read that file.',
  'figma.briefFlows': 'Flows',
  'figma.briefTasks': 'Suggested tasks',
  'figma.briefGaps': 'States the names do not mention',
  'figma.briefDisclaimer':
    'Written from page and frame names only — the assistant never sees the artwork.',
  'figma.briefSave': 'Save as a page',
  'figma.briefSaved': 'Brief saved as a page.',

  // ---- Inviting people, one at a time or a team at a time ----------------
  'invite.individually': 'People',
  'invite.byTeam': 'Teams',
  'invite.searchPeople': 'Search people',
  'invite.noMatches': 'Nobody here by that name',
  'invite.noTeams': 'No teams here yet',
  'invite.selectedCount': '{count} selected',
  'invite.page': '{from}–{to} of {total}',
  'invite.previousPage': 'Previous people',
  'invite.nextPage': 'More people',

  // ---- Whiteboard --------------------------------------------------------
  'board.inkCleared': 'Ink cleared. Post-its are untouched.',
  'board.adminsOnlyClear': 'Only project admins can clear the canvas.',
  // ---- Undo / redo on a board -------------------------------------------
  'board.undo': 'Undo',
  'board.redo': 'Redo',
  'board.nothingToUndo': 'Nothing to undo',
  'board.nothingToRedo': 'Nothing to redo',
  'board.history.addNote': 'note added',
  'board.history.delete': 'note deleted',
  'board.history.edit': 'note edited',
  'board.history.move': 'note moved',
  'board.history.connect': 'notes connected',
  'board.history.group': 'notes grouped',
  'board.history.draw': 'ink drawn',
  'board.history.erase': 'ink rubbed out',
  'board.history.clearInk': 'ink cleared',

  'board.arrange': 'Arrange',
  'board.arrangeHint': 'Drag notes, lasso to select',
  'board.connect': 'Connect',
  'board.connectHint': 'Draw arrows between notes',
  'board.pen': 'Pen',
  'board.penHint': 'Draw on the canvas',
  'board.eraser': 'Eraser',
  'board.eraserHint': 'Rub ink out',
  'board.projectWhiteboard': 'Project whiteboard',
  'board.addPostIt': 'Add a Post-it',
  'board.addPostItTitle': 'Add a Post-it for the team',
  'board.pinImage': 'Pin an image',
  'board.image': 'Image',
  'board.pickSeveral': 'Pick several',
  'board.pickSeveralTitle': 'Click several notes in a row to select them together',
  'board.liveWithTeam': 'Live with your team',
  'board.offlineLocal': 'Offline — changes stay local',

  'toast.taskCreated': '"{title}" created.',
  'toast.taskDone': '“{title}” is done.',
  'toast.projectReady': '“{name}” is ready.',
  'toast.skinApplied': '{name} applied.',
  // ---- Mutation toasts ---------------------------------------------------
  'toast.boardAddFailed': 'Could not add that to the board.',
  'toast.noteSaveFailed': 'Could not save the note.',
  'toast.layoutSaveFailed': 'Could not save the layout.',
  'toast.connectFailed': 'Could not connect those notes.',
  'toast.strokeSaveFailed': 'Could not save that stroke.',
  'toast.clearPageFailed': 'Could not clear the page.',
  'toast.pageAdded': 'Page added.',
  'toast.pageAddFailed': 'Could not add a page.',
  'toast.pageRemoved': 'Page removed. Its notes are in the recycle bin.',
  'toast.pageRemoveFailed': 'Could not remove the page.',
  'toast.groupFailed': 'Could not group those notes.',
  'toast.noteCreateFailed': 'Could not create the note.',
  'toast.noteBinned': 'Note moved to the recycle bin.',
  'toast.noteRestored': 'Note restored.',
  'toast.notePurged': 'Note deleted for good.',
  'toast.authorOnlyRemove': 'Only the author can remove that note.',
  'toast.projectCreateFailed': 'Could not create the project.',
  'toast.projectUpdated': 'Project updated.',
  'toast.projectBinned': 'Project moved to the recycle bin.',
  'toast.projectRestored': 'Project restored.',
  'toast.projectPurged': 'Project deleted for good.',
  'toast.pinFailed': 'Could not update the pin.',
  'toast.inviteSent': 'Invitation sent.',
  'toast.inviteFailed': 'Could not send the invitation.',
  'toast.joinedProject': 'You joined the project.',
  'toast.inviteDeclined': 'Invitation declined.',
  'toast.rosterUpdated': 'Roster updated.',
  'toast.rosterRemoveFailed': 'Could not remove that member.',
  'toast.rosterRoleFailed': 'Could not change that role.',
  'toast.taskCreateFailed': 'Could not create the task.',
  'toast.taskUpdateFailed': 'Could not update the task.',
  'toast.statusFailed': 'Could not change the status.',
  'toast.taskBinned': 'Task moved to the recycle bin.',
  'toast.taskPurged': 'Task deleted for good.',
  'toast.taskRestored': '“{title}” restored.',
  'toast.restoreFailed': 'Could not restore it.',
  'toast.deleteFailed': 'Could not delete it.',

  // ---- Notes board -------------------------------------------------------
  'notes.title': 'Notes',
  'notes.yourBoard': 'Your Post-it board',
  'notes.opening': 'Opening your board',
  'notes.emptyDesk': 'An empty desk',
  'notes.emptyDeskBody':
    'Add a note or pin an image, then connect and arrange them however you think.',
  'notes.addFirst': 'Add the first note',
  'notes.group': 'Group',
  'notes.groupHint': 'Bind these into one unit — dragging any member moves them all',
  'notes.groupNeedsTwo': 'Select at least two notes to group them',
  'notes.ungroup': 'Ungroup',
  'notes.arrowLands': 'Now click where the arrow lands.',
  'notes.arrowStarts': 'Click the note the arrow starts from.',
  'notes.arrowRemove': 'Click an existing arrow to remove it.',
  'notes.lassoHint': 'Drag across the board to select several notes at once',
  'notes.clearSelection': 'Clear selection',
  'notes.leaveConnect': 'Leave connect mode',
  'notes.eraseInk': 'Erase ink',
  'notes.clearAll': 'Clear all',
  'notes.linkHere': 'Link here',
  'notes.fromHere': 'From here',
  'notes.partOfGroup': 'Part of a group — dragging one moves them all',
  'notes.changeColour': 'Change colour',
  'notes.deleteNote': 'Delete note',
  'notes.resizeNote': 'Drag to resize this note',
  'notes.writeSomething': 'Write something…',
  'notes.addPage': 'Add page',
  'notes.savePageName': 'Save page name',
  'notes.doubleClickRename': 'Double-click to rename',
  'notes.renamePage': 'Rename this page',

  // ---- Task views --------------------------------------------------------
  'views.today': 'Today',
  'views.nothingDue': 'Nothing due this day.',
  'views.noDateYet': 'No date yet',
  'views.previousMonth': 'Previous month',
  'views.nextMonth': 'Next month',
  'views.emptyMonth': 'An empty month',
  'views.noMatch': 'No task matches these filters.',
  'views.sprint': 'Sprint',
  'sprint.done': '{done}/{total} done',
  'sprint.late': '{count} late',
  'views.nothingInSprint': 'Nothing in this sprint',
  'views.nothingToList': 'Nothing to list',
  'views.moveToBin': 'Move to recycle bin',
  'views.clear': 'Clear',
  'views.pinned': 'Pinned',
  'views.withNotes': 'Notes',
  'views.withNotesTitle': 'Only tasks somebody has attached a note to',
  'views.searchTasks': 'Search tasks',

  // ---- Image viewer ------------------------------------------------------
  'image.expand': 'Open the full-size picture',
  'image.collapse': 'Back',
  'image.zoomIn': 'Zoom in',
  'image.zoomOut': 'Zoom out',
  'image.resetZoom': 'Reset zoom',
  'views.markNotDone': 'Mark as not done',
  'views.assigneesOnly': 'Only the people this task is assigned to can complete it.',
  'views.unpinTask': 'Unpin task',
  'views.pinTask': 'Pin task',
  'notes.unpinNote': 'Unpin note',
  'notes.pinNote': 'Pin note',

  'board.showAllCount': 'Show all · {count} more',
  'board.showFewer': 'Show fewer',
  'board.nothingHere': 'Nothing here',
  'board.dropHere': 'Drop a task in this column.',
  'doc.createFailed': 'Could not create the document.',
  'doc.saveFailed': 'Could not save the document.',
  'doc.deleted': 'Document deleted.',
  'doc.new': 'New document',
  'doc.noPages': 'No pages yet. The first one is a click away.',
  'doc.tasks': 'Tasks',
  'doc.loadTheirVersion': 'Load their version',
  'doc.keepMine': 'Keep mine',
  'project.opening': 'Opening project',
  'themes.default': 'Default',
  'themes.browse': 'Browse themes',
  'notes.addFirstNote': 'Add the first note',
  'app.openingStudio': 'Opening studio',
  'app.checkingSession': 'Checking session',
  'themes.defaultSkin': 'Default',
  'views.layout': 'Layout',
  'notes.addPageAction': 'Add page',
  'nav.dropToPin': 'Drop to pin',
  'ai.nextSteps': 'Next steps',
  'task.hasImage': 'Has an image attached',
  'session.expired': 'Your session expired. Please sign in again.',
  'session.refreshFailed': 'Could not refresh data.',
  'task.signOff': 'Sign-off',
  'task.assignees': 'Assignees',
  'task.multiTaskNote': 'More than one assignee — this becomes a MultiTask.',
  'nav.dragAnywhere': 'Drag me anywhere',
  'nav.sendBackToMenu': 'Send it back to the menu',
  'nav.pinnedShortcuts': 'Pinned shortcuts',
  'nav.hoverHintBar': 'Move the pointer here for the bar',
  'board.clearInk': 'Clear ink',
  /* The holder of a Post-it whose name the roster has not supplied yet. */
  'board.someone': 'Someone',
  'board.sharedWall': 'A shared wall',
  'notes.privateDrag': 'Private to you. Drag things anywhere — the layout is saved as you go.',
  'notes.privateOutgrew': 'Private to you. For the things that outgrew a Post-it.',
  'nav.studioWorkspace': 'Studio workspace',
  'nav.hoverHintMenu': 'Move the pointer here for the menu',
  'nav.pinnedHint': 'Pinned to the screen — use its return arrow to drop the copy',
  'themes.inUse': 'In use',
  'themes.paidBadge': 'Paid',
  'themes.lockedHint': 'Part of the paid plans',
  'themes.lockedToast': 'That theme is part of the paid plans — upgrade under Plan to wear it.',
  'themes.upgradeToWear': 'Upgrade to wear {name}',
  'themes.preview': 'Preview',
  'themes.currentlyApplied': 'Currently applied',
  'themes.bothModes': 'Light and dark stay your own setting — every theme ships both.',
  'themes.cursor': 'Theme cursor',
  // ---- Dashboard ---------------------------------------------------------
  'dash.title': 'Dashboard',
  'dash.subtitle': 'Everything you own and everything assigned to you, in one place.',
  'dash.createProject': 'Create a project',
  'dash.openTaskMenu': 'Open task menu',
  'dash.openTaskMenuFor': 'Open the task menu — {label}',
  'dash.openOrganizations': 'All organizations',
  'dash.openTasks': 'Open tasks',
  'dash.completed': 'Completed',
  'dash.overdue': 'Overdue',
  'dash.projects': 'Projects',
  'dash.pinnedProjects': 'Pinned projects',
  'dash.noProjects': 'No projects yet',
  'dash.noProjectsBody': 'Create your first project, then invite people into its roster.',
  'dash.noArchived': 'You do not have any archived Projects yet',
  'dash.noArchivedBody':
    'Archiving a project takes it out of your list without deleting anything. Whatever you archive shows up here.',
  'dash.archivedOnly': 'Showing archived projects only',
  'dash.upNext': 'Up next for you',
  'dash.nothingAssigned': 'Nothing assigned to you',
  'dash.nothingAssignedBody': 'When a project owner assigns you work, it shows up here.',

  // ---- Task menu / agenda ------------------------------------------------
  'agenda.title': 'Task menu',
  'agenda.heading': 'Your agenda',
  'agenda.showingCompleted':
    'Showing completed work. It moves to the recycle bin a week after it is done.',
  'agenda.everythingSlotted': 'Everything has a slot on the calendar.',
  'agenda.building': 'Building your agenda',
  'agenda.unscheduled': 'Unscheduled',
  'agenda.unscheduledBody': 'No start time and no deadline yet.',
  'agenda.newTask': 'New task',
  'agenda.newPersonalTask': 'New personal task',
  'agenda.personal': 'Personal',
  'agenda.personalTitle': 'A personal task — no project behind it',
  'agenda.nothingPersonal': 'No personal tasks yet',
  'agenda.personalBody':
    'Personal tasks have no project behind them. They live here, on your task menu, and nowhere else.',
  'agenda.personalComposerBody':
    'This one is yours alone: no project, no roster, nobody else can see it.',

  // ---- Personal meeting agenda -------------------------------------------
  'agenda.meetingsEyebrow': 'Your week',
  'agenda.meetingsTitle': 'Meetings',
  'agenda.meetingsBody':
    'Everything you are expected at, across every project you are on.',
  'agenda.searchMeetings': 'Search meetings',
  'agenda.allProjects': 'All projects',
  'agenda.openProject': 'Open {name}',
  'agenda.noMeetings': 'Nothing on the calendar',
  'agenda.noMeetingsBody':
    'When somebody posts a meeting on one of your projects, it turns up here.',
  'agenda.noMeetingMatch': 'Nothing matches that',
  'agenda.noMeetingMatchBody': 'Try a different name, room or project.',

  // ---- Organizations -----------------------------------------------------
  'org.eyebrow': 'Companies and their work',
  'org.title': 'Organizations',
  'org.subtitle': 'A company, its people, and the projects they run.',
  'org.new': 'New organization',
  'org.newTitle': 'New organization',
  'org.newSubtitle':
    'A company: its staff, its projects, its calendar and its numbers. You can set all of it up now, or add to it later.',
  'org.editTitle': 'Organization settings',
  'org.editSubtitle':
    'Rename it, re-colour it, or delete it. People and projects are managed on the organization’s own page.',
  'org.create': 'Create organization',
  'org.name': 'Name',
  'org.namePlaceholder': 'Acme Inc.',
  'org.descriptionPlaceholder': 'What belongs in here?',
  'org.emptyTitle': 'No organizations yet',
  'org.emptyBody':
    'An organization holds a company’s people and the projects they run, with a shared calendar and one place to see how all of it is going.',
  'org.noDescription': 'No description yet.',
  'org.archived': 'Archived',
  'org.projectCount': '{count} project(s)',
  'org.ownedByYou': 'Yours',
  'org.ownedBy': 'Owned by {name}',
  'org.fileProject': 'File a project here',
  'org.chooseProject': 'Choose a project…',
  'org.nothingToFile': 'Every project you own is already filed somewhere.',
  'org.unfile': 'Take {name} out of this organization',
  'org.filedUnder': 'Filed under {name}',
  'org.dangerZone': 'Delete this organization',
  'org.deleteExplain':
    'The organization, its staff list and its own meetings go. Every project inside it is unfiled and otherwise left exactly as it is — nobody loses access to any work.',
  'org.delete': 'Delete organization',
  'org.created': '"{name}" is ready.',
  'org.createFailed': 'Could not create that organization.',
  'org.updated': 'Organization updated.',
  'org.deleted': 'Organization deleted. Its projects were left untouched.',
  'org.projectFiled': 'Project filed.',
  'org.projectUnfiled': 'Project unfiled.',

  // ---- Organizations: the company page -----------------------------------
  'org.opening': 'Opening organization',
  'org.open': 'Open {name}',
  'org.tabProjects': 'Projects',
  'org.tabMetrics': 'Metrics',
  'org.tabStaff': 'People',
  'org.tabMeetings': 'Meetings',
  'org.crunching': 'Adding it up',
  'org.metricsStaffOnly':
    'Only people on this organization can see its numbers. You are here through a project inside it.',
  'org.staffStaffOnly':
    'The staff list is for people on this organization. You are here through a project inside it.',
  'org.meetingsStaffOnly':
    'This calendar is for people on this organization. Your own project’s meetings are on its board, and on your agenda.',

  // Projects board
  'org.searchProjects': 'Search projects',
  'org.laneAtRisk': 'Behind',
  'org.laneAtRiskHint': 'Has work past its deadline',
  'org.laneActive': 'On track',
  'org.laneActiveHint': 'Running, nothing overdue',
  'org.laneArchived': 'Archived',
  'org.laneArchivedHint': 'Finished or shelved',
  'org.laneEmpty': 'Nothing here.',
  'org.boardEmpty': 'No projects filed here yet',
  'org.boardEmptyBody': 'Projects filed under this organization appear on this board.',
  'org.boardEmptyAdmin': 'File a project you own to start this organization off.',
  'org.overdueCount': '{count} overdue',
  'org.openCount': '{count} open',
  'org.noTasks': 'No tasks yet',
  'org.completion': 'Completed',
  'org.noAccess': 'You are not on this project',
  'org.noAccessHint':
    'You can see this project exists because you are on this organization. Opening it needs a place on its roster.',

  // Metrics
  'org.tileProjects': 'Live projects',
  'org.tileArchivedHint': '+{count} archived',
  'org.tileMembers': 'People',
  'org.tileMeetings': 'Meetings ahead',
  'org.throughput': 'Throughput',
  'org.tasksDone': '{completed} of {total} tasks done',
  'org.dueThisWeek': '{count} due in the next seven days',
  'org.trendPoint': '{count} completed on {date}',
  'org.mostAtRisk': 'Needs attention',
  'org.nothingOverdue': 'Nothing is overdue',
  'org.busiest': 'Most delivered',
  'org.completedCount': '{count} completed',
  'org.byProject': 'By project',
  'org.byProjectHint': 'Furthest behind first.',
  'org.nextDeadline': 'Next deadline',
  'org.nextDeadlines': 'Next deadlines across the organization',

  // Staff
  'org.staff': 'People',
  'org.headcount': '{count} on the staff list',
  'org.you': 'you',
  'org.role': 'Role',
  'org.roleOwner': 'Owner',
  'org.roleAdmin': 'Admin',
  'org.roleAdminHint': 'Can file projects, invite people and edit the organization',
  'org.roleMember': 'Member',
  'org.roleMemberHint': 'Can see the organization and everything it has filed',
  'org.roleGuest': 'Guest',
  'org.guestNotice':
    'You can see this organization because you are on one of its projects. You are not on its staff list.',
  'org.jobTitle': 'Job title',
  'org.jobTitlePlaceholder': 'Head of delivery',
  'org.noJobTitle': 'No job title',
  'org.editJobTitle': 'Edit job title',
  'org.joined': 'joined',
  'org.removeMember': 'Remove from the organization',
  'org.leave': 'Leave this organization',
  'org.memberUpdated': 'Updated.',
  'org.memberRemoved': 'Removed from the organization.',

  // Invitations
  'org.invite': 'Invite',
  'org.invitePeople': 'Invite people',
  'org.invitePeopleHint':
    'They are invited once the organization is created. Everybody needs a confirmed Task Studio account.',
  'org.inviteEmail': 'Email address',
  'org.inviteEmailPlaceholder': 'colleague@company.com',
  'org.inviteDuplicate': 'Already on the list.',
  'org.addPerson': 'Add',
  'org.removePerson': 'Do not invite {name}',
  'org.inviteSent': 'Invitation sent.',
  'org.inviteFailed': 'Could not send that invitation.',
  'org.inviteRevoked': 'Invitation withdrawn.',
  'org.inviteAccepted': 'You are on the staff list.',
  'org.inviteDeclined': 'Invitation declined.',
  'org.invitesSkipped': '{count} invitation(s) were skipped: {names}',
  'org.pendingInvites': 'Invited, not yet answered',
  'org.noPendingInvites': 'Nobody is waiting',
  'org.noPendingInvitesBody': 'Invitations you send appear here until they are answered.',
  'org.invitedRelative': 'invited {when}',
  'org.revokeInvite': 'Withdraw this invitation',

  // Create dialog
  'org.linkProjects': 'Bring projects with it',
  'org.linkProjectsHint':
    'Projects you own and have not filed anywhere else. You can file more at any time.',

  // Banner
  'org.bannerAdd': 'Add a banner',
  'org.bannerReplace': 'Replace banner',
  'org.bannerRemove': 'Remove banner',
  'org.bannerFailed': 'That image could not be uploaded.',
  'org.bannerNotAnImage': 'Pick an image file.',
  'org.bannerPreviewTitle': 'How this will look',
  'org.bannerPreviewSubtitle':
    'The banner is a wide strip, so a tall picture keeps its middle band. Check the crop before it goes up.',
  'org.bannerAsShown': 'On the page',
  'org.bannerWholeImage': 'The whole picture',
  'org.bannerCropHint': 'Everything outside the strip above is cut off.',
  'org.bannerPickAnother': 'Pick another',
  'org.bannerUse': 'Use this image',

  'org.newProject': 'New project',
  'org.tabTeams': 'Teams',
  'org.teamsStaffOnly':
    'Teams are for people on this organization. You are here through a project inside it.',

  // ---- Teams -------------------------------------------------------------
  'team.title': 'Teams',
  'team.new': 'New team',
  'team.newTitle': 'New team',
  'team.editTitle': 'Edit team',
  'team.composerSubtitle': 'A name, a colour, and who is on it.',
  'team.create': 'Create team',
  'team.name': 'Name',
  'team.namePlaceholder': 'Back end',
  'team.descriptionPlaceholder': 'What does this group do?',
  'team.members': 'Members',
  'team.noStaffYet': 'Invite people to the organization first.',
  'team.noRosterYet': 'Invite people to the project first.',
  'team.headcount': '{count} member(s)',
  'team.edit': 'Edit this team',
  'team.delete': 'Delete this team',
  'team.deleteHint': 'Deletes the group. Nobody loses access to anything.',
  'team.none': 'No teams yet',
  'team.noneBody': 'Teams created here will show up on this tab.',
  'team.noneBodyAdmin': 'Group people who work together, so you can pick them all at once later.',
  'team.created': '"{name}" is ready.',
  'team.createFailed': 'Could not create that team.',
  'team.updated': 'Team updated.',
  'team.deleted': 'Team deleted. Nobody lost access to anything.',

  'project.tabTeams': 'Teams',
  'project.organization': 'Organization',
  'project.organizationNone': 'Not filed anywhere',
  'project.organizationHint':
    'Only organizations you own or administer. You can file it later instead.',
  'project.staffFromTeams': 'Start with a roster',

  // ---- Recycle bin -------------------------------------------------------
  'bin.title': 'Recycle bin',
  'bin.deletedItems': 'Deleted items',
  'bin.autoArchived': 'Auto-archived',
  'bin.restore': 'Restore',
  'bin.emptyNote': 'Empty note',
  'bin.opening': 'Opening the bin',
  'bin.deletedWhen': 'deleted {when}',
  'bin.recently': 'recently',
  'bin.notesBoardPage': 'Notes board · page {page}',

  // Projects: the only tab where an item expires on its own.
  'bin.projects': 'Projects',
  'bin.noDeletedProjects': 'No deleted projects',
  'bin.deletedProjectsBody':
    'A deleted project waits here for 30 days with everything still inside it. After that it is destroyed, and so are its files.',
  'bin.expiresOn': 'Destroyed {date}',
  'bin.expiresToday': 'Destroyed today',
  'bin.expiresInDays': 'Destroyed in {days} day(s)',
  'bin.projectContents': '{tasks} task(s) · {notes} note(s) · {messages} message(s)',
  'bin.projectStillFinished': 'Was finished',
  'bin.deleteForever': 'Delete for good',
  'bin.purgeTaskTitle': 'Delete this task for good?',
  'bin.purgeNoteTitle': 'Delete this note for good?',
  'bin.emptyBin': 'Empty bin',
  'bin.emptyBinTitle': 'Empty the recycle bin?',
  'bin.emptyBinBody': 'This deletes {tasks} task(s), {notes} note(s) and {projects} project(s) for good. It cannot be undone.',
  'bin.emptyBinProjects': 'Projects can only be deleted with your password. Leave it blank to keep them and empty everything else.',
  'bin.emptied': '{count} item(s) deleted for good.',
  'bin.emptiedWithSkipped': '{count} item(s) deleted for good. {skipped} belong to colleagues and were kept.',
  'bin.purgeProjectTitle': 'Delete "{name}" for good?',
  'bin.purgeProjectBody':
    'This destroys the project and everything still inside it — tasks, pages, notes, the whiteboard, the conversation, the meetings and every uploaded file. It cannot be undone, and there is no copy anywhere. Enter your password to confirm.',
  'bin.purgeProjectLabel': 'Your password',
  'bin.purgeProjectAction': 'Delete permanently',

  // ---- Invitations -------------------------------------------------------
  'invites.title': 'Invitations',
  'invites.heading': 'Project invitations',
  'invites.subtitle':
    "Accepting adds you to the roster and unlocks the project's board and chat.",
  'invites.accept': 'Accept',
  'invites.decline': 'Decline',
  'invites.projectExplain': 'Joining gives you the board, the chat and the work.',
  'invites.orgExplain':
    'Joining lets you see the organization. Its projects still invite you separately.',
  'invites.checking': 'Checking invitations',
  'invites.none': 'No pending invitations',
  'invites.noneBody':
    'When someone invites you to a project, it shows up here and as a notification.',

  // ---- Project rail ------------------------------------------------------
  'rail.soonestFirst': 'Soonest first',
  'rail.noProjects': 'You are not on any project yet.',
  'rail.yourCompanies': 'Where you work',
  'rail.noOrganizations': 'You are not on any organization yet.',
  'rail.dragToPin': 'Drag me out to pin this project anywhere on screen',
  'rail.pinnedHint': 'Pinned to the screen — use its return arrow to drop the copy',
  'rail.pinnedProject': 'Pinned project',
  'rail.hoverHint': 'Move the pointer here for your projects',

  // ---- Project dashboard -------------------------------------------------
  'projectDash.crunching': 'Crunching metrics',
  'projectDash.completion': 'Completion',
  'projectDash.doneOfTotal': '{done} of {total} tasks done',
  'projectDash.completedOnDay': '{count} completed on {date}',
  'projectDash.perMonth': 'Tasks completed per month',
  'projectDash.lastMonths': 'Last {count} months',
  'projectDash.completedInMonth': '{count} completed in {month}',
  'projectDash.monthBest': 'Best month',
  'projectDash.monthAverage': 'Monthly average',
  'projectDash.monthTotal': 'Completed this year',
  'projectDash.noMonthlyData': 'Nothing completed in the last year yet.',
  'projectDash.nextDeadlines': 'Next deadlines',
  'projectDash.nothingScheduled': 'Nothing scheduled',
  'projectDash.noDeadlines': 'No upcoming deadlines.',

  // ---- Notifications -----------------------------------------------------
  'notif.markAllRead': 'Mark all read',
  'notif.nothingYet': 'Nothing yet',
  'notif.nothingYetBody': 'Task assignments and deadlines land here.',
  'notif.dueAt': 'Due {when} · {countdown}',
  'session.blocked':
    'The browser blocked the connection to the server. If this keeps happening, the site may not be allowed to reach the API.',
  'notif.optInTitle': 'Get notified on your desktop',
  'notif.optInBody':
    'Be told the moment a task is assigned to you or a deadline is close — even when Task Studio is in another tab. You can turn this off whenever you like.',
  'notif.optInAllow': 'Turn on',
  'notif.optInLater': 'Not now',

  // ---- Chat --------------------------------------------------------------
  'chat.placeholder': 'Write a message…',
  'chat.offline': 'Offline',
  'chat.offlineQueued': 'Offline — messages send when you reconnect',
  'chat.loadEarlier': 'Load earlier messages',
  'chat.loadingEarlier': 'Loading…',
  'chat.retry': 'Not sent — send again',
  'chat.send': 'Send',
  'chat.empty': 'No messages yet. Say hello to the roster.',
  'chat.loading': 'Loading the conversation…',
  'chat.connected': 'Connected',
  'chat.reconnecting': 'Reconnecting…',
  'chat.pinned': 'Pinned',
  'chat.close': 'Close chat',
  'chat.closeTitle': 'Close the chat',
  'chat.typing': '{count} typing…',
  'chat.sending': 'Sending',
  'chat.sent': 'Sent',
  'chat.notSent': 'Not sent',
  'chat.notSentHelp': 'This message was not sent. Check your connection and send it again.',

  // ---- Theme toggle ------------------------------------------------------
  'theme.toLight': 'Switch to light mode',
  'theme.toDark': 'Switch to dark mode',

  // ---- Generic -----------------------------------------------------------
  'common.cancel': 'Cancel',
  'common.save': 'Save',
  'common.edit': 'Edit',
  'common.add': 'Add',
  'common.delete': 'Delete',
  'common.close': 'Close',
  'common.retry': 'Try again',
  'common.loading': 'Loading…',
  'common.you': 'You',
  'common.somethingWentWrong': 'Something went wrong.',
  /*
   * What a failure says when the server's own words are not fit to show.
   *
   * Each of these is the honest reading of one status code and nothing more -
   * they are what is left after `errorMessage` rejects a class-validator
   * string or one of Nest's spelled-out status words. A caller that knows what
   * was being attempted passes its own fallback instead, and should: "Could
   * not save the document" is worth more than any of these.
   */
  'error.badRequest': 'That could not be completed. Check the details and try again.',
  'error.invalid': 'Some of the details are not valid. Check the form and try again.',
  'error.unauthorized': 'You need to be signed in to do that.',
  'error.forbidden': 'You do not have permission to do that.',
  'error.notFound': 'That is not there any more.',
  'error.conflict': 'That clashes with something that already exists.',
  'error.tooLarge': 'That is too large to upload.',
  'error.tooMany': 'Too many attempts. Wait a moment and try again.',
  'error.server': 'Something went wrong on our side. Try again in a moment.',
  'error.unavailable': 'The server is unavailable right now. Try again in a moment.',
  'common.showPassword': 'Show password',
  /*
   * The phone notice. See `MobileGate`.
   *
   * "Being built" rather than "not supported": the first is a date and the
   * second is a decision, and only one of them is true.
   */
  'mobile.title': 'The phone version is still being built',
  'mobile.body':
    'Task Studio is a desk tool for now — boards you drag, a whiteboard, a chat dock. The phone build is coming, and we would rather finish it than hand you half of it.',
  'mobile.desktop': 'Open this on a laptop or desktop and everything is there.',
  'mobile.tablet': 'A tablet works too — anything wider than a phone opens normally.',
  'mobile.note': 'Your account and everything in it is untouched.',
  'common.hidePassword': 'Hide password',
  'common.noDescription': 'No description yet.',
  'session.unreachable': 'Cannot reach the server. Check your connection.',
  'session.slowStart':
    'The server is taking too long to respond — it may be starting up. Try again in a moment.',
  'board.pagesFull': 'A board holds at most {max} pages',
  'board.pagesFullFree': 'Your plan holds {max} pages — upgrade for more',
  'board.projectPagesFull':
    "This project's plan holds {max} whiteboard pages — the owner can upgrade for more",
  'board.addPage': 'Add a page',
  'themes.previewFirst': 'Preview any of them before you commit',
  'themes.moreInGallery':
    '{count} more in the gallery — preview any of them before you commit',
  'auth.dragCard.signIn': 'Sign in',
  'auth.dragCard.dragMe': 'Drag me anywhere',
  'nav.unpinHint': 'Unpin — let the menu hide again',
  'nav.pinHint': 'Pin the menu open',
  'nav.dragToPin': 'Drag me out to pin this anywhere on screen',
  'stage.collapse': 'Back to the page',
  'stage.expand': 'Expand to the full screen',
  'chat.pinnedHint': 'Pinned to the screen — the chat follows you between tabs. Click to unpin.',
  'chat.dragToPin': 'Drag me onto the chat to keep it on screen everywhere',
  'chat.unpinAria': 'Unpin the chat from the screen',
  'chat.pinAria': 'Pin the chat to the screen',
  'board.moveLocked':
    'Only the people this task is assigned to (or a project admin) can move it.',
  'common.fullScreen': 'Full screen',
  'common.renameNamed': 'Rename {name}',
  'common.removeNamed': 'Remove {name}',
  'common.openNamed': 'Open {name}',
  'common.pageNumber': 'Page {number}',
  'common.useColour': 'Use {color}',
  'common.textColour': 'Text colour {color}',
  'common.decrease': 'Decrease {label}',
  'common.increase': 'Increase {label}',
  'common.value': 'value',
  'common.createdBy': 'Created by {name} · {date}',
  'common.createdByWithEmail': 'Created by {name} ({email}) · {date}',
  'common.lastEditedBy': 'Last edited by {name} · {date}',
  'common.startsOn': 'Starts {date}',
  'common.dueOn': 'Due {date}',
  'common.completedOn': 'Completed {date}',
  'common.attachedTo': 'Attached to {title}',
  'common.overdueCount': '{count} overdue',
  'common.notifications': 'Notifications',
  'common.notificationsUnread': 'Notifications ({count} unread)',
  'common.notesFrom': '{count} note(s) from {names}',
  'common.notesCount': '{count} note(s)',
  // ---- Task taxonomy -----------------------------------------------------
  //
  // The type, status and priority vocabularies. These live in
  // `shared/config/constants.ts` as metadata, which is where the colours and
  // the accents belong — but the words are words, so the metadata carries the
  // key and the call site resolves it.
  'type.MEGA': 'MegaTask',
  'type.MEGA.short': 'Mega',
  'type.MEGA.hint': 'Longer than 2 days',
  'type.MICRO': 'MicroTask',
  'type.MICRO.short': 'Micro',
  'type.MICRO.hint': 'Under 8 hours',
  'type.MULTI': 'MultiTask',
  'type.MULTI.short': 'Multi',
  'type.MULTI.hint': 'Several assignees',
  'type.STANDARD': 'Task',
  'type.STANDARD.short': 'Task',
  'type.STANDARD.hint': 'Between 8 hours and 2 days',

  'status.TODO': 'To do',
  'status.IN_PROGRESS': 'In progress',
  'status.COMPLETED': 'Completed',

  'priority.LOW': 'Low',
  'priority.NORMAL': 'Normal',
  'priority.HIGH': 'High',
  'priority.URGENT': 'Urgent',

  // ---- Task filters ------------------------------------------------------
  'dates.noDeadline': 'No deadline',
  'dates.late': '{amount} late',
  'dates.dueIn': 'in {amount}',
  'dates.windowUnderHour': '< 1 hour',
  'dates.windowHours': '{count}h window',
  'dates.windowDays': '{count}d window',
  'views.lateTag': 'Late',
  'views.doneLateTag': 'Done late',
  'views.signedOff': '{done}/{total} signed off',
  /** The badge on a finished card, which shows when it actually landed. */
  'views.completedOn': 'Completed {date}',
  'views.late': 'Late',
  'views.taskCount': '{count} task(s)',
  'filters.myTasks': 'My tasks',
  'filters.all': 'All',
  'filters.personal': 'Personal',
  'filters.status': 'Status',
  'filters.type': 'Type',
  'filters.priority': 'Priority',
  'filters.time': 'Time',
  'filters.late': 'Late',
  'filters.lateHint': 'Past the deadline, still open',
  'filters.lateFinish': 'Late finish',
  'filters.lateFinishHint': 'Finished after the deadline',
  'filters.onTime': 'On time',

  // ---- Layout switcher ---------------------------------------------------
  'views.board': 'Board',
  'views.boardHint': 'Status columns you can drag between',
  'views.sprintHint': 'Status columns crossed with priority lanes',
  'views.list': 'List',
  'views.listHint': 'One dense line per task',
  'views.calendar': 'Calendar',
  'views.calendarHint': 'A month grid by deadline',
  'views.agenda': 'Agenda',
  'views.agendaHint': 'Day buckets, ordered by hour',

  // ---- Boards ------------------------------------------------------------
  'board.expand': 'Expand',
  'board.shrink': 'Shrink',
  'board.draw': 'Draw',
  'board.drawHint': 'Freehand ink over the board',
  'board.arrangeNotesHint': 'Drag notes, lasso to select',
  'board.connectNotesHint': 'Draw arrows between notes',
  'board.inkSize': 'Size',
  'board.eraserNib': 'Nib',
  'board.sharedWallBody':
    'Stick up a Post-it, pin an image or draw straight on the canvas. Everyone on the project sees it as you go.',

  'notes.selectNote': 'Select this note',
  'notes.deselectNote': 'Deselect this note',
  'notes.noteTitle': 'Title',
  'notes.imageCaption': 'Caption',
  'notes.boardImage': 'Board image',
  'notes.useColour': 'Use {color}',
  'notes.postIts': 'Post-its',
  'notes.textBoardTab': 'Documents',
  'notes.yourPostItBoard': 'Your Post-it board',
  'notes.confirmClearPage': 'Clear this page? Notes move to the recycle bin.',
  'notes.confirmRemovePage': 'Remove "{name}"? Its notes move to the recycle bin.',

  // ---- Project workspace -------------------------------------------------
  'project.tabChat': 'Chat',
  'project.tabsLabel': 'Project section',
  'project.tabBoard': 'Board',
  'project.tabMetrics': 'Metrics',
  'project.tabRoster': 'Roster',
  'project.tabWhiteboard': 'Whiteboard',
  'project.tabText': 'Documents',
  'project.tabMeetings': 'Meetings',
  'project.tabLive': 'Live',
  // ---- Live rooms --------------------------------------------------------
  'live.heading': 'Live rooms',
  'live.subheading': 'Talk, with camera and screen. Peer to peer — nothing is recorded.',
  'live.new': 'New room',
  'live.newTitle': 'Open a live room',
  'live.editTitle': 'Edit this room',
  'live.open': 'Open the room',
  'live.composerHint': 'Everything here can be changed later, including while the call is running.',
  'live.emptyTitle': 'No live rooms yet',
  'live.emptyBody':
    'A live room is a call anybody on this project can join — voice, camera and screen, with no recording. Open one when a conversation would be faster than a thread.',
  'live.showEnded': 'Show finished',

  'live.titleLabel': 'What is this call?',
  'live.titlePlaceholder': 'Sprint review, design walkthrough…',
  'live.descriptionLabel': 'Notes for whoever joins',
  'live.descriptionPlaceholder': 'Optional. What you want to cover.',

  'live.whenLabel': 'When',
  'live.opensNow': 'Open it now',
  'live.opensLater': 'Schedule it',
  'live.opensAtLabel': 'Doors open',
  'live.closesAtLabel': 'Doors close (optional)',
  'live.closesAtHint': 'Leave empty to keep it open until somebody ends it.',

  'live.whoLabel': 'Who',
  'live.audienceLabel': 'Who can come in',
  'live.audienceEveryone': 'Everyone on the project',
  'live.audienceEveryoneHint': 'Anybody on the roster can join.',
  'live.audienceSelected': 'Only the people invited',
  'live.audienceSelectedHint': 'Named people, teams, and anybody on a task below.',
  'live.guestsLabel': 'Who is invited',
  'live.notifyLabel': 'Who to tell',

  'live.subjectLabel': 'What it is about',
  'live.tasksLabel': 'Tasks on the agenda',
  'live.tasksHint':
    'Everybody assigned to a task you pick is invited and notified — by email as well, since they did not ask for this meeting.',
  'live.noTasks': 'No open tasks on this project yet.',
  'live.tasksAdds': 'Also invited and emailed:',
  'live.documentLabel': 'Page to have open',
  'live.documentHint': 'From the text board, so everybody arrives looking at the same thing.',
  'live.noDocument': 'No page',

  'live.permissionsLabel': 'What people can do',
  'live.talkLabel': 'Talking',
  'live.talkOpen': 'Anyone can talk',
  'live.talkOpenHint': 'Everybody arrives muted and can unmute themselves.',
  'live.talkLimited': 'Only if allowed',
  'live.talkLimitedHint': 'People ask, and a moderator hands over the microphone.',
  'live.screenLabel': 'Screen sharing',
  'live.screenOpen': 'Anyone can share',
  'live.screenOpenHint': 'Any participant can present their screen.',
  'live.screenLimited': 'Only if allowed',
  'live.screenLimitedHint': 'A moderator lets one person present at a time.',
  'live.moderatorHint':
    'You and any project admin are always moderators — you can talk, present, and hand either out during the call.',

  'live.stateLive': 'Live',
  'live.stateScheduled': 'Scheduled',
  'live.stateEnded': 'Finished',
  'live.join': 'Join',
  'live.joinEarly': 'Join early',
  'live.joinHint':
    'Your browser will ask for the microphone and camera. Both start switched off — nothing is sent until you turn them on.',
  'live.leave': 'Leave',
  'live.end': 'End room',
  'live.opensIn': 'Opens {when}',
  'live.openedAt': 'Open since {when}',
  'live.endedAt': 'Ended {when}',
  'live.hostedBy': 'Opened by {name}',
  'live.aboutTasks': '{count} tasks',
  'live.inRoom': '{count} of {max} here',

  'live.requestingDevices': 'Waiting for your microphone…',
  'live.connecting': 'Connecting to the room…',
  'live.you': 'You',
  'live.moderator': 'Moderator',
  'live.sharing': 'Sharing their screen',
  'live.handRaised': 'Hand raised',
  'live.micOn': 'Microphone on',
  'live.micOff': 'Muted',
  'live.muteSelf': 'Mute',
  'live.unmuteSelf': 'Unmute',
  'live.cameraOn': 'Turn camera on',
  'live.cameraOff': 'Turn camera off',
  'live.share': 'Share screen',
  'live.stopShare': 'Stop sharing',
  'live.raiseHand': 'Ask to speak',
  'live.allowSpeak': 'Let them speak',
  'live.denySpeak': 'Mute them',
  'live.allowPresent': 'Let them share',
  'live.denyPresent': 'Stop them sharing',
  'live.listenOnly': 'You are listening only. Ask to speak and a moderator can hand you the microphone.',

  /*
   * The connection readout.
   *
   * Both strings lead with what the reader can *do* and put the numbers last,
   * because the numbers are for the one person in ten who wants to know why
   * and the advice is for the other nine. Packet loss and jitter are named
   * rather than rolled into a score: a score would have to be explained, and
   * these two terms are searchable.
   */
  'live.qualityWeak': 'Connection is struggling - {loss}% of packets lost, {jitter}ms jitter. Turning your camera off will help.',
  'live.qualityBad': 'Connection is bad - {loss}% of packets lost, {jitter}ms jitter. Audio only may be the best this link can do.',
  'live.relayed': 'Going through the relay rather than directly, which adds a little delay. Usually means one side is behind a strict firewall.',

  'live.created': 'Room open.',
  'live.ended': 'Room closed.',
  'live.removed': 'Room deleted.',
  'live.createFailed': 'Could not open that room',
  'live.updateFailed': 'Could not save that room',
  'live.endFailed': 'Could not close that room',
  'live.removeFailed': 'Could not delete that room',
  'live.grantFailed': 'Could not change that permission',
  'live.joinFailed': 'Could not join that room',
  'live.noDevices': 'No microphone or camera — you can still listen.',
  'live.mediaBlocked':
    'Could not connect audio and video with {name}. A firewall on one of your networks is probably blocking calls — try another network.',
  'live.noMicrophone': 'No microphone available.',
  'live.noCamera': 'No camera available.',
  'live.askToSpeak': 'A moderator has to let you speak in this room.',
  'live.askToPresent': 'A moderator has to let you share a screen in this room.',
  'live.muteByModerator': 'A moderator muted you.',
  'live.displaced': 'You joined this room somewhere else.',
  'project.tabAssistant': 'Assistant',
  'project.tabChangelog': 'Changelog',

  // ---- Project changelog --------------------------------------------------
  //
  // Every one of these is a *whole sentence* with the names substituted in,
  // rather than a fragment the component glues together. Word order is not a
  // property that survives translation: "Ana joined" and "Entrou Ana" put the
  // subject in different places, and a component that concatenated a name and
  // a verb would be a component that only works in English.
  'task.afterProjectEnd': 'This project finishes on {date}. Pick an earlier deadline.',

  'activity.title': 'Changelog',
  'activity.subtitle': 'Everything that has happened in this project, newest first.',
  'activity.entryCount': '{count} entries',
  'activity.loadMore': 'Load older entries',
  'activity.empty': 'Nothing has happened yet',
  'activity.emptyHint':
    'Tasks, pages, meetings and roster changes all leave a line here. The first one arrives as soon as somebody does something.',
  'activity.failed': 'Could not load the changelog',
  'activity.failedHint': 'The rest of the project is unaffected — try again in a moment.',
  'activity.someone': 'Someone',
  'activity.somethingUnnamed': 'something',

  'activity.projectCreated': '{actor} created this project',
  'activity.projectImported': '{actor} imported this project from GitHub',
  'activity.projectRenamed': '{actor} renamed the project from “{from}” to “{subject}”',
  'activity.projectCompleted': '{actor} concluded the project',
  'activity.projectReopened': '{actor} reopened the project',
  'activity.projectFiled': '{actor} filed the project under {target}',
  'activity.projectUnfiled': '{actor} removed the project from {target}',
  'activity.memberInvited': '{actor} invited {target}',
  'activity.memberJoined': '{actor} is now a member',
  'activity.memberLeft': '{actor} left the project',
  'activity.memberRemoved': '{actor} removed {target} from the project',
  'activity.memberRoleChanged': '{actor} made {target} an {role}',
  'activity.taskCreated': '{actor} created the task “{subject}”',
  'activity.taskCompleted': '{actor} completed “{subject}”',
  'activity.taskReopened': '{actor} reopened “{subject}”',
  'activity.taskDeleted': '{actor} deleted the task “{subject}”',
  'activity.documentCreated': '{actor} wrote the page “{subject}”',
  'activity.documentImported': '{actor} imported the file “{subject}”',
  'activity.documentConverted': '{actor} converted “{subject}” into an editable page',
  'activity.documentDeleted': '{actor} deleted the page “{subject}”',
  'activity.meetingScheduled': '{actor} scheduled the meeting “{subject}”',
  'activity.actionReverted': '{actor} undid an action by {target}',

  // ---- Undoing one line ---------------------------------------------------
  'activity.revert': 'Undo',
  'activity.revertConfirm': 'Undo it',
  'activity.revertHint': 'Reverse this action. It stays on the log, struck through.',
  'activity.revertedBy': 'undone by {actor}',
  'activity.revertFailed': 'Could not undo that.',
  'project.newTask': 'New task',
  'project.newTaskShort': 'Task',
  'project.pinProject': 'Pin project',
  'project.unpinProject': 'Unpin project',

  // ---- Composer ----------------------------------------------------------
  'task.autoClassified': 'auto-classified',
  'task.subChecklist': 'Sub-checklist',

  'bin.confirmPurgeTask': 'Delete "{title}" for good?',
  // ---- Note checklist (task sheet) ---------------------------------------
  'task.noteChecklist': 'Note checklist',
  'task.noteChecklistEmpty':
    'No steps yet. Press + to write one on a Post-it — up to three per task.',
  'task.noteChecklistFull': 'A task holds at most {max} notes. Tear one up to make room.',
  'task.note': 'Note',
  'task.newNote': 'New note',
  'task.newNoteHint': 'One step, written on a Post-it. Anyone on the task can tick it off.',
  'task.newNotePlaceholder': 'What needs doing?',
  'task.noteColour': 'Paper',
  'task.ticked': 'Done',
  'task.tickedBy': 'Done by {name}',
  'task.suggestSteps': 'Suggest',
  'task.suggestStepsHint': 'Ask the assistant for up to three steps for this task.',
  'task.stepsFull': 'A task holds three starting steps.',
  'task.lateOpen': 'Past its deadline and still open',
  'task.lateDone': 'Finished after the deadline',
  'task.pin': 'Pin task',
  'task.unpin': 'Unpin task',
  'task.completionNotYours': 'Only the people this task is assigned to can complete it.',
  'task.completionOwnedByAssignees': 'Completion is owned by the assignees',
  'task.completionTicksOwnBox':
    'Ticks your own box. The task is only done once every assignee has ticked theirs.',
  'task.everyoneTicked': 'Everyone has ticked their box.',
  'task.waitingOn': 'Waiting on {names}',
  'task.sharedBlocked':
    'Shared task — still waiting on {people}. Everyone assigned has to tick their own box, or a project admin can close it.',
  'task.sharedBlockedCount': '{count} more assignee(s)',
  'task.andMore': ' and {count} more',
  'task.listJoin': '{list} and {last}',
  'task.noteBy': 'Written by {name} — open it to read the whole note.',

  // ---- Grouping board ----------------------------------------------------
  'project.tabGroups': 'Groups',
  'groups.tagLabel': 'Group',
  'groups.noTag': 'No group',
  'groups.untagged': 'Untagged',
  'groups.untaggedHint': 'Tasks with no group yet. Drag one into a column to file it.',
  'groups.addColumn': 'Add column',
  'groups.newColumn': 'New column',
  'groups.renameColumn': 'Rename column',
  'groups.columnHint': 'A label this project invents for itself — not a status.',
  'groups.columnName': 'Name',
  'groups.columnNamePlaceholder': 'Wireframe, Back end, Prospect…',
  'groups.columnColour': 'Colour',
  'groups.rename': 'Rename column',
  'groups.delete': 'Delete column',
  'groups.moveLeft': 'Move column left',
  'groups.moveRight': 'Move column right',
  'groups.dropHere': 'Drop a task here',
  'groups.full': 'This board already has the maximum of {max} columns.',
  'groups.deleteConfirm': 'Delete the column "{name}"?',
  'groups.deleteConfirmWithTasks':
    'Delete the column "{name}"? Its {count} task(s) are kept and become untagged.',
  'groups.deleted': 'Column deleted. {count} task(s) are now untagged.',
  'groups.createFailed': 'Could not create that column.',
  'groups.emptyTitle': 'No columns yet',
  'groups.emptyBody':
    'Group this project’s tasks by whatever it actually runs on — phases, areas, clients. Each task keeps its own status, shown on the card.',
  'groups.emptyBodyMember':
    'An owner or admin can set up columns here to group the project’s tasks.',

  // Filtering, hiding and paging the board
  'groups.filterOpen': 'Open',
  'groups.filterDone': 'Completed',
  'groups.noneDoneHere': 'Nothing finished in this column yet',
  'groups.hiddenCount': '{count} hidden',
  'groups.showAllColumns': 'Show all',
  'groups.hideColumn': 'Hide “{name}” on my screen',
  'groups.showColumn': 'Show “{name}” again',
  'groups.previousPage': 'Previous columns',
  'groups.nextPage': 'More columns',
  'groups.allHiddenTitle': 'Every column is hidden',
  'groups.allHiddenBody':
    'You have folded away all of this board’s columns. They are hidden only on this device — bring them back whenever you like.',
  'groups.noneMatchTitle': 'Nothing to show here',
  'groups.noneOpenBody': 'There is no open work on this board right now.',
  'groups.noneDoneBody': 'Nothing on this board has been completed yet.',
  'groups.addTaskHere': 'New task in “{name}”',
  'groups.tagLocked': 'Fixed by the column this was created from.',
  'groups.signOff': '{done} of {total} assignees have signed off',
  'groups.completeShared':
    'Ticks your own box. The task is only done once every assignee has ticked theirs.',
  'groups.completeAsAdmin': 'Close this task on the team’s behalf.',

  // ---- Steps from the assistant ------------------------------------------
  'ai.suggestFailed': 'Could not reach the assistant.',
  'ai.stepsAdded': '{count} step(s) added.',
  'ai.noStepsAdded': 'Nothing to add — this task is already full.',
  'ai.needsTitleAndBody':
    'Write a title and a description first — they are what the assistant reads.',


  // ---- Theme gallery ------------------------------------------
  'skin.STUDIO.tagline': 'The quiet default',
  'skin.STUDIO.body':
    'Soft surfaces, one petrol accent and nothing that asks for your attention — the look the rest of these are a departure from.',
  'skin.PAPER.tagline': 'Illustrated and loud',
  'skin.PAPER.body':
    'Thick ink outlines, chunky corners and shadows that read as cut paper lying on the page rather than as a blur.',
  'skin.TERMINAL.tagline': 'Phosphor and scanlines',
  'skin.TERMINAL.body':
    'Monospace everywhere, hard corners, neon rules and a faint scanline wash over the whole tube.',
  'skin.VINTAGE.tagline': 'Brass and leather',
  'skin.VINTAGE.body':
    'Machined corners, engraved rules and controls that catch a highlight along the top edge and sink when pressed.',
  'skin.PIXEL.tagline': '8-bit cartridge',
  'skin.PIXEL.body':
    'Nine-slice tiles with the corner pixels knocked out, ordered dither for a material, and motion quantised into whole frames.',
  'skin.SPACE.tagline': 'The deep field',
  'skin.SPACE.body':
    'Lit glass instruments over a drifting starfield, with a singularity sunk into every screen edge where a menu hides.',
  'skin.HAZARD.tagline': 'Containment breach',
  'skin.HAZARD.body':
    'Stencilled warnings, hazard tape at every seam and toxic sludge pooled in the bottom of every card — which sloshes.',
  'skin.NEWSPAPER.tagline': 'The morning edition',
  'skin.NEWSPAPER.body':
    'Halftone paper, ruled columns, banner headlines under a double rule, and photographs printed the only way a press could — in black.',
  'skin.ELDRITCH.tagline': 'Something is reading this',
  'skin.ELDRITCH.body':
    'Corners that grew rather than being cut, light with no source, inscribed capitals — and an eye that opens somewhere on the page while you are looking elsewhere.',
  'skin.AUTUMN.tagline': 'Warm paper and falling leaves',
  'skin.AUTUMN.body':
    'Pressed rag paper under a low sun, corners rounded the whole way through, a serif you could read a novel in — and leaves coming down over all of it.',
  'skin.RUNIC.tagline': 'Inked on old paper',
  'skin.RUNIC.body':
    'Foxed rag paper with the mould lines still in it, ruled margins that have worn through in places, and oxblood lettering that soaked into the fibre — on the rails, in the icons, under your pointer.',
  'skin.UNDERWATER.tagline': 'Held under',
  'skin.UNDERWATER.body':
    'Broken light from a surface somewhere above you, corners water has had a long time to round off, colour that drains toward blue with depth — and bubbles going up past all of it.',
  'skin.VIBECODED.tagline': 'Looks like every other app',
  'skin.VIBECODED.body': 'i did not write a single line for this one',
  'skin.HALLOWEEN.tagline': 'Something moved in the corner',
  'skin.HALLOWEEN.body':
    'Candlelit dark with a carved lantern for a mark. Webs gather on the doors you have not opened, and opening anything startles whatever was sleeping in it.',
  'skin.VOLCANO.tagline': 'The crust is a lid',
  'skin.VOLCANO.body':
    'Ash-grey plates crazed with fractures, a hot seam under every edge where the melt shows through, type struck rather than set — and embers going up off all of it.',
  'skin.DRAGON.tagline': 'Something long passes the window',

  'theme.light': 'Light',
  'theme.dark': 'Dark',

  'editor.sizeSmall': 'Small',
  'editor.sizeNormal': 'Normal',
  'editor.sizeLarge': 'Large',
  'editor.sizeHuge': 'Huge',

  // ---- Assistant schedule ------------------------------------------------
  'ai.startsToday': 'starts today',
  'ai.startsInDays': 'starts in {count}d',
  'ai.lastsHours': '{count}h of work',
  'ai.lastsDays': '{count}d of work',

  // ---- Meetings ----------------------------------------------------------
  'meetings.new': 'New meeting',
  'meetings.newTitle': 'Post a meeting',
  'meetings.editTitle': 'Edit meeting',
  'meetings.composerSubtitle':
    'Everybody on the roster can read it. Only owners and admins can change it.',
  'meetings.post': 'Post meeting',
  'meetings.saveChanges': 'Save changes',
  'meetings.nameLabel': 'Meeting name',
  'meetings.namePlaceholder': 'Sprint review',
  'meetings.roomLabel': 'Room',
  'meetings.roomPlaceholder': 'Meeting room 2',
  'meetings.roomElsewhere': 'Somewhere else',

  // --- Rooms -------------------------------------------------------------
  //
  // A room is registered once and booked many times, which is why the strings
  // here are short: the picker shows a name, and everything else about a room
  // is context under it.
  'rooms.title': 'Rooms',
  'rooms.manage': 'Rooms',
  'rooms.manageHint': 'Register the rooms this calendar can book',
  'rooms.add': 'Add room',
  'rooms.nameLabel': 'Name',
  'rooms.namePlaceholder': 'Meeting room 2',
  'rooms.locationLabel': 'Where it is',
  'rooms.locationPlaceholder': '3rd floor, east wing',
  'rooms.capacityLabel': 'Seats',
  'rooms.capacityInvalid': 'Between 1 and 10000.',
  'rooms.none': 'No rooms registered',
  'rooms.noneBody': 'Add one and it becomes bookable — two meetings can never share it.',
  'rooms.inherited': 'From the organization',
  'rooms.remove': 'Remove room',
  'rooms.removeHint': 'Meetings already booked in it keep the name.',
  'rooms.created': 'Room added.',
  'rooms.saved': 'Room saved.',
  'rooms.saveFailed': 'Could not save that room.',
  'rooms.deleted': 'Room removed.',
  'rooms.deleteFailed': 'Could not remove that room.',
  'meetings.starts': 'Starts',
  'meetings.ends': 'Ends',
  'meetings.windowInvalid': 'A meeting has to end after it starts.',
  'meetings.descriptionLabel': 'Agenda',
  'meetings.descriptionPlaceholder': 'What this meeting is for, and what to bring.',
  'meetings.participants': 'Participants',
  'meetings.linkProject': 'About a project',
  'meetings.linkProjectNone': 'The organization as a whole',
  'meetings.linkProjectHint':
    'Pick one and the meeting is posted to that project’s board as well as here. It cannot be changed afterwards.',
  'meetings.companyWide': 'Organization-wide',
  'meetings.wholeRoster': 'Whole roster',
  'meetings.until': 'until',
  'meetings.liveNow': 'Happening now',
  'meetings.edit': 'Edit meeting',
  'meetings.markComplete': 'Mark as done',
  'meetings.markCompleteHint': 'Mark as done - it leaves the board.',
  'meetings.delete': 'Delete meeting',
  'meetings.confirm': 'Confirm',
  'meetings.search': 'Search meetings',
  'meetings.searchPlaceholder': 'Search by name',
  'meetings.viewUpcoming': 'Upcoming',
  'meetings.viewByDay': 'By day',
  'meetings.viewCalendar': 'Calendar',
  'meetings.count': '{count} meeting(s)',
  'meetings.none': 'Nothing scheduled',
  'meetings.noneBody': 'When an owner or an admin posts a meeting, it shows up here.',
  'meetings.noneBodyAdmin': 'Post the first one — the whole roster will see it.',
  'meetings.noMatch': 'No meeting by that name',
  'meetings.noMatchBody': 'Nothing scheduled matches “{query}”.',
  'meetings.today': 'Today',
  'meetings.previousDay': 'Previous day',
  'meetings.nextDay': 'Next day',
  'meetings.pickDay': 'Jump to a day',
  'meetings.previousMonth': 'Previous month',
  'meetings.nextMonth': 'Next month',
  'meetings.nothingOnDay': 'Nothing on this day',
  'meetings.nothingOnDayBody': 'The calendar is clear.',
  'meetings.jumpPrevious': 'Previous scheduled day',
  'meetings.jumpNext': 'Next scheduled day',
  'meetings.created': 'Meeting posted.',
  'meetings.updated': 'Meeting updated.',
  'meetings.completed': 'Meeting marked as done.',
  'meetings.deleted': 'Meeting deleted.',
  'meetings.createFailed': 'Could not post the meeting.',
  'meetings.saveFailed': 'Could not save the meeting.',
  'meetings.deleteFailed': 'Could not delete the meeting.',

  'ai.noCreditsLeft': 'No assistant calls left this month',
  'ai.noCreditsLeftBody': 'You have used all {limit} on your plan. They reset on {date} — or upgrade in Settings for more.',


  // ---- Landing: pricing --------------------------------------------------
  'landing.nav.pricing': 'Pricing',
  'landing.pricing.title': 'Choose your plan',
  'landing.pricing.subtitle': 'Start free. Move up when the ceiling starts showing.',
  'landing.pricing.monthly': 'Monthly',
  'landing.pricing.yearly': 'Yearly',
  'landing.pricing.save': '2 months free',
  'landing.pricing.popular': 'Popular',
  'landing.pricing.free': 'Free',
  'landing.pricing.perMonth': '/month',
  'landing.pricing.perYear': '/year',
  'landing.pricing.billedYearly': '{amount} a month, billed yearly',
  'landing.pricing.ctaFree': 'Get started',
  'landing.pricing.ctaPaid': 'Get started',
  'landing.pricing.unavailable': 'Not sold here yet',
  'landing.pricing.feat.projects': '{count} projects at a time',
  'landing.pricing.feat.orgOne': '1 organization at a time',
  'landing.pricing.feat.orgMany': '{count} organizations at a time',
  'landing.pricing.feat.members': '{count} people per project',
  'landing.pricing.feat.membersUnlimited': 'Unlimited people per project',
  'landing.pricing.feat.tasks': '{count} tasks at a time in each project',
  'landing.pricing.feat.docs': '{size} per document board',
  'landing.pricing.feat.ai': '{count} assistant calls each month',
  'landing.pricing.feat.connectionsFree': 'Discord and Figma connections',
  'landing.pricing.feat.themesAll': 'Every theme in the gallery',
  'landing.pricing.feat.themesFree': 'Studio and Paper themes',
  'landing.pricing.feat.connectionsAll': 'Every connection — Slack, Discord, Figma, webhooks',

  // ---- The assistant's optional note -------------------------------------
  'ai.steer': 'Steer it',
  'ai.steerOptional': 'Steer it (optional)',
  'ai.steerPlaceholder': 'Focus on onboarding — we ship in a fortnight.',
  'ai.steerHint': 'A note about what to propose. It steers the subject, not how the assistant works.',
  'ai.steerTooLong': 'Keep the note under {max} characters.',

  // ---- Plans & billing ---------------------------------------------------
  'billing.section': 'Plan',
  'billing.sectionHint': 'What your account is allowed to hold, and what it costs.',
  'billing.plan.FREE': 'Free',
  'billing.plan.STARTUP': 'Startup',
  'billing.plan.BARON': 'Baron',
  'billing.currentPlan': 'You are on {plan}',
  'billing.freeBlurb': 'Everything you need to run one real project, on your own.',
  'billing.startupBlurb': 'Three times the room, and every connection unlocked.',
  'billing.baronBlurb': 'Five times the room, rosters with no ceiling, and 30 assistant calls each.',
  'billing.perMonth': '/month',
  'billing.perYear': '/year',
  'billing.monthly': 'Monthly',
  'billing.yearly': 'Yearly',
  'billing.yearlySave': '2 months free',
  'billing.choosePlan': 'Choose {plan}',
  'billing.currentBadge': 'Current plan',
  'billing.upgrade': 'Upgrade',
  'billing.comparePlans': 'Compare plans',
  'billing.hidePlans': 'Hide plans',
  'billing.manage': 'Manage billing',
  'billing.manageHint': 'Change your card, switch how often you pay, or cancel.',
  'billing.renewsOn': 'Renews on {date}',
  'billing.endsOn': 'Ends on {date}',
  'billing.pastDue': 'A payment did not go through',
  'billing.pastDueBody': 'Your plan is still active while the card is retried. Update it to avoid losing it.',
  'billing.grantedByAdmin': 'Applied to your account directly — nothing to pay.',
  'billing.grantedByAdminHint': 'Contact support to change it.',
  'billing.notConfigured': 'Payments are not set up on this deployment.',
  'billing.notConfiguredBody': 'Every account behaves as Free. The limits below still apply.',
  'billing.checkoutFailed': 'Could not open checkout.',
  'billing.portalFailed': 'Could not open the billing portal.',
  'billing.checkoutDone': 'Payment received — setting your plan up now.',
  'billing.checkoutCancelled': 'Checkout cancelled. Nothing was charged.',
  'billing.usage': 'What you are using',
  'billing.usage.projects': 'Projects',
  'billing.usage.organizations': 'Organizations',
  'billing.usage.ai': 'Assistant calls this month',
  'billing.usage.personalBoard': 'Your personal board',
  'billing.usage.personalBoardNote': 'A project board has its own allowance, on its owner’s plan',
  'billing.usage.resets': 'Resets on {date}',
  'billing.limit.unlimited': 'Unlimited',
  'billing.limit.ofLimit': 'of {limit}',
  'planSoon.title': 'This room is still being built',
  'planSoon.body': 'Paid plans are not switched on yet. We are finishing the rest of the studio first.',
  'planSoon.bodyNamed': '{plan} is not switched on yet. We are finishing the rest of the studio before we start taking payments.',
  'planSoon.reassure': 'Nothing you already have changes, and nothing has been charged.',
  'planSoon.back': 'Back to settings',
  'planSoon.backHome': 'Back to the home page',
  'planSoon.notify': 'Tell me when it opens',
  'planSoon.note': 'Everything on your free plan keeps working exactly as it does now.',
  'billing.feature.projects': 'Projects at a time',
  'billing.feature.organizations': 'Organizations at a time',
  'billing.feature.membersPerProject': 'People per project',
  'billing.feature.membersPerOrganization': 'People per organization',
  'billing.feature.tasksPerProject': 'Tasks at a time, per project',
  'billing.feature.boardPages': 'Pages on your Post-it board',
  'billing.feature.whiteboardPages': 'Pages on each project whiteboard',
  'billing.feature.documentBoard': 'Storage per document board',
  'billing.feature.ai': 'Assistant calls, each person, each month',
  'billing.feature.discord': 'Discord',
  'billing.feature.broadcast': 'Slack & custom endpoints',
  'billing.feature.figma': 'Figma files',
  'billing.feature.customThemes': 'Every theme',
  'billing.feature.yes': 'Included',
  'billing.feature.no': 'Not on this plan',
  'billing.currency': 'Currency',
  // Whiteboard picture folders on the Documents board — see `FolderDocument`.
  'folder.rowCount': '{count} pictures · from the whiteboard',
  'folder.chip': 'Whiteboard folder',
  'folder.chipHint': 'Filled automatically with every picture pinned to this whiteboard page.',
  'folder.summary': '{count} pictures · {size}',
  'folder.explain':
    'Every picture pinned to this whiteboard page lands here too. Open one to view it, or save one or all of them.',
  'folder.detached':
    'Its whiteboard page was removed. The pictures stay here, and new ones go to a new folder.',
  'folder.downloadAll': 'Download all (.zip)',
  'folder.download': 'Download',
  'folder.downloadOne': 'Download {name}',
  'folder.remove': 'Remove {name} from this folder',
  'folder.removeHint':
    'Remove from folder — frees space on the Documents board. A picture still pinned on the whiteboard stays there.',
  'folder.removeConfirm': 'Remove?',
  'folder.removeFailed': 'Could not remove that picture.',
  'folder.saveFailed': 'Could not download that.',
  'folder.zipPartial':
    'Saved {count} of {total} pictures — the rest were too large for one zip or are no longer in storage.',
  'folder.empty': 'No pictures in this folder',
  'folder.emptyBody': 'Pictures pinned to its whiteboard page will appear here.',
  'folder.loadFailed': 'Could not open this folder',
  'folder.loadFailedBody': 'Check your connection and try again.',
  'folder.filed': 'Also saved to Documents › {title}',
  'folder.full.title': 'Documents board is full',
  'folder.full.subtitle': 'Your picture is on the whiteboard — only its Documents copy is missing.',
  'folder.full.bodyBytes':
    'This project\'s Documents board has no room left to file this picture in the whiteboard page\'s folder.',
  'folder.full.bodyPages':
    'This project\'s Documents board has reached its page limit, so a new folder for this whiteboard page could not be created.',
  'folder.full.storageLabel': 'Storage',
  'folder.full.pagesLabel': 'Pages',
  'folder.full.pictureSize': 'This picture: {size}',
  'folder.full.hintTitle': 'Clear unused assets',
  'folder.full.hintBody':
    'Open the Documents board and remove pictures, folders or pages you no longer need. Removing a picture from a folder never takes it off the whiteboard.',
  'folder.full.askOwner':
    'The board\'s size follows the project owner\'s plan — ask them to upgrade for more room.',
  'folder.full.onBaron':
    'This project is already on Baron, the largest plan — clearing space is the way to make room.',
  'folder.full.openDocuments': 'Open Documents board',
  'folder.full.upgrade': 'Upgrade plan',
  // Leaving a project from its settings — see `ProjectSettingsDialog`.
  'project.settingsSubtitleMember':
    'You are a member of this project. Only its owner and admins can change it.',
  'project.leaveTitle': 'Leave this project',
  'project.leaveExplain':
    'You will lose access to its board, pages and chat, and your assignments go back to the team. Nothing is deleted — an invitation brings you back.',
  'project.leaveExplainOwner':
    'A project always has an owner, so choose who takes it over. They become the owner — and its limits follow their plan — and you leave.',
  'project.leaveOnlyMember':
    'You are the only person on this project, so there is nobody to hand it to. Invite someone first, or finish or delete it below.',
  'project.leaveSuccessor': 'New owner',
  'project.leave': 'Leave project',
  'project.leaveConfirm': 'Click again to leave',
  'project.leaveOwner': 'Hand over and leave',
  'project.leaveConfirmOwner': 'Give it to {name} and leave',
  'project.leftToast': 'You left the project.',
  'project.leaveFailed': 'Could not leave the project.',
  // The empty card at the foot of To do — see `AddTaskSlot`.
  'board.addTask': 'Add a task',
} as const;

export type TranslationKey = keyof typeof en;

/**
 * Brazilian Portuguese.
 *
 * Written for the register the app uses in English — direct, second person,
 * no ceremony. A few notes where the choice was not mechanical:
 *
 *   - "Board" is left as *Quadro*, and the notes board as *Mural*, because the
 *     app treats them as two different objects and collapsing both onto one
 *     word would lose that.
 *   - "Recycle bin" is *Lixeira*, the Windows term, which is what the audience
 *     for this app will recognise — not the more literal *Reciclagem*.
 *   - Placeholders keep the English name inside the braces; only the sentence
 *     around them is translated.
 */
const ptBR: Record<TranslationKey, string> = {
  'lang.label': 'Idioma',
  'lang.change': 'Alterar idioma',

  'auth.hero.title': 'Trabalho que se comporta como papel.',
  'auth.hero.tagline': 'Estúdio de projetos pessoais e colaborativos',
  'auth.desk.tickIt': 'Vai, marque',
  'auth.desk.today': 'Hoje',
  'auth.desk.designIt': 'Desenhar tudo',
  'auth.desk.dragMe': 'Me arraste',
  'auth.desk.shipFriday': 'Entregar sexta ✓',
  'auth.desk.halfDone': 'Uma lista, pela metade',
  'auth.desk.thisWeek': 'Esta semana',
  'auth.desk.miniTask': 'Uma tarefa, em miniatura',
  'auth.desk.rollOut': 'Lançar o novo quadro',
  'auth.desk.onTime': 'No prazo',
  'auth.desk.clipOn': 'Prenda algo',
  'auth.desk.connect': 'Conecte duas ideias',
  'auth.desk.deadlines': 'Prazos que se fazem ouvir',

  'auth.email': 'E-mail',
  'auth.password': 'Senha',
  'auth.emailPlaceholder': 'voce@empresa.com',

  'auth.signIn.title': 'Entrar',
  'auth.signIn.subtitle': 'Seus projetos, tarefas e quadros estão esperando.',
  'auth.signIn.submit': 'Entrar',
  'auth.signIn.waking': 'Acordando o servidor…',
  'auth.signIn.wakingHint':
    'A API hiberna quando ninguém está usando. Ligar leva alguns segundos — seu acesso segue assim que ela responder.',
  'auth.signIn.noAccount': 'Ainda não tem conta?',
  'auth.signIn.createOne': 'Criar uma',
  'auth.signIn.forgot': 'Esqueceu sua senha?',
  'auth.signIn.welcomeBack': 'Bem-vindo de volta, {name}.',
  'auth.signIn.failed': 'Não foi possível entrar.',

  'auth.signUp.title': 'Crie seu estúdio',
  'auth.signUp.subtitle': 'Uma conta para notas pessoais e projetos em equipe.',
  'auth.signUp.submit': 'Criar conta',
  'auth.signUp.haveAccount': 'Já tem uma conta?',
  'auth.signUp.signIn': 'Entrar',
  'auth.signUp.displayName': 'Nome de exibição',
  'auth.signUp.namePlaceholder': 'Ana Ribeiro',
  'auth.signUp.passwordHint': 'Pelo menos 8 caracteres',
  'auth.signUp.passwordError': 'Adicione uma letra e um número.',
  'auth.signUp.checkInbox': 'Verifique sua caixa de entrada para confirmar seu endereço.',
  'auth.signUp.confirmNote':
    'Enviamos um único e-mail de confirmação. As contas ficam bloqueadas até o endereço ser verificado.',
  'auth.signUp.failed': 'Não foi possível criar a conta.',

  // ---- Auth: entrar com um provedor -------------------------------------
  'auth.oauth.divider': 'ou',
  'auth.oauth.waking':
    'Acordando o servidor antes de te encaminhar, para você não ficar numa página em branco. Isso só acontece na primeira visita depois de um tempo.',
  'auth.oauth.continueWith': 'Continuar com {provider}',
  'auth.oauth.signUpWith': 'Criar conta com {provider}',
  'auth.oauth.finishing': 'Entrando…',
  'auth.oauth.finishingBody': 'Um momento — finalizando com o seu provedor.',
  'auth.oauth.failedTitle': 'Este acesso não foi concluído',
  'auth.oauth.failed': 'Não foi possível concluir este acesso.',
  'auth.oauth.missingCode': 'Este link está sem o código de acesso. Comece de novo pela tela de entrada.',
  'auth.oauth.backToSignIn': 'Voltar para a entrada',

  'auth.backToSignIn': 'Voltar para o login',
  'auth.forgot.title': 'Redefinir sua senha',
  'auth.forgot.subtitle': 'Enviamos um link de uso único que expira em uma hora.',
  'auth.forgot.submit': 'Enviar link de redefinição',
  'auth.forgot.sent': 'Se existir uma conta para',
  'auth.forgot.sentSuffix': ', o link de redefinição está a caminho.',
  'auth.reset.title': 'Escolha uma nova senha',
  'auth.reset.subtitle': 'Todas as sessões ativas serão encerradas ao salvar.',
  'auth.reset.submit': 'Salvar nova senha',
  'auth.reset.newPassword': 'Nova senha',
  'auth.reset.confirmPassword': 'Confirmar senha',
  'auth.reset.needLink':
    'Esta página precisa de um link de redefinição. Solicite um novo na tela de login.',
  'auth.reset.hint': 'No mínimo 8 caracteres, com ao menos uma letra e um número.',
  'auth.reset.mismatch': 'As senhas não coincidem.',

  'auth.verify.validating': 'Validando seu link',
  'auth.verify.confirmingTitle': 'Confirmando seu e-mail',
  'auth.verify.oneMoment': 'Um momento…',
  'auth.verify.confirmedTitle': 'E-mail confirmado',
  'auth.verify.youAreIn': 'Tudo certo',
  'auth.verify.redirecting': 'Redirecionando para seu painel.',
  'auth.verify.welcome': 'E-mail confirmado. Bem-vindo ao Task Studio.',
  'auth.verify.linkInvalid': 'Este link de confirmação não é mais válido.',
  'auth.verify.wrongAddress': 'Endereço errado?',
  'auth.verify.startOver': 'Começar de novo',
  'auth.verify.sentTo': 'Enviado para',
  'auth.verify.resend': 'Reenviar link de confirmação',
  'auth.verify.pendingTitle': 'Confirme seu e-mail',
  'auth.verify.pendingSubtitle':
    'Enviamos um link de confirmação. Abra-o e seu espaço de trabalho será liberado.',
  'auth.verify.expiredTitle': 'Link expirado',
  'auth.verify.expiredSubtitle':
    'Links de confirmação duram 24 horas. Envie um novo para você mesmo.',
  'auth.verify.spamNote': '. Verifique a pasta de spam se não tiver chegado.',
  'auth.verify.enterAddress': 'Digite seu endereço abaixo e enviaremos o link novamente.',

  'error.notFound.title': 'Este quadro não existe',
  'error.notFound.body': 'A página pode ter sido excluída, ou o link está errado.',
  'error.notFound.back': 'Voltar ao painel',
  'error.boundary.title': 'Não foi possível abrir esta página',
  'error.boundary.retry': 'Tentar novamente',
  'error.boundary.reload': 'Recarregar o aplicativo',
  'error.boundary.chunk':
    'Parte do aplicativo não foi baixada. Recarregar traz a versão atual.',

  'nav.groupWorkspace': 'Área de trabalho',
  'nav.groupManage': 'Gerenciar',
  'nav.dashboard': 'Painel',
  'nav.dashboardHint': 'Projetos e métricas',
  'nav.taskMenu': 'Menu de tarefas',
  'nav.taskMenuHint': 'Seu dia, hora a hora',
  'nav.notesBoard': 'Mural de notas',
  'nav.notesBoardHint': 'Sua mesa de recados',
  'nav.meetings': 'Reuniões',
  'nav.meetingsHint': 'Onde você é esperado',
  'nav.organizations': 'Organizações',
  'nav.returnToMenu': 'Devolver {label} ao menu',
  'nav.home': 'Início',
  'nav.projects': 'Projetos',
  'nav.tasks': 'Tarefas',
  'nav.board': 'Quadro',
  'nav.notes': 'Mural',
  'nav.themes': 'Temas',
  'nav.settings': 'Configurações',
  'nav.recycleBin': 'Lixeira',
  'nav.invitations': 'Convites',
  'nav.openMenu': 'Abrir menu',
  'nav.closeMenu': 'Fechar menu',
  'nav.account': 'Conta',
  'nav.signOut': 'Sair',
  'nav.signingOut': 'Saindo…',
  'nav.notifications': 'Notificações',
  'nav.search': 'Buscar',
  'nav.newProject': 'Novo projeto',
  'nav.accountMenu': 'Menu da conta',
  'nav.profileSettings': 'Perfil e configurações',
  'nav.realtimeConnected': 'Tempo real conectado',
  'nav.realtimeOffline': 'Tempo real offline',
  'nav.live': 'Ao vivo',
  'nav.offline': 'Offline',

  'dash.allProjects': 'Todos os projetos',
  'dash.yourProjects': 'Seus projetos',
  'dash.newProject': 'Novo projeto',
  'dash.showArchived': 'Arquivados',
  'dash.greeting': 'Que bom te ver, {name}.',
  'dash.greetingFallback': 'por aqui',
  'agenda.nothingCompleted': 'Nada concluído ainda',
  'agenda.nothingScheduled': 'Nada agendado',
  'agenda.noNotesBody': 'Ninguém anexou uma nota às suas tarefas ainda.',
  'agenda.scheduledBody':
    'Tarefas com horário de início ou prazo aparecem aqui, ordenadas por hora.',
  'bin.tasks': 'Tarefas',
  'bin.notes': 'Notas',
  'bin.noDeletedTasks': 'Nenhuma tarefa excluída',
  'bin.noDeletedNotes': 'Nenhuma nota excluída',
  'bin.deletedTasksBody': 'Tarefas excluídas param aqui primeiro, para nada se perder por acidente.',
  'bin.deletedNotesBody':
    'Recados que você remove de um mural esperam aqui antes de sumirem de vez.',
  'bin.projectWhiteboard': 'Lousa do projeto',
  'bin.pinnedToTask': 'Fixada em uma tarefa',
  'bin.confirmPurgeNote': 'Excluir esta nota definitivamente?',
  'invites.invitedYou': 'convidou você',
  'rail.noDeadline': 'Sem prazo',
  'rail.openCount': 'abertas',
  'projectDash.totalTasks': 'Total de tarefas',
  'projectDash.dueThisWeek': 'Vencem esta semana',
  'projectDash.noDate': 'Sem data',
  'roster.title': 'Equipe',
  'roster.memberCount': '{count} integrante(s) neste projeto.',
  'roster.loadingMembers': 'Buscando a equipe…',
  'roster.invite': 'Convidar',
  'roster.you': '(você)',
  'roster.remove': 'Remover {name}',
  'roster.pending': 'Convites pendentes',
  'roster.invitedRelative': 'convidado',
  'roster.inviteTitle': 'Convidar para a equipe',
  'roster.inviteSubtitle':
    'Busque usuários cadastrados pelo nome, ou cole um endereço de e-mail exato.',
  'roster.search': 'Buscar',
  'roster.searchPlaceholder': 'Nome ou e-mail',
  'roster.role': 'Função',
  'roster.roleAdmin': 'Administrador',
  'roster.roleAdminHint': 'Pode convidar pessoas, marcar reuniões e gerenciar tarefas',
  'roster.roleMember': 'Membro',
  'roster.roleMemberHint': 'Vê o projeto e trabalha no que for atribuído a ele',
  'roster.searching': 'Buscando…',
  'roster.noMatch': 'Nenhum resultado',
  'roster.noMatchBody': 'Somente contas verificadas do Task Studio podem ser convidadas.',

  'project.newTitle': 'Novo projeto',
  'project.pin': 'Fixar projeto',
  'project.unpin': 'Desafixar projeto',
  'project.doneCount': '{count} concluídas',
  'project.openCount': '{count} em aberto',
  'project.newSubtitle': 'Você se torna o dono e pode convidar sua equipe logo em seguida.',
  'project.create': 'Criar projeto',
  'project.name': 'Nome',
  'project.namePlaceholder': 'Lançamento do produto',
  'project.description': 'Descrição',
  'project.descriptionPlaceholder': 'Sobre o que é este projeto?',
  'project.accentColour': 'Cor de destaque',
  'project.settingsTitle': 'Configurações do projeto',
  'project.settingsSubtitle': 'Renomeie, mude a cor ou mova para a lixeira.',
  'project.saveChanges': 'Salvar alterações',
  'project.modeBlank': 'Projeto em branco',

  'project.window': 'Período previsto',
  'project.startsAt': 'Começa',
  'project.endsAt': 'Termina',
  'project.windowHint':
    'Os dois são opcionais. A data de término é um teto: nenhuma tarefa pode ser marcada depois dela, e ela não pode ser puxada para antes de um trabalho que já está agendado.',
  'project.endsBeforeTasks': 'Já existe trabalho agendado depois dessa data.',
  'project.windowRange': '{from} – {to}',
  'project.windowFrom': 'A partir de {from}',
  'project.windowUntil': 'Até {to}',
  'project.windowDaysLeft': 'faltam {count} dias',
  'project.windowOverrun': 'passou da data de término',
  'project.modeGithub': 'Do GitHub',

  'github.repository': 'Repositório',
  'github.look': 'Buscar',
  'github.hint':
    'Cole um repositório público — github.com/dono/nome, ou só dono/nome. Nada é criado antes de você ver o que foi encontrado.',
  'github.noDescription': 'Sem descrição no GitHub.',
  'github.openIssues': '{count} issues abertas',
  'github.archivedWarning':
    'Este repositório está arquivado no GitHub. A importação funciona — o projeto só nasce como registro de um trabalho concluído.',
  'github.pagesTitle': 'Páginas que seriam criadas',
  'github.noPages': 'Nenhum README ou changelog encontrado — o projeto começa com a aba Documentos vazia.',
  'github.contributorsTitle': '{count} colaboradores podem ser convidados',
  'github.contributorsHint': 'Só dá para reconhecer quem entra aqui pelo GitHub.',
  'github.willInvite': '{name} será convidado',
  'github.noAccount': '{login} não tem conta do Task Studio ligada ao GitHub',
  'github.useAssistant': 'Deixar o assistente ler o repositório',
  'github.import': 'Criar o projeto',
  'github.backgroundHint':
    'Isso continua em segundo plano — pode fechar e seguir trabalhando. Um cartão no canto acompanha o progresso, e dá para cancelar a qualquer momento.',
  'github.importedSummary': '{tasks} tarefas · {documents} páginas · {invited} convidados',
  'github.previewFailed': 'Não foi possível ler esse repositório.',
  'github.importFailed': 'Não foi possível importar esse repositório.',
  'github.cancelFailed': 'Não foi possível cancelar essa importação.',

  'importTracker.title': 'Importando um repositório',
  'importTracker.aRepository': 'o repositório',
  'importTracker.dismiss': 'Esconder',
  'importTracker.collapse': 'Recolher',
  'importTracker.expand': 'Abrir',
  'importTracker.openProject': 'Abrir projeto',
  'importTracker.cancelling': 'Cancelando…',
  'importTracker.cancelled': 'Cancelado. Nada foi criado.',
  'importTracker.step.queued': 'Aguardando para começar…',
  'importTracker.step.resolving': 'Localizando o repositório…',
  'importTracker.step.reading': 'Lendo o repositório…',
  'importTracker.step.analysing': 'O assistente está lendo…',
  'importTracker.step.writing': 'Escrevendo o projeto…',
  'importTracker.step.inviting': 'Convidando quem contribuiu…',
  'importTracker.step.done': 'Pronto',

  'project.filedUnderTitle': 'Arquivado em',
  'project.unfileExplain':
    'Tira o projeto desta empresa e deixa todo o resto no lugar — a equipe, as tarefas, as páginas e os times continuam. Dá para arquivá-lo de novo pela página da empresa quando quiser.',
  'project.unfile': 'Remover da organização',
  'project.unfileConfirm': 'Confirmar — remover',
  'project.archiveTitle': 'Guardar este projeto',
  'project.archivedTitle': 'Este projeto está arquivado',
  'project.archiveExplain':
    'Tira o projeto do painel e não mexe em nada que está dentro dele — a equipe, o quadro, as páginas e o histórico ficam exatamente como estão. Nada é apagado e você pode tirá-lo do arquivo quando quiser.',
  'project.unarchiveExplain':
    'Ele fica fora do painel até você tirá-lo do arquivo. Tudo que está dentro dele continua intacto.',
  'project.archive': 'Arquivar projeto',
  'project.archivedChip': 'Arquivado',
  'project.unarchive': 'Tirar do arquivo',
  'project.dangerZone': 'Encerrando este projeto',

  'project.finishProject': 'Concluir projeto',
  'project.finishExplain':
    'Mantém o nome, a descrição, a cor, as pessoas e os times como registro, e apaga todo o resto: tarefas, páginas, notas, a lousa, a conversa, as reuniões e a capa. Nada do que for apagado pode ser recuperado.',
  'project.finishConfirmBody':
    'Isso apaga permanentemente todas as tarefas, páginas, notas, traços da lousa, mensagens, reuniões e arquivos enviados neste projeto. Só o nome, a descrição, a cor, a equipe e os times permanecem. Digite sua senha para confirmar.',
  'project.finishConfirmLabel': 'Sua senha',
  'project.finishConfirmAction': 'Concluir',
  'project.finishedToast':
    'Projeto concluído. {items} item(ns) apagado(s), incluindo {tasks} tarefa(s) e {documents} página(s).',
  'project.finished': 'Concluído',
  'project.finishedOn': 'Concluído em {date}',
  'project.finishedBanner':
    'Este projeto está concluído. Ele fica somente leitura até alguém reabri-lo.',
  'project.reopen': 'Reabrir projeto',
  'project.reopenExplain':
    'Coloca o projeto de volta em atividade com as pessoas e os times. Nada do que foi apagado volta.',
  'project.reopenedToast': 'Projeto reaberto.',
  'project.archiveWhileFinished':
    'Um projeto concluído já sai do quadro — reabra antes se quiser arquivá-lo.',

  'project.deleteExplain':
    'Tudo que está nele — tarefas, notas, documentos e a conversa — vai junto. Ele fica na sua lixeira, onde pode ser restaurado por 30 dias antes de ser destruído.',
  'project.deleteProject': 'Excluir projeto',
  'project.deleteConfirmBody': 'Digite {name} para confirmar.',
  'project.deleteConfirmLabel': 'Nome do projeto',
  'project.deleteConfirmAction': 'Excluir',

  'settings.title': 'Configurações',
  'settings.heading': 'Perfil e preferências',
  'settings.avatar': 'Foto de perfil',
  'settings.uploading': 'Enviando…',
  'settings.uploadImage': 'Enviar imagem',
  'settings.remove': 'Remover',
  'settings.identity': 'Identidade',
  'settings.bio': 'Sobre você',
  'settings.bioPlaceholder': 'No que você está trabalhando?',
  'settings.saveProfile': 'Salvar perfil',
  'settings.theme': 'Tema',
  'settings.appearance': 'Aparência',
  'settings.light': 'Claro',
  'settings.dark': 'Escuro',
  'settings.system': 'Sistema',
  'settings.password': 'Senha',
  'settings.integrations': 'Serviços conectados',
  'settings.integrationsHint': 'O que o Task Studio pode acessar em seu nome.',

  'cli.title': 'Task Studio CLI',
  'cli.body':
    'Crie projetos, gere tarefas e conclua-as com um commit, direto do repositório em que você já está.',
  'cli.projectBody':
    'Crie tarefas a partir deste repositório, mude para a branch de uma tarefa e conclua-a com um commit — sem sair do editor.',
  'cli.install': 'Instalar e entrar',
  'cli.thenInAnyRepo': 'Depois, em qualquer repositório',
  'cli.inThisRepo': 'Depois, neste repositório',
  'cli.accountHint':
    'Entrar abre o navegador para aprovar esta máquina — nenhuma senha é digitada no terminal. O que fica é um token que você pode revogar abaixo.',
  'cli.projectHint':
    'O “init” liga o repositório em que você está a este projeto. O “hook install” faz um commit comum concluir a tarefa da branch.',
  'cli.showCommands': 'Comandos',
  'cli.hideCommands': 'Ocultar',
  'cli.docs': 'Ver a documentação',
  'cliAuth.heading': 'Conectar um terminal',
  'cliAuth.subheading': 'Confira se a máquina abaixo é a que você está usando.',
  'cliAuth.codeLabel': 'O código que o terminal está mostrando',
  'cliAuth.continue': 'Continuar',
  'cliAuth.unnamedDevice': 'Uma máquina sem nome',
  'cliAuth.grantWarning':
    'Ao aprovar, esta máquina recebe um token que faz tudo o que você faz, até você revogá-lo nas Configurações.',
  'cliAuth.approve': 'Aprovar',
  'cliAuth.deny': 'Não fui eu',
  'cliAuth.approvedTitle': 'Conectado',
  'cliAuth.approvedBody': 'Volte ao terminal — ele já tem o que precisava.',
  'cliAuth.deniedTitle': 'Recusado',
  'cliAuth.deniedBody': 'Nada foi entregue àquela máquina.',
  'cliAuth.expiredTitle': 'Esse código não está mais esperando',
  'cliAuth.expiredBody':
    'Os códigos duram dez minutos. Rode taskstudio login de novo para gerar outro.',
  'cliAuth.tryAnother': 'Digitar outro código',
  'cli.machines': 'Máquinas conectadas',
  'cli.lastUsed': 'usado {when}',
  'cli.neverUsed': 'nunca usado',
  'cli.expires': 'expira {when}',
  'cli.neverExpires': 'sem expiração',
  'cli.noMachines': 'Nenhuma máquina conectada',
  'cli.noMachinesBody': 'Nada fora do seu navegador consegue acessar sua conta agora.',
  'cli.machinesError': 'Não foi possível carregar suas máquinas conectadas',
  'cli.machinesErrorBody':
    'Esta lista é o único lugar onde você vê o que consegue acessar sua conta, então vale tentar de novo.',
  'cli.revoke': 'Revogar',
  'cli.revokeNamed': 'Revogar {name}',
  'cli.revokeTitle': 'Revogar “{name}”?',
  'cli.revokeBody': 'Isso não pode ser desfeito.',
  'cli.revokeDetail':
    '{name} deixará de acessar sua conta imediatamente. Quem estiver usando essa máquina terá de rodar “taskstudio login” de novo para obter um novo token.',
  'cli.revoked': 'Essa máquina foi desconectada.',
  'cli.copy': 'Copiar',
  'cli.copied': 'Copiado',
  'cli.copyFailed': 'Não foi possível copiar. Selecione a linha e copie manualmente.',
  'cli.commandsRegion': 'Comandos do Task Studio CLI',
  'cli.commandScrollable': 'Comando — role para o lado para ver o resto',

  // ---- Spotify ------------------------------------------------------------
  'spotify.title': 'Spotify',
  'spotify.pitch':
    'Coloca um player pequeno na sua tela: o que está tocando, pular e pausar, volume e uma busca. Só seu — ninguém nos seus projetos vê nem mexe nele.',
  'spotify.connect': 'Conectar o Spotify',
  'spotify.connected': 'O Spotify está conectado.',
  'spotify.cancelled': 'Nenhuma conta do Spotify foi conectada.',
  'spotify.failed': 'Não foi possível conectar o Spotify. Tente de novo.',
  'spotify.connectedAs': 'Conectado como {name}',
  'spotify.disconnect': 'Desconectar',
  'spotify.disconnected': 'O Spotify foi desconectado.',
  'spotify.disconnectFailed': 'Não foi possível desconectar o Spotify.',
  'spotify.updateFailed': 'Não foi possível alterar isso.',
  'spotify.commandFailed': 'O Spotify não aceitou esse comando.',
  'spotify.showPlayer': 'Mostrar o player',
  'spotify.showPlayerHint': 'Mantém a conexão e esconde o player da sua tela.',
  'spotify.unavailable': 'Não está ativo aqui',
  'spotify.unavailableHint':
    'Esta instalação não tem um aplicativo do Spotify configurado, então não há o que conectar.',
  'spotify.open': 'Abrir o player',
  'spotify.close': 'Fechar o player',
  'spotify.play': 'Tocar',
  'spotify.pause': 'Pausar',
  'spotify.next': 'Próxima faixa',
  'spotify.previous': 'Faixa anterior',
  'spotify.idle': 'Nada tocando',
  'spotify.premiumOnly': 'O Spotify só permite pular e mudar o volume no Premium.',
  'spotify.searchPlaceholder': 'Buscar músicas ou artistas',
  'spotify.queued': '“{name}” entrou na fila — toca em seguida.',
  'spotify.queueFailed': 'O Spotify não colocou isso na fila.',
  'spotify.pin': 'Manter o player aberto',
  'spotify.unpin': 'Deixar o player fechar de novo',
  'spotify.noDevice': 'Abra o Spotify em algum aparelho para usar isto.',
  'spotify.notice':
    'O player controla o aparelho em que o Spotify já está tocando. Nada do que você ouve fica guardado aqui.',

  'calendar.title': 'Google Agenda',
  'calendar.pitch':
    'Espelhe na sua própria agenda as reuniões em que você é esperado, e deixe as alterações feitas lá voltarem para cá.',
  'calendar.connect': 'Conectar o Google Agenda',
  'calendar.connected': 'Sua agenda está conectada.',
  'calendar.cancelled': 'Nenhuma agenda foi conectada.',
  'calendar.connectFailed': 'Não foi possível conectar essa agenda.',
  'calendar.noRefreshToken':
    'O Google não concedeu acesso offline, então a sincronização em segundo plano não é possível. Tente conectar de novo e aceite todas as etapas.',
  'calendar.unavailable':
    'A sincronização de agenda não está configurada nesta instalação. Quem a administra precisa definir as credenciais do Google e uma chave de criptografia.',

  'calendar.enabled': 'Manter esta agenda sincronizada',
  'calendar.push': 'Enviar minhas reuniões para o Google',
  'calendar.pull': 'Aplicar alterações feitas no Google',
  'calendar.pushHint':
    'As reuniões em que você é esperado aparecem na sua agenda do Task Studio, e saem dela quando são concluídas ou canceladas.',
  'calendar.pullHint':
    'Mover ou cancelar um evento espelhado no Google atualiza a reunião aqui — se você tiver permissão para alterá-la.',

  'calendar.lastSynced': 'Sincronizado {when}',
  'calendar.neverSynced': 'Ainda não sincronizado',
  'calendar.syncNow': 'Sincronizar agora',
  'calendar.syncNowHint': 'Traz as alterações do Google e envia o que estiver faltando.',
  'calendar.syncedChanges': '{count} reuniões atualizadas a partir da sua agenda.',
  'calendar.syncedClean': 'Está tudo em dia.',
  'calendar.syncFailed': 'Não foi possível sincronizar com o Google.',
  'calendar.updateFailed': 'Não foi possível salvar essa preferência.',

  'calendar.disconnect': 'Desconectar',
  'calendar.disconnectExplain':
    'O Task Studio vai parar de sincronizar. A agenda que ele criou na sua conta do Google pode ir junto, ou ficar como registro.',
  'calendar.disconnectAndRemove': 'Desconectar e apagar a agenda',
  'calendar.disconnectKeep': 'Desconectar e manter',
  'calendar.disconnected': 'Sua agenda foi desconectada.',
  'calendar.disconnectFailed': 'Não foi possível desconectar essa agenda.',

  'calendar.badgeOn': 'Sincronizado',
  'calendar.badgeOff': 'Sincronizar Google Agenda',
  'calendar.badgeOnHint':
    'Suas reuniões e o seu Google Agenda ficam em dia nos dois sentidos. Desative em configurações.',
  'calendar.badgeOffHint':
    'Mantenha estas reuniões e o seu Google Agenda em dia, nos dois sentidos.',


  // ---- O repositório que o projeto representa -----------------------------
  'repo.open': 'Abrir o repositório',
  'repo.connectedTitle': 'O repositório está conectado',
  'repo.urlLabel': 'Endereço do repositório',
  'repo.publicOnly':
    'Só repositórios públicos — esta instalação lê o GitHub sem nenhuma credencial sua.',
  'repo.connect': 'Conectar um repositório',
  'repo.connectTitle': 'Conectar este projeto ao GitHub',
  'repo.connectBody':
    'Cole um repositório público. Ele vira um link ao lado do nome do projeto, e as tarefas daqui podem indicar a branch em que o trabalho acontece.',
  'repo.placeholder': 'github.com/dono/nome',
  'repo.connectAction': 'Conectar',
  'repo.connected': 'Repositório conectado.',
  'repo.connectFailed': 'Não foi possível conectar esse repositório.',
  'repo.disconnect': 'Desconectar',
  'repo.disconnected': 'Repositório desconectado.',
  'repo.disconnectFailed': 'Não foi possível desconectar esse repositório.',
  'repo.disconnectHint': 'As branches já escritas nas tarefas são mantidas.',
  'repo.adminOnly': 'Só o dono ou um admin pode conectar um repositório.',

  // ---- A branch em que a tarefa acontece ----------------------------------
  'task.branch': 'Branch',
  'task.branchHint': 'A branch em que este trabalho acontece. Ela ainda não precisa existir.',
  'task.branchPlaceholder': 'feature/webhook-cobranca',
  'task.openBranch': 'Abrir esta branch no GitHub',
  'task.commitTitle': 'Concluída em {branch}',
  'task.commitBody':
    'Esta tarefa indicava uma branch. Abra no GitHub para commitar, abrir um pull request ou fazer o merge.',
  'task.commitOpen': 'Abrir a branch',
  'task.commitLater': 'Agora não',

  // ---- Central de conexões -------------------------------------------------
  'connections.broadcast': 'Transmissão',
  'connections.sync': 'Sincronização',
  'connections.editor': 'Seu editor',
  'connections.features': 'Recursos',
  'connections.connect': 'Conectar',
  'connections.connected': 'Conectado',
  'connections.adminOnly': 'As conexões são gerenciadas pelo dono e pelos admins do projeto.',
  'connections.adminOnlyBody':
    'Tudo aqui envia o trabalho deste projeto para fora dele, então fica no mesmo nível de gerenciar a equipe.',
  'connections.personal': 'Pessoal',
  'connections.personalHint': 'Conectado à sua conta, não a este projeto.',
  'connections.soon': 'Ainda não',
  'connections.svc.discord': 'Discord',
  'connections.svc.slack': 'Slack',
  'connections.svc.webhook': 'Endpoint próprio',
  'connections.svc.googleCalendar': 'Google Agenda',
  'connections.svc.github': 'GitHub',
  'connections.svc.trello': 'Trello',

  // ---- Denunciar alguém ----------------------------------------------------
  'report.action': 'Denunciar esta pessoa',
  'report.title': 'Denunciar {name}',
  'report.body':
    'Isto vai para quem administra esta instalação. Eles veem o que você escrever e o seu nome; {name} nunca é avisado de que existe uma denúncia.',
  'report.reasonLabel': 'O que aconteceu',
  'report.placeholder': 'Descreva o que a pessoa fez, e onde.',
  'report.submit': 'Enviar denúncia',
  'report.sent': 'Denúncia enviada. Obrigado.',
  'report.failed': 'Não foi possível enviar a denúncia.',
  'report.already': 'Denunciado',
  'report.alreadyHint':
    'Você já denunciou esta pessoa. Denunciar de novo substitui o que você escreveu.',
  'report.tooShort': 'Escreva um pouco mais — uma linha basta.',

  // ---- Orientando o assistente da importação ------------------------------
  'github.guidanceLabel': 'No que ele deve focar?',
  'github.guidanceHint':
    'Opcional. Aponte uma pasta, ou diga para que serve este projeto. Isso orienta o que o assistente lê — não muda o que ele produz.',
  'github.guidancePlaceholder': 'Foque em packages/api — esta é a reescrita da cobrança.',
  'github.overviewTitle': 'O que seria importado',
  'github.overviewBack': 'Buscar outro',

  'landing.nav.docs': 'Docs',
  'docs.contents': 'Conteúdo',
  'docs.backHome': 'Voltar para a página inicial',
  'landing.nav.skip': 'Pular para o conteúdo',
  'landing.nav.how': 'Como funciona',
  'landing.nav.connects': 'Conexões',
  'landing.nav.themes': 'Temas',
  'landing.nav.inside': 'O que tem dentro',
  'landing.nav.signIn': 'Entrar',
  'landing.nav.getStarted': 'Começar',
  'landing.nav.home': 'Task Studio — voltar ao topo',
  'landing.pitico.before': 'Uma solução',
  'landing.pitico.after': '',
  'landing.pitico.letter': 'P.',
  'landing.pitico.word': 'Pico.',
  'landing.pitico.aria': 'Pico — o estúdio por trás do Task Studio',
  'landing.pitico.body':
    'O Task Studio é feito por um estúdio pequeno, que faz software do jeito que quer usar.',

  'landing.word.tasks': 'tarefas',
  'landing.word.meetings': 'reuniões',
  'landing.word.notes': 'anotações',
  'landing.word.projects': 'projetos',
  'landing.word.work': 'trabalho',

  /*
   * Sem possessivo, de propósito.
   *
   * O inglês carrega o "your" aqui porque uma palavra serve para os cinco
   * substantivos. O português precisaria de três — suas tarefas, seus projetos,
   * seu trabalho — e trocar o determinante junto com o substantivo era
   * exatamente o que quebrava a linha: duas colunas de largura variável girando
   * ao mesmo tempo. O título fica com o substantivo sozinho, que é o que a
   * frase estava dizendo de qualquer jeito.
   */
  'landing.hero.titleLead': 'Organize',
  'landing.hero.titleTail': 'como você faria no papel.',
  'landing.hero.body':
    'O Task Studio é um estúdio de projetos que se comporta como uma mesa. Arraste o trabalho entre colunas, cole notas na parede, marque a sala e deixe tudo conversar com as ferramentas que você já usa.',
  'landing.hero.primary': 'Começar de graça',
  'landing.hero.secondary': 'Ver funcionando',


  'landing.how.page': 'Página {page} de {total}',
  'landing.how.goToPage': 'Ir para a página {page}',
  'landing.how.previous': 'Recursos anteriores',
  'landing.how.next': 'Mais recursos',

  'landing.demo.notesTab': 'Mural de notas',
  'landing.demo.notesTitle': 'Cole na parede e ligue ao resto',
  'landing.notes.one': 'Preço precisa de decisão',
  'landing.notes.two': 'Falar com o Bruno sobre a nota',
  'landing.notes.three': 'Parado no contrato',

  'landing.demo.meetTab': 'Reuniões',
  'landing.demo.meetTitle': 'Marque a sala aqui, veja no celular',
  'landing.meet.here': 'Task Studio',
  'landing.meet.there': 'Sua agenda',
  'landing.meet.title': 'Revisão de cobrança',
  'landing.meet.when': 'Quinta, 15:00',
  'landing.meet.onPhone': 'Em todos os aparelhos',

  'landing.demo.pagesTab': 'Páginas',
  'landing.demo.pagesTitle': 'Escreva onde o trabalho está',
  'landing.pages.title': 'Reescrita da cobrança — briefing',
  'landing.pages.saved': 'Salvo',
  'landing.pages.hint': 'Ou solte um PDF e ele fica exatamente como está.',

  'landing.demo.undoTab': 'Histórico',
  'landing.demo.undoTitle': 'Toda mudança, e um caminho de volta',
  'landing.undo.task': 'Publicar a página de cobrança',
  'landing.undo.log': 'Histórico',
  'landing.undo.deleted': 'Ana mandou uma tarefa para a lixeira',
  'landing.undo.restored': 'Ana mandou uma tarefa para a lixeira',
  'landing.undo.button': 'Desfazer',
  'landing.undo.done': 'Desfeito',

  'landing.demo.commitTab': 'Seu editor',
  'landing.demo.commitTitle': 'Commite como sempre, e o quadro acompanha',
  'landing.commit.checking': 'taskstudio: lendo o commit…',
  'landing.commit.task': 'Limitar a taxa nas rotas de importação',
  'landing.commit.branch': 'na fix/import-rate-limit',
  'landing.commit.closed': 'Tarefa concluída.',

  'landing.demo.drawTab': 'Quadro branco',
  'landing.demo.drawTitle': 'Desenhe o que não dá para descrever',
  'landing.draw.hint': 'Caneta, borracha e notas na mesma superfície.',
  'landing.demo.boardTab': 'Quadro',
  'landing.demo.boardTitle': 'Pegue uma tarefa e ponha onde ela pertence',

  'landing.demo.importTab': 'Novo projeto',
  'landing.demo.importTitle': 'Transforme um repositório em projeto, em segundo plano',

  'landing.demo.chatTab': 'Chat do projeto',
  'landing.demo.chatTitle': 'Converse sobre o trabalho, do lado do trabalho',

  'landing.board.todo': 'A fazer',
  'landing.board.doing': 'Fazendo',
  'landing.board.done': 'Feito',
  'landing.board.mover': 'Publicar a página de cobrança',
  'landing.board.due': 'Prazo sexta',
  'landing.board.cardA': 'Escrever o texto de preços',
  'landing.board.cardB': 'Revisar o contrato da API',
  'landing.board.cardC': 'Escolher o meio de pagamento',

  'landing.import.resolving': 'Localizando o repositório…',
  'landing.import.reading': 'Lendo o repositório…',
  'landing.import.analysing': 'O assistente está lendo…',
  'landing.import.writing': 'Escrevendo o projeto…',
  'landing.import.done': 'Projeto pronto',
  'landing.import.tasks': '3 tarefas',
  'landing.import.pages': '4 páginas',
  'landing.import.invited': '2 convidados',

  'landing.chat.project': 'Reescrita da cobrança',
  'landing.chat.online': '3 online',
  'landing.chat.typing': 'Digitando',
  'landing.chat.placeholder': 'Escreva uma mensagem…',
  'landing.chat.one': 'A doc do provedor mudou — a gente ainda entrega sexta?',
  'landing.chat.two': 'Só a parte do webhook. Meio dia, no máximo.',
  'landing.chat.three': 'Então sexta está de pé. Movendo para Fazendo.',


  'landing.svc.github': 'GitHub',
  'landing.svc.githubWhat': 'Vire um repositório público em projeto',
  'landing.svc.figma': 'Figma',
  'landing.svc.figmaWhat': 'Leia um arquivo de design e exporte qualquer frame dele',
  'landing.svc.googleCalendar': 'Google Agenda',
  'landing.svc.googleCalendarWhat': 'Sincronização nos dois sentidos',
  'landing.svc.feed': 'Link de agenda',
  'landing.svc.feedWhat': 'Assine pelo Apple, Outlook ou qualquer outro',
  'landing.svc.discord': 'Discord',
  'landing.svc.discordWhat': 'Avise um canal quando algo for concluído',
  'landing.svc.slack': 'Slack',
  'landing.svc.slackWhat': 'O mesmo, no seu workspace',
  'landing.svc.trello': 'Trello',
  'landing.svc.trelloWhat': 'Importe um quadro inteiro com suas listas',
  'landing.svc.exports': 'Jira, Asana e CSV',
  'landing.svc.exportsWhat': 'Traga um quadro por exportação',
  'landing.svc.webhooks': 'Webhooks e API',
  'landing.svc.webhooksWhat': 'Eventos assinados e tokens para seus scripts',

  'landing.themes.title': 'Vários jeitos de olhar para o mesmo trabalho',
  'landing.themes.pick': 'Escolha um tema para ver',
  'landing.themes.light': 'Claro',
  'landing.themes.dark': 'Escuro',
  'landing.themes.hint': 'Arraste para comparar',
  'landing.themes.apply': 'Testar este tema',
  'landing.themes.applied': 'Aplicado',
  'landing.themes.applyHint': 'Aplica nesta página — e no app quando você entrar.',
  'landing.themes.preview': 'Pré-visualizar este tema',
  'landing.themes.previewing': 'Pré-visualizando',
  'landing.themes.previewHint': 'Só uma prévia nesta página — este tema faz parte dos planos pagos.',
  'landing.themes.dragonVerse': 'Shān duì shān lái, yá duì yá\nMìfēng cǎihuā shēnshān lǐ lái\nMìfēng běn wèi cǎihuā sǐ\nLiángshānbo wèi Zhù Yīngtái',
  'theme.switchLabel': 'Esquema de cores',
  'landing.themes.count': '{index} de {total}',

  'landing.inside.title': 'Um estúdio, não uma planilha',

  'landing.note.boards': 'Dois quadros, um só conjunto de tarefas',
  'landing.note.notes': 'Uma parede de post-its',
  'landing.note.meetings': 'Reuniões que chegam na sua agenda',
  'landing.note.docs': 'Páginas que vivem com o projeto',
  'landing.note.undo': 'Um histórico que dá para desfazer',
  'landing.note.skins': 'Vários jeitos de aparecer',

  'landing.cta.primary': 'Criar uma conta',
  'landing.cta.secondary': 'Já tenho conta',

  'landing.footer.talk': 'Vamos conversar.',
  'landing.footer.coffee': 'Puxe uma cadeira.',
  'landing.footer.openSource': 'Código aberto',
  'landing.footer.themes': 'Temas',
  'landing.footer.startLabel': 'Comece de graça',
  'landing.footer.startAction': 'Continuar para o cadastro',
  'landing.footer.rights': '© 2026 Task Studio',
  'landing.footer.note1Title': 'Acompanhe',
  'landing.footer.note1Body': 'Cada mudança, à vista.',
  'landing.footer.note2Title': 'Linha direta',
  'landing.footer.note2Body': 'Achou algo quebrado? Conte pra gente.',
  'landing.footer.note3Title': 'Leia a documentação',
  'landing.footer.note3Body': 'Quadros, projetos e a CLI.',
  'landing.footer.tagline': 'Trabalho que se comporta como papel.',
  'landing.footer.source': 'Código',
  'landing.footer.by': 'por',
  'landing.footer.byWho': 'Instagram — abre em uma nova aba',

  'project.modeBoard': 'De um quadro',
  'boardImport.drop': 'Solte aqui a exportação de um quadro',
  'boardImport.formats': 'Um .json exportado do Trello, ou um .csv do Jira, Asana ou Trello.',
  'boardImport.choose': 'Escolher arquivo',
  'boardImport.clear': 'Remover este arquivo',
  'boardImport.readsTrello': 'lido como quadro do Trello',
  'boardImport.readsCsv': 'lido como exportação de quadro',
  'boardImport.whatComesTitle': 'O que vem junto',
  'boardImport.whatComes':
    'Listas viram colunas, cartões viram tarefas, e prazos e cartões concluídos são preservados.',
  'boardImport.whatDoesNot':
    'Etiquetas, checklists e responsáveis vão para a descrição de cada tarefa em vez de serem descartados — este app tem um único conjunto de colunas e nenhuma forma de representar o resto.',
  'boardImport.start': 'Criar o projeto',
  'boardImport.summary': '{tasks} tarefas · {documents} colunas',
  'boardImport.failed': 'Não foi possível ler essa exportação.',

  'feed.title': 'Link de agenda',
  'feed.pitch':
    'Um link privado que seu app de agenda assina. Funciona com Apple Calendar, Outlook, Thunderbird e Google — sem conta para conectar.',
  'feed.create': 'Criar link de agenda',
  'feed.copyNow': 'Copie agora — ele aparece uma única vez e não pode ser consultado depois.',
  'feed.copy': 'Copiar',
  'feed.copied': 'Copiado',
  'feed.copyFailed': 'Não foi possível copiar. Selecione o link e copie manualmente.',
  'feed.howTo':
    'No seu app de agenda, procure por “Assinar agenda”, “Adicionar por URL” ou “Da internet”, e cole o link lá.',
  'feed.lastFetched': 'Última leitura {when}',
  'feed.neverFetched': 'Nada assinou o link ainda',
  'feed.rotate': 'Novo link',
  'feed.rotateExplain':
    'Um link novo derruba o antigo. Onde você já tiver assinado vai parar de atualizar até colar o novo.',
  'feed.rotateConfirm': 'Substituir',
  'feed.revoke': 'Excluir o link',
  'feed.revoked': 'O link de agenda foi excluído.',
  'feed.issueFailed': 'Não foi possível criar o link de agenda.',
  'feed.revokeFailed': 'Não foi possível excluir o link de agenda.',

  'tokens.title': 'Tokens de API',
  'tokens.pitch':
    'Para scripts, tarefas agendadas e automações — tudo que não é um navegador e não consegue fazer login.',
  'tokens.new': 'Novo token',
  'tokens.name': 'Para que serve',
  'tokens.namePlaceholder': 'Script de deploy',
  'tokens.lifetime': 'Expira',
  'tokens.days30': 'Em 30 dias',
  'tokens.days90': 'Em 90 dias',
  'tokens.days365': 'Em um ano',
  'tokens.never': 'Nunca',
  'tokens.create': 'Criar token',
  'tokens.scopeWarning':
    'Um token pode fazer tudo o que você pode. Defina uma validade e revogue assim que não precisar mais dele.',
  'tokens.copyNow': 'Copie “{name}” agora — aparece uma única vez e não pode ser consultado depois.',
  'tokens.copy': 'Copiar',
  'tokens.copied': 'Copiado',
  'tokens.copyFailed': 'Não foi possível copiar. Selecione o token e copie manualmente.',
  'tokens.dismiss': 'Pronto',
  'tokens.lastUsed': 'usado {when}',
  'tokens.neverUsed': 'nunca usado',
  'tokens.expires': 'expira {when}',
  'tokens.revoke': 'Revogar',
  'tokens.revoked': 'Esse token não funciona mais.',
  'tokens.revokedTag': 'Revogado',
  'tokens.expiredTag': 'Expirado',
  'tokens.createFailed': 'Não foi possível criar esse token.',
  'tokens.revokeFailed': 'Não foi possível revogar esse token.',

  'project.tabConnections': 'Conexões',
  'webhooks.title': 'Webhooks',
  'webhooks.subtitle': 'Publique os eventos deste projeto no Discord, no Slack ou onde quiser.',
  'webhooks.none': 'Ninguém está ouvindo ainda',
  'webhooks.noneBody':
    'Adicione uma URL e este projeto vai publicar lá quando uma tarefa for concluída, uma reunião for marcada ou alguém entrar.',
  'webhooks.new': 'Adicionar webhook',
  'webhooks.add': 'Adicionar',
  'webhooks.url': 'Para onde publicar',
  'webhooks.urlHint':
    'Cole uma URL de webhook do Discord ou do Slack, ou qualquer endereço HTTPS que aceite POST. Discord e Slack são formatados automaticamente.',
  'webhooks.eventsTitle': 'O que publicar',
  'webhooks.allEvents': 'Tudo',
  'webhooks.someEvents': 'Só os eventos selecionados',
  'webhooks.event.taskCreated': 'Tarefa criada',
  'webhooks.event.taskCompleted': 'Tarefa concluída',
  'webhooks.event.taskDeleted': 'Tarefa excluída',
  'webhooks.event.meetingScheduled': 'Reunião marcada',
  'webhooks.event.memberJoined': 'Alguém entrou',
  'webhooks.event.projectCompleted': 'Projeto encerrado',
  'webhooks.flavour.discord': 'Discord',
  'webhooks.flavour.slack': 'Slack',
  'webhooks.flavour.generic': 'JSON',
  'webhooks.enabled': 'Entregar neste endereço',
  'webhooks.secretNow': 'Copie esta chave de assinatura agora — ela aparece uma única vez.',
  'webhooks.secretLabel': 'Chave de assinatura',
  'webhooks.secretExplain':
    'Toda entrega leva uma assinatura feita com ela, para o seu endereço distinguir nossas requisições das de qualquer outra pessoa. Discord e Slack ignoram; o seu próprio servidor não deveria.',
  'webhooks.copy': 'Copiar',
  'webhooks.copied': 'Copiado',
  'webhooks.copyFailed': 'Não foi possível copiar. Selecione a chave e copie manualmente.',
  'webhooks.dismissSecret': 'Pronto',
  'webhooks.test': 'Enviar teste',
  'webhooks.testDelivered': 'O teste chegou.',
  'webhooks.testFailed': 'Não foi possível enviar o teste.',
  'webhooks.testFailedStatus': 'O endereço respondeu {status}.',
  'webhooks.lastOk': 'Entregue ({status})',
  'webhooks.failures': '{count} falhas seguidas',
  'webhooks.remove': 'Remover',
  'webhooks.removeConfirm': 'Remover',
  'webhooks.saveFailed': 'Não foi possível salvar esse webhook.',
  'webhooks.deleteFailed': 'Não foi possível remover esse webhook.',
  'webhooks.adminOnly': 'Só administradores veem isto',
  'webhooks.adminOnlyBody':
    'A URL de um webhook é uma senha para onde ela aponta, então fica restrita ao dono e aos administradores do projeto.',
  'settings.currentPassword': 'Senha atual',
  'settings.changePassword': 'Alterar senha',
  'settings.signsOutEverywhere': 'Encerra a sessão em todos os dispositivos, inclusive este.',
  'settings.profileSaved': 'Perfil salvo.',
  'settings.avatarRemoved': 'Foto removida.',
  'settings.avatarUpdated': 'Foto atualizada.',
  'settings.uploadFailed': 'Não foi possível enviar a imagem.',

  'task.editTitle': 'Editar tarefa',
  'task.newTitle': 'Nova tarefa',
  'task.composerSubtitle': 'O tipo é derivado do agendamento e dos responsáveis.',
  'task.saveChanges': 'Salvar alterações',
  'task.create': 'Criar tarefa',
  'task.titleLabel': 'Título',
  'task.titlePlaceholder': 'Entregar o fluxo de integração',
  'task.descriptionPlaceholder': 'Contexto, links, critérios de aceite…',
  'task.starts': 'Começa',
  'task.deadline': 'Prazo',
  'task.windowInvalid': 'O prazo deve vir depois do início.',
  'task.dateInvalid': 'Esta data não pode ser usada — confira o ano.',
  'task.dateOutOfRange': 'Escolha uma data dentro de {years} anos a partir de hoje.',
  'task.colour': 'Cor',
  'task.priority': 'Prioridade',
  'task.addStep': 'Adicione uma etapa e pressione Enter',
  'task.addChecklistItem': 'Adicionar item à lista',
  'task.imageAttachment': 'Imagem anexada',
  'task.removeAttachment': 'Remover anexo',
  'task.attachImage': 'Anexar uma imagem (máx. 5 MB)',
  'task.imageAttached': 'Imagem anexada.',

  'file.attach': 'Anexar um PDF ou documento do Word',
  'file.attached': 'Documento anexado.',
  'file.remove': 'Remover documento',
  'file.hint': 'PDF, DOCX ou DOC, até 10 MB. Enviado como está — nada é recodificado.',
  'file.uploadFailed': 'Não foi possível enviar este documento.',
  'task.documentAttachment': 'Documento anexado',
  'task.hasDocument': 'Tem um documento anexado',
  'meetings.documentAttachment': 'Documento anexado',

  'task.editTask': 'Editar tarefa',
  'task.waiting': 'Aguardando',
  'task.checklist': 'Lista de etapas',
  'task.addAll': 'Adicionar todas',
  'task.dismiss': 'Descartar',
  'task.markPending': 'Marcar como pendente',
  'task.markDone': 'Marcar como concluída',
  'task.removeStep': 'Remover “{step}”',
  'task.addStepShort': 'Adicionar uma etapa',
  'task.addStepAction': 'Adicionar etapa',
  'task.notes': 'Notas',
  'task.deleteNote': 'Excluir nota',
  'task.pinNote': 'Fixar uma nota nesta tarefa',
  'task.addNote': 'Adicionar nota',

  'ai.failed': 'O assistente não conseguiu terminar',
  'ai.stillWriting': 'Ainda escrevendo…',
  'ai.unavailable': 'O assistente está indisponível.',
  'ai.notConfigured': 'Assistente de IA não configurado',
  'ai.notConfiguredBody':
    'Defina GEMINI_API_KEY na API para liberar a análise de fluxo e as sugestões de subtarefas.',
  'ai.taskIdeas': 'Ideias de tarefas',
  'ai.taskIdeasBody':
    'O Gemini lê a descrição e o quadro do projeto e propõe o que pode vir a seguir.',
  'ai.decideFirstHint':
    'Aceite ou dispense o que já foi proposto antes de pedir mais.',
  'ai.suggestTasks': 'Sugerir tarefas',
  'ai.noIdeas': 'Nenhuma sugestão ainda',
  'ai.noIdeasBody': 'Peça algumas ideias e aceite as que fizerem sentido.',
  'ai.accept': 'Aceitar',
  'ai.decline': 'Recusar',
  'ai.taskAdded': '"{title}" adicionada ao quadro.',
  'ai.addFailed': 'Não foi possível adicionar essa tarefa.',
  'ai.workflowReview': 'Análise do fluxo',
  'ai.workflowReviewBody':
    'O Gemini lê o trabalho concluído e em aberto do projeto e aponta os gargalos.',
  'ai.reanalyse': 'Analisar de novo',
  'ai.analyse': 'Analisar projeto',
  'ai.noAnalysis': 'Nenhuma análise ainda',
  'ai.noAnalysisBody': 'Execute uma análise quando houver algumas tarefas concluídas.',
  'ai.reading': 'Lendo o quadro…',
  'ai.readingBody': 'Resumindo conclusões, prazos e equilíbrio das atribuições.',
  'ai.reviewReady': 'Análise do fluxo pronta.',
  'ai.generated': 'gerado',

  'themes.heading': 'Galeria de temas',
  'themes.searchPlaceholder': 'Buscar temas — tente “escuro”, “retrô”, “tóxico”…',
  'themes.searchLabel': 'Buscar temas',
  'themes.clearSearch': 'Limpar a busca',
  'themes.noMatch': 'Nenhum tema com esse nome',
  'themes.noMatchBody': 'Tente um clima — “escuro”, “vibrante”, “papel”, “retrô”.',
  'themes.pages': 'Páginas da galeria de temas',
  'themes.previousPage': 'Página anterior',
  'themes.nextPage': 'Próxima página',

  'editor.bold': 'Negrito',
  'editor.italic': 'Itálico',
  'editor.underline': 'Sublinhado',
  'editor.strikethrough': 'Tachado',
  'editor.heading1': 'Título 1',
  'editor.heading2': 'Título 2',
  'editor.heading3': 'Título 3',
  'editor.bodyText': 'Texto normal',
  'editor.quote': 'Citação',
  'editor.codeBlock': 'Bloco de código',
  'editor.bulletedList': 'Lista com marcadores',
  'editor.numberedList': 'Lista numerada',
  'editor.alignLeft': 'Alinhar à esquerda',
  'editor.alignCentre': 'Centralizar',
  'editor.alignRight': 'Alinhar à direita',
  'editor.insertLink': 'Inserir um link',
  'editor.removeLink': 'Remover o link',
  'editor.insertImage': 'Inserir uma imagem',
  'editor.insertVideo': 'Inserir um vídeo por URL',
  'editor.clearFormatting': 'Limpar formatação',
  'editor.insert': 'Inserir',
  'editor.documentBody': 'Corpo do documento',
  'editor.needsAbsoluteUrl': 'Links e mídias precisam de um endereço http:// ou https:// completo.',
  'editor.uploadFailed': 'Não foi possível enviar essa imagem.',
  'upload.storageBlocked':
    'O armazenamento de imagens recusou este envio a partir deste site. Tente novamente — se continuar, falta este endereço nas configurações do armazenamento.',
  'upload.storageFailed': 'O armazenamento de imagens não aceitou o arquivo. Tente novamente em instantes.',

  'doc.untitled': 'Documento sem título',
  'doc.created': 'Documento criado.',
  'doc.saved': 'Documento salvo.',
  'doc.discardChanges': 'Descartar alterações não salvas?',
  'doc.emptyPage': 'Página vazia',
  'doc.yourBoard': 'Seu quadro de texto',
  'doc.projectBoard': 'Quadro de texto do projeto',
  'doc.storage': '{used} de {limit}',
  'doc.storageLabel': 'Armazenamento do quadro',
  'doc.storageHintProject':
    'Cada página deste quadro conta para um único limite de {limit}, compartilhado por todo mundo no projeto e definido pelo plano de quem é dono do projeto.',
  'doc.storageHintPersonal':
    'Cada página desta mesa conta para um único limite de {limit}, definido pelo seu plano.',
  'doc.storageFull': 'Este quadro está cheio. Apague uma página para abrir espaço, ou melhore o plano.',
  'doc.projectDocument': 'Documento do projeto',
  'doc.belongsToProject': 'Pertence ao projeto inteiro',
  'doc.pinnedToTask': 'Fixado nesta tarefa',
  'doc.pinnedToMeeting': 'Ata desta reunião',
  'doc.meetings': 'Reuniões',
  'doc.noOpenTasks': 'Todas as tarefas aqui estão concluídas',
  'doc.noOpenMeetings': 'Nenhuma reunião em aberto',
  'doc.download': 'Baixar',
  'doc.format.docx': 'Word',
  'doc.format.docx.hint': 'Continua editável, com as imagens',
  'doc.formatUnavailable': 'Indisponível para esta página',
  'doc.originalOnly': 'Um arquivo enviado é devolvido exatamente como chegou.',
  'doc.designOnly': 'Um design é baixado objeto a objeto, na própria página.',
  'doc.downloadImages': 'Imagens desta página',
  'doc.downloadImagesHint': '{count} para salvar separadamente',
  'doc.imageDownloadFailed': 'Não foi possível salvar essa imagem.',
  'doc.imageDownloaded': '{name} salva.',
  'doc.imageUntitled': 'Imagem {index}',
  'doc.downloadAllImages': 'Salvar todas as imagens',
  'doc.downloadAllImagesHint': 'Um .zip com todas as {count}',
  'doc.imagesDownloaded': 'Imagens salvas.',
  'doc.imagesDownloadedCount': '{count} imagens salvas.',
  'doc.imagesDownloadFailed': 'Não foi possível salvar as imagens.',
  'doc.downloadAs': 'Baixar como',
  'doc.downloadFailed': 'Não foi possível baixar este documento.',
  'doc.format.pdf': 'PDF',
  'doc.format.pdf.hint': 'Diagramado para imprimir e compartilhar, com as imagens',
  'doc.format.txt': 'Texto simples',
  'doc.format.txt.hint': 'Só as palavras, sem formatação',
  'doc.format.html': 'Página web',
  'doc.format.html.hint': 'Abre no navegador e mantém todos os links',
  'doc.originalFile': 'Arquivo original',

  // ---- Importando um documento existente ---------------------------------
  'doc.import': 'Importar',
  'doc.importing': 'Enviando…',
  'doc.importHint': 'Traga um PDF, um documento do Word (.docx) ou um arquivo de texto',
  'doc.imported': 'Documento importado.',
  'doc.importFailed': 'Não foi possível importar este documento.',
  'doc.keptAsUploaded': 'Mantido exatamente como enviado',
  'doc.keptAsUploadedHint':
    'Um PDF ou documento do Word importado é mantido como foi enviado e não é editado aqui. Baixe o arquivo, ou escreva uma página nova ao lado dele.',
  'doc.asUploaded': 'como enviado',
  'doc.uploadedFile': 'Arquivo enviado',
  'doc.openOriginal': 'Abrir original',
  'doc.loadingPreview': 'Carregando o documento…',
  'doc.noPreview': 'Sem pré-visualização para arquivos {format}',
  'doc.noPreviewHint':
    'Abra no programa que lê esse formato no seu computador, ou baixe pelo menu acima.',
  'doc.previewFailed': 'Não foi possível carregar o arquivo',
  'doc.previewFailedHint': 'Ele continua no armazenamento — tente baixar o original.',
  'doc.confirm': 'Confirmar',
  'doc.pages': 'Páginas',
  'doc.project': 'Projeto',
  'doc.nothingOpen': 'Nada aberto',
  'doc.nothingOpenPersonal':
    'Um lugar para escrever mais do que cabe em um recado — um plano, um rascunho, uma lista que cresceu.',
  'doc.nothingOpenProject':
    'Escreva o briefing, a especificação ou a ata — para o projeto, ou fixado em uma tarefa.',
  'doc.documentTitle': 'Título do documento',
  'doc.tooLong': 'Esta página está longa demais para salvar. Reduza o texto ou divida em duas páginas.',
  'doc.someone': 'Alguém',
  'doc.linkedDocuments': 'Páginas vinculadas',
  'doc.openOnTextBoard': 'Abrir no quadro de textos',
  'doc.readOnly': 'Somente leitura',
  'doc.readOnlyHint': 'Só {author} pode editar esta página. Peça para compartilhar com você.',
  'doc.whoCanEdit': 'Quem pode editar',
  'doc.whoCanEditHint':
    'Você escreveu esta página, então só você pode alterá-la. Quem você marcar aqui também poderá — todo mundo do projeto já consegue ler.',
  'doc.shareEditing': 'Compartilhar edição',
  'doc.editorsSaved': 'Permissões de edição atualizadas.',
  'doc.editorsFailed': 'Não foi possível atualizar quem pode editar esta página.',
  'doc.noOneElse': 'Ninguém mais por enquanto',
  'doc.editorCount': '{count} podem editar',
  'doc.authorOnly': 'Só você pode editar',

  'doc.dropHere': 'Solte para adicionar a este quadro',
  'doc.dropHint': 'PDF, Word, texto simples, imagens ou um .zip',
  'doc.dropRejectedType': 'Este quadro não aceita esse arquivo. PDF, .docx, .txt, PNG, JPEG ou .zip.',
  'doc.dropRejectedEmpty': 'Esse arquivo está vazio.',
  'doc.dropRejectedSize': 'Grande demais — o limite é {limit}.',
  'doc.dropOneAtATime': 'Um arquivo por vez. Adicionando {name}.',

  'doc.picture': 'Imagem',
  'doc.optimised': 'Otimizada de {from} para {to}',
  'doc.optimisedBadge': 'Otimizada',
  'doc.optimisedHint':
    'Recodificada no seu navegador antes do envio: ocupa menos, abre mais rápido e os metadados de localização se foram.',
  'doc.imageFailed': 'Não foi possível carregar essa imagem',

  'doc.archive': 'Arquivo compactado',
  'doc.archiveContents': 'Dentro deste arquivo',
  'doc.archiveCount': '{files} arquivos · {size} descompactados',
  'doc.archiveLoading': 'Lendo o arquivo…',
  'doc.archiveEmpty': 'Este arquivo está vazio.',
  'doc.archiveTruncated': 'Mostrando os primeiros {shown} de {total} itens.',
  'doc.archiveFailed': 'Não foi possível ler esse arquivo',
  'doc.archiveHint': 'A lista é lida sem descompactar nada. Baixe o arquivo para obter os arquivos.',
  'doc.archiveSearch': 'Filtrar por nome',
  'doc.archiveNoMatches': 'Nada aqui com esse nome',
  'doc.archiveFolder': 'Pasta',
  'doc.archiveHeavy': 'Descompacta para {size} — {ratio}x o tamanho baixado.',
  'doc.archiveHeavyHint': 'Isso é normal em pastas de texto ou logs, mas confira se há espaço antes de descompactar.',

  'figma.name': 'Figma',
  'figma.connectedTitle': 'O Figma está conectado',
  'figma.securityLabel': 'Sobre este token',
  'figma.howTo': 'Como configurar',
  'figma.open': 'Abrir no Figma',
  'figma.connect': 'Conectar o Figma',
  'figma.connectTitle': 'Conectar este projeto ao Figma',
  'figma.connectBody':
    'Cole o link de um arquivo do Figma e um token de acesso pessoal que possa lê-lo. O design passa a aparecer ao lado do nome do projeto e pode virar uma página viva na aba Documentos.',
  'figma.urlLabel': 'Link do arquivo',
  'figma.urlPlaceholder': 'figma.com/design/<chave>/<nome>',
  'figma.tokenLabel': 'Token de acesso pessoal',
  'figma.tokenPlaceholder': 'figd_…',
  'figma.tokenHint':
    'Figma → Settings → Security → Personal access tokens. Acesso de leitura aos arquivos já basta.',
  'figma.sharedCredential':
    'Todo mundo neste projeto lê o design por meio deste token. Ele é criptografado em repouso e nunca é mostrado de novo.',
  'figma.connectAction': 'Conectar',
  'figma.connected': 'Arquivo do Figma conectado.',
  'figma.connectFailed': 'Não foi possível conectar esse arquivo do Figma.',
  'figma.disconnect': 'Desconectar',
  'figma.disconnected': 'Figma desconectado.',
  'figma.disconnectFailed': 'Não foi possível desconectar o Figma.',
  'figma.disconnectHint':
    'O token é apagado. As páginas de design já no quadro mantêm a última sincronização e param de atualizar.',
  'figma.adminOnly': 'Só o dono ou um admin pode conectar o Figma.',
  'figma.unavailable': 'Indisponível aqui',
  'figma.unavailableHint':
    'Esta instalação não tem chave de criptografia, então não pode guardar uma credencial do Figma.',
  'figma.connectedFile': 'Conectado a {file}',
  'figma.connectedBy': 'Usando o token de {name}',

  'figma.addPage': 'Figma',
  'figma.addPageHint': 'Adicionar o arquivo de design conectado como página',
  'figma.addPageNeedsConnection': 'Conecte este projeto ao Figma antes, na aba Conexões.',
  'figma.pageAdded': 'Design adicionado ao quadro.',
  'figma.pageFailed': 'Não foi possível adicionar esse design.',
  'figma.design': 'Arquivo de design',
  'figma.sync': 'Sincronizar',
  'figma.syncing': 'Consultando o Figma…',
  'figma.synced': 'Design atualizado a partir do Figma.',
  'figma.alreadyCurrent': 'Já está em dia.',
  'figma.syncFailed': 'Não foi possível falar com o Figma.',
  'figma.syncedAt': 'Sincronizado {when}',
  'figma.neverSynced': 'Nunca sincronizado',
  'figma.pagesCount': '{count} páginas',
  'figma.objectsCount': '{count} objetos',
  'figma.noFrames': 'Nada nesta página ainda',
  'figma.noPages': 'Este arquivo não tem páginas que o quadro consiga ler.',
  'figma.truncated': 'O arquivo é maior que isto — só os primeiros objetos aparecem.',
  'figma.previewFailed': 'O Figma não conseguiu gerar a prévia.',
  'figma.selectObject': 'Escolha um objeto para ver a prévia',
  'figma.download': 'Baixar',
  'figma.downloadAs': 'Baixar este objeto como',
  'figma.downloaded': '{name} baixado.',
  'figma.downloadFailed': 'Não foi possível exportar esse objeto.',
  'figma.format.png': 'PNG',
  'figma.format.png.hint': 'Raster, com fundo transparente',
  'figma.format.jpg': 'JPEG',
  'figma.format.jpg.hint': 'Raster, menor, sem transparência',
  'figma.format.svg': 'SVG',
  'figma.format.svg.hint': 'Vetor, escala para qualquer tamanho',
  'figma.format.pdf': 'PDF',
  'figma.format.pdf.hint': 'Vetor, para imprimir e compartilhar',

  'figma.brief': 'Resumo técnico',
  'figma.briefHint': 'Ler os nomes de páginas e frames e rascunhar um resumo',
  'figma.briefTitle': 'Resumo técnico',
  'figma.briefLoading': 'Lendo a estrutura…',
  'figma.briefFailed': 'O assistente não conseguiu ler esse arquivo.',
  'figma.briefFlows': 'Fluxos',
  'figma.briefTasks': 'Tarefas sugeridas',
  'figma.briefGaps': 'Estados que os nomes não mencionam',
  'figma.briefDisclaimer':
    'Escrito só a partir dos nomes de páginas e frames — o assistente nunca vê a arte.',
  'figma.briefSave': 'Salvar como página',
  'figma.briefSaved': 'Resumo salvo como página.',

  'invite.individually': 'Pessoas',
  'invite.byTeam': 'Equipes',
  'invite.searchPeople': 'Buscar pessoas',
  'invite.noMatches': 'Ninguém aqui com esse nome',
  'invite.noTeams': 'Nenhuma equipe por aqui ainda',
  'invite.selectedCount': '{count} selecionados',
  'invite.page': '{from}–{to} de {total}',
  'invite.previousPage': 'Pessoas anteriores',
  'invite.nextPage': 'Mais pessoas',

  'board.inkCleared': 'Traços apagados. Os recados não foram tocados.',
  'board.adminsOnlyClear': 'Somente administradores do projeto podem limpar a tela.',
  'board.arrange': 'Organizar',
  'board.arrangeHint': 'Arraste recados, use o laço para selecionar',
  'board.connect': 'Conectar',
  'board.connectHint': 'Desenhe setas entre os recados',
  'board.pen': 'Caneta',
  'board.penHint': 'Desenhe na tela',
  'board.eraser': 'Borracha',
  'board.eraserHint': 'Apague os traços',
  'board.projectWhiteboard': 'Lousa do projeto',
  'board.addPostIt': 'Adicionar um recado',
  'board.addPostItTitle': 'Adicionar um recado para a equipe',
  'board.pinImage': 'Fixar uma imagem',
  'board.image': 'Imagem',
  'board.pickSeveral': 'Selecionar vários',
  'board.pickSeveralTitle': 'Clique em vários recados seguidos para selecioná-los juntos',
  'board.liveWithTeam': 'Ao vivo com sua equipe',
  'board.offlineLocal': 'Offline — as alterações ficam locais',

  'toast.taskCreated': '"{title}" criada.',
  'toast.taskDone': '“{title}” foi concluída.',
  'toast.projectReady': '“{name}” está pronto.',
  'toast.skinApplied': '{name} aplicado.',
  'toast.boardAddFailed': 'Não foi possível adicionar isso ao mural.',
  'toast.noteSaveFailed': 'Não foi possível salvar a nota.',
  'toast.layoutSaveFailed': 'Não foi possível salvar o layout.',
  'toast.connectFailed': 'Não foi possível conectar essas notas.',
  'toast.strokeSaveFailed': 'Não foi possível salvar esse traço.',
  'toast.clearPageFailed': 'Não foi possível limpar a página.',
  'toast.pageAdded': 'Página adicionada.',
  'toast.pageAddFailed': 'Não foi possível adicionar uma página.',
  'toast.pageRemoved': 'Página removida. Suas notas estão na lixeira.',
  'toast.pageRemoveFailed': 'Não foi possível remover a página.',
  'toast.groupFailed': 'Não foi possível agrupar essas notas.',
  'toast.noteCreateFailed': 'Não foi possível criar a nota.',
  'toast.noteBinned': 'Nota movida para a lixeira.',
  'toast.noteRestored': 'Nota restaurada.',
  'toast.notePurged': 'Nota excluída definitivamente.',
  'toast.authorOnlyRemove': 'Somente o autor pode remover essa nota.',
  'toast.projectCreateFailed': 'Não foi possível criar o projeto.',
  'toast.projectUpdated': 'Projeto atualizado.',
  'toast.projectBinned': 'Projeto movido para a lixeira.',
  'toast.projectRestored': 'Projeto restaurado.',
  'toast.projectPurged': 'Projeto excluído definitivamente.',
  'toast.pinFailed': 'Não foi possível atualizar a fixação.',
  'toast.inviteSent': 'Convite enviado.',
  'toast.inviteFailed': 'Não foi possível enviar o convite.',
  'toast.joinedProject': 'Você entrou no projeto.',
  'toast.inviteDeclined': 'Convite recusado.',
  'toast.rosterUpdated': 'Equipe atualizada.',
  'toast.rosterRemoveFailed': 'Não foi possível remover esse integrante.',
  'toast.rosterRoleFailed': 'Não foi possível alterar este papel.',
  'toast.taskCreateFailed': 'Não foi possível criar a tarefa.',
  'toast.taskUpdateFailed': 'Não foi possível atualizar a tarefa.',
  'toast.statusFailed': 'Não foi possível alterar o status.',
  'toast.taskBinned': 'Tarefa movida para a lixeira.',
  'toast.taskPurged': 'Tarefa excluída definitivamente.',
  'toast.taskRestored': '“{title}” restaurada.',
  'toast.restoreFailed': 'Não foi possível restaurar.',
  'toast.deleteFailed': 'Não foi possível excluir.',

  'notes.title': 'Notas',
  'notes.yourBoard': 'Seu mural de recados',
  'notes.opening': 'Abrindo seu mural',
  'notes.emptyDesk': 'Uma mesa vazia',
  'notes.emptyDeskBody':
    'Adicione uma nota ou fixe uma imagem, depois conecte e organize como quiser.',
  'notes.addFirst': 'Adicionar a primeira nota',
  'notes.group': 'Agrupar',
  'notes.groupHint': 'Une estas em um bloco — arrastar qualquer uma move todas',
  'notes.groupNeedsTwo': 'Selecione ao menos duas notas para agrupá-las',
  'notes.ungroup': 'Desagrupar',
  'notes.arrowLands': 'Agora clique onde a seta termina.',
  'notes.arrowStarts': 'Clique na nota de onde a seta começa.',
  'notes.arrowRemove': 'Clique em uma seta existente para removê-la.',
  'notes.lassoHint': 'Arraste pelo mural para selecionar várias notas de uma vez',
  'notes.clearSelection': 'Limpar seleção',
  'notes.leaveConnect': 'Sair do modo de conexão',
  'notes.eraseInk': 'Apagar traços',
  'notes.clearAll': 'Limpar tudo',
  'notes.linkHere': 'Ligar aqui',
  'notes.fromHere': 'A partir daqui',
  'notes.partOfGroup': 'Faz parte de um grupo — arrastar uma move todas',
  'notes.changeColour': 'Mudar a cor',
  'notes.deleteNote': 'Excluir nota',
  'notes.resizeNote': 'Arraste para redimensionar esta nota',
  'notes.writeSomething': 'Escreva algo…',
  'notes.addPage': 'Adicionar página',
  'notes.savePageName': 'Salvar nome da página',
  'notes.doubleClickRename': 'Clique duas vezes para renomear',
  'notes.renamePage': 'Renomear esta página',

  'views.today': 'Hoje',
  'views.nothingDue': 'Nada com prazo neste dia.',
  'views.noDateYet': 'Sem data ainda',
  'views.previousMonth': 'Mês anterior',
  'views.nextMonth': 'Próximo mês',
  'views.emptyMonth': 'Um mês vazio',
  'views.noMatch': 'Nenhuma tarefa corresponde a estes filtros.',
  'views.sprint': 'Sprint',
  'sprint.done': '{done}/{total} concluídas',
  'sprint.late': '{count} atrasadas',
  'views.nothingInSprint': 'Nada neste sprint',
  'views.nothingToList': 'Nada para listar',
  'views.moveToBin': 'Mover para a lixeira',
  'views.clear': 'Limpar',
  'views.pinned': 'Fixadas',
  'views.withNotes': 'Notas',
  'views.withNotesTitle': 'Apenas tarefas em que alguém anexou uma nota',
  'views.searchTasks': 'Buscar tarefas',

  // ---- Visualizador de imagens -------------------------------------------
  'image.expand': 'Abrir a imagem em tamanho real',
  'image.collapse': 'Voltar',
  'image.zoomIn': 'Aproximar',
  'image.zoomOut': 'Afastar',
  'image.resetZoom': 'Redefinir o zoom',
  'views.markNotDone': 'Marcar como não concluída',
  'views.assigneesOnly': 'Somente os responsáveis por esta tarefa podem concluí-la.',
  'views.unpinTask': 'Desafixar tarefa',
  'views.pinTask': 'Fixar tarefa',
  'notes.unpinNote': 'Desafixar nota',
  'notes.pinNote': 'Fixar nota',

  'board.showAllCount': 'Ver todas · mais {count}',
  'board.showFewer': 'Ver menos',
  'board.nothingHere': 'Nada aqui',
  'board.dropHere': 'Solte uma tarefa nesta coluna.',
  'doc.createFailed': 'Não foi possível criar o documento.',
  'doc.saveFailed': 'Não foi possível salvar o documento.',
  'doc.deleted': 'Documento excluído.',
  'doc.new': 'Novo documento',
  'doc.noPages': 'Nenhuma página ainda. A primeira está a um clique.',
  'doc.tasks': 'Tarefas',
  'doc.loadTheirVersion': 'Carregar a versão deles',
  'doc.keepMine': 'Manter a minha',
  'project.opening': 'Abrindo projeto',
  'themes.default': 'Padrão',
  'themes.browse': 'Explorar temas',
  'notes.addFirstNote': 'Adicionar a primeira nota',
  'app.openingStudio': 'Abrindo o estúdio',
  'app.checkingSession': 'Verificando a sessão',
  'themes.defaultSkin': 'Padrão',
  'views.layout': 'Layout',
  'notes.addPageAction': 'Adicionar página',
  'nav.dropToPin': 'Solte para fixar',
  'ai.nextSteps': 'Próximos passos',
  'task.hasImage': 'Tem uma imagem anexada',
  'session.expired': 'Sua sessão expirou. Entre novamente.',
  'session.refreshFailed': 'Não foi possível atualizar os dados.',
  'task.signOff': 'Conclusão',
  'task.assignees': 'Responsáveis',
  'task.multiTaskNote': 'Mais de um responsável — isto vira uma MultiTask.',
  'nav.dragAnywhere': 'Arraste para onde quiser',
  'nav.sendBackToMenu': 'Devolver ao menu',
  'nav.pinnedShortcuts': 'Atalhos fixados',
  'nav.hoverHintBar': 'Passe o ponteiro aqui para ver a barra',
  'board.clearInk': 'Apagar traços',
  'board.someone': 'Alguém',
  'board.sharedWall': 'Um mural compartilhado',
  'notes.privateDrag':
    'Só seu. Arraste as coisas para onde quiser — o layout é salvo conforme você mexe.',
  'notes.privateOutgrew': 'Só seu. Para as coisas que não coubereram em um recado.',
  'nav.studioWorkspace': 'Espaço de trabalho',
  'nav.hoverHintMenu': 'Passe o ponteiro aqui para ver o menu',
  'nav.pinnedHint': 'Fixado na tela — use a seta de retorno para soltar a cópia',
  'themes.inUse': 'Em uso',
  'themes.paidBadge': 'Pago',
  'themes.lockedHint': 'Faz parte dos planos pagos',
  'themes.lockedToast': 'Esse tema faz parte dos planos pagos — faça upgrade em Plano para usá-lo.',
  'themes.upgradeToWear': 'Faça upgrade para usar {name}',
  'themes.preview': 'Prévia',
  'themes.currentlyApplied': 'Aplicado no momento',
  'themes.bothModes': 'Claro e escuro continuam sendo sua escolha — todo tema traz os dois.',
  'themes.cursor': 'Cursor do tema',
  'dash.title': 'Painel',
  'dash.subtitle': 'Tudo o que é seu e tudo o que foi atribuído a você, em um só lugar.',
  'dash.createProject': 'Criar um projeto',
  'dash.openTaskMenu': 'Abrir menu de tarefas',
  'dash.openTaskMenuFor': 'Abrir o menu de tarefas — {label}',
  'dash.openOrganizations': 'Todas as organizações',
  'dash.openTasks': 'Tarefas abertas',
  'dash.completed': 'Concluídas',
  'dash.overdue': 'Atrasadas',
  'dash.projects': 'Projetos',
  'dash.pinnedProjects': 'Projetos fixados',
  'dash.noProjects': 'Nenhum projeto ainda',
  'dash.noProjectsBody': 'Crie seu primeiro projeto e convide pessoas para a equipe.',
  'dash.noArchived': 'Você ainda não tem nenhum projeto arquivado',
  'dash.noArchivedBody':
    'Arquivar um projeto tira ele da sua lista sem apagar nada. O que você arquivar aparece aqui.',
  'dash.archivedOnly': 'Mostrando só os projetos arquivados',
  'dash.upNext': 'A seguir para você',
  'dash.nothingAssigned': 'Nada atribuído a você',
  'dash.nothingAssignedBody':
    'Quando o dono de um projeto atribuir trabalho a você, ele aparece aqui.',

  'agenda.title': 'Menu de tarefas',
  'agenda.heading': 'Sua agenda',
  'agenda.showingCompleted':
    'Mostrando trabalho concluído. Ele vai para a lixeira uma semana depois de pronto.',
  'agenda.everythingSlotted': 'Tudo tem um horário no calendário.',
  'agenda.building': 'Montando sua agenda',
  'agenda.unscheduled': 'Sem agendamento',
  'agenda.unscheduledBody': 'Sem horário de início e sem prazo ainda.',
  'agenda.newTask': 'Nova tarefa',
  'agenda.newPersonalTask': 'Nova tarefa pessoal',
  'agenda.personal': 'Pessoal',
  'agenda.personalTitle': 'Uma tarefa pessoal — sem projeto por trás',
  'agenda.nothingPersonal': 'Nenhuma tarefa pessoal ainda',
  'agenda.personalBody':
    'Tarefas pessoais não têm projeto por trás. Elas ficam aqui, no seu menu de tarefas, e em nenhum outro lugar.',
  'agenda.personalComposerBody':
    'Esta é só sua: sem projeto, sem equipe, ninguém mais consegue ver.',

  // ---- Agenda pessoal de reuniões ----------------------------------------
  'agenda.meetingsEyebrow': 'Sua semana',
  'agenda.meetingsTitle': 'Reuniões',
  'agenda.meetingsBody':
    'Tudo em que você é esperado, em todos os projetos de que participa.',
  'agenda.searchMeetings': 'Buscar reuniões',
  'agenda.allProjects': 'Todos os projetos',
  'agenda.openProject': 'Abrir {name}',
  'agenda.noMeetings': 'Nada na agenda',
  'agenda.noMeetingsBody':
    'Quando alguém marcar uma reunião em um dos seus projetos, ela aparece aqui.',
  'agenda.noMeetingMatch': 'Nada corresponde a isso',
  'agenda.noMeetingMatchBody': 'Tente outro nome, sala ou projeto.',

  // ---- Organizações ------------------------------------------------------
  'org.eyebrow': 'Empresas e o trabalho delas',
  'org.title': 'Organizações',
  'org.subtitle': 'Uma empresa, as pessoas dela e os projetos que elas tocam.',
  'org.new': 'Nova organização',
  'org.newTitle': 'Nova organização',
  'org.newSubtitle':
    'Uma empresa: sua equipe, seus projetos, sua agenda e seus números. Dá para configurar tudo agora ou acrescentar depois.',
  'org.editTitle': 'Configurações da organização',
  'org.editSubtitle':
    'Renomeie, mude a cor ou exclua. Pessoas e projetos são gerenciados na página da própria organização.',
  'org.create': 'Criar organização',
  'org.name': 'Nome',
  'org.namePlaceholder': 'Acme Ltda.',
  'org.descriptionPlaceholder': 'O que pertence a esta pasta?',
  'org.emptyTitle': 'Nenhuma organização ainda',
  'org.emptyBody':
    'Uma organização reúne as pessoas de uma empresa e os projetos que elas tocam, com uma agenda comum e um lugar só para ver como tudo está indo.',
  'org.noDescription': 'Sem descrição ainda.',
  'org.archived': 'Arquivado',
  'org.projectCount': '{count} projeto(s)',
  'org.ownedByYou': 'Sua',
  'org.ownedBy': 'De {name}',
  'org.fileProject': 'Guardar um projeto aqui',
  'org.chooseProject': 'Escolha um projeto…',
  'org.nothingToFile': 'Todos os projetos que você criou já estão em alguma pasta.',
  'org.unfile': 'Tirar {name} desta organização',
  'org.filedUnder': 'Guardado em {name}',
  'org.dangerZone': 'Excluir esta organização',
  'org.deleteExplain':
    'A organização, a lista de pessoas e as reuniões próprias dela são excluídas. Todos os projetos saem da pasta e continuam exatamente como estão — ninguém perde acesso a nenhum trabalho.',
  'org.delete': 'Excluir organização',
  'org.created': '"{name}" está pronta.',
  'org.createFailed': 'Não foi possível criar esta organização.',
  'org.updated': 'Organização atualizada.',
  'org.deleted': 'Organização excluída. Os projetos dela não foram alterados.',
  'org.projectFiled': 'Projeto guardado.',
  'org.projectUnfiled': 'Projeto retirado da pasta.',

  'org.opening': 'Abrindo organização',
  'org.open': 'Abrir {name}',
  'org.tabProjects': 'Projetos',
  'org.tabMetrics': 'Métricas',
  'org.tabStaff': 'Pessoas',
  'org.tabMeetings': 'Reuniões',
  'org.crunching': 'Somando tudo',
  'org.metricsStaffOnly':
    'Só quem faz parte desta organização vê os números dela. Você chegou aqui por um projeto que está dentro dela.',
  'org.staffStaffOnly':
    'A lista de pessoas é para quem faz parte desta organização. Você chegou aqui por um projeto que está dentro dela.',
  'org.meetingsStaffOnly':
    'Esta agenda é para quem faz parte desta organização. As reuniões do seu projeto estão no quadro dele e na sua agenda.',

  'org.searchProjects': 'Buscar projetos',
  'org.laneAtRisk': 'Atrasados',
  'org.laneAtRiskHint': 'Têm trabalho com prazo vencido',
  'org.laneActive': 'Em dia',
  'org.laneActiveHint': 'Em andamento, nada atrasado',
  'org.laneArchived': 'Arquivados',
  'org.laneArchivedHint': 'Concluídos ou pausados',
  'org.laneEmpty': 'Nada aqui.',
  'org.boardEmpty': 'Nenhum projeto guardado aqui ainda',
  'org.boardEmptyBody': 'Projetos guardados nesta organização aparecem neste quadro.',
  'org.boardEmptyAdmin': 'Guarde aqui um projeto que você criou para começar.',
  'org.overdueCount': '{count} atrasada(s)',
  'org.openCount': '{count} aberta(s)',
  'org.noTasks': 'Sem tarefas ainda',
  'org.completion': 'Concluído',
  'org.noAccess': 'Você não participa deste projeto',
  'org.noAccessHint':
    'Você vê que este projeto existe porque faz parte desta organização. Para abri-lo é preciso estar na equipe dele.',

  'org.tileProjects': 'Projetos ativos',
  'org.tileArchivedHint': '+{count} arquivado(s)',
  'org.tileMembers': 'Pessoas',
  'org.tileMeetings': 'Reuniões futuras',
  'org.throughput': 'Entregas',
  'org.tasksDone': '{completed} de {total} tarefas concluídas',
  'org.dueThisWeek': '{count} com prazo nos próximos sete dias',
  'org.trendPoint': '{count} concluída(s) em {date}',
  'org.mostAtRisk': 'Precisa de atenção',
  'org.nothingOverdue': 'Nada está atrasado',
  'org.busiest': 'Mais entregou',
  'org.completedCount': '{count} concluída(s)',
  'org.byProject': 'Por projeto',
  'org.byProjectHint': 'Os mais atrasados primeiro.',
  'org.nextDeadline': 'Próximo prazo',
  'org.nextDeadlines': 'Próximos prazos da organização',

  'org.staff': 'Pessoas',
  'org.headcount': '{count} na equipe',
  'org.you': 'você',
  'org.role': 'Papel',
  'org.roleOwner': 'Dono',
  'org.roleAdmin': 'Administrador',
  'org.roleAdminHint': 'Pode guardar projetos, convidar pessoas e editar a organização',
  'org.roleMember': 'Membro',
  'org.roleMemberHint': 'Pode ver a organização e tudo o que ela guarda',
  'org.roleGuest': 'Convidado',
  'org.guestNotice':
    'Você vê esta organização porque participa de um dos projetos dela. Você não está na equipe dela.',
  'org.jobTitle': 'Cargo',
  'org.jobTitlePlaceholder': 'Gerente de entregas',
  'org.noJobTitle': 'Sem cargo',
  'org.editJobTitle': 'Editar cargo',
  'org.joined': 'entrou',
  'org.removeMember': 'Remover da organização',
  'org.leave': 'Sair desta organização',
  'org.memberUpdated': 'Atualizado.',
  'org.memberRemoved': 'Removido da organização.',

  'org.invite': 'Convidar',
  'org.invitePeople': 'Convidar pessoas',
  'org.invitePeopleHint':
    'Elas são convidadas assim que a organização for criada. Cada pessoa precisa de uma conta confirmada no Task Studio.',
  'org.inviteEmail': 'E-mail',
  'org.inviteEmailPlaceholder': 'colega@empresa.com',
  'org.inviteDuplicate': 'Já está na lista.',
  'org.addPerson': 'Adicionar',
  'org.removePerson': 'Não convidar {name}',
  'org.inviteSent': 'Convite enviado.',
  'org.inviteFailed': 'Não foi possível enviar este convite.',
  'org.inviteRevoked': 'Convite cancelado.',
  'org.inviteAccepted': 'Você entrou na equipe.',
  'org.inviteDeclined': 'Convite recusado.',
  'org.invitesSkipped': '{count} convite(s) não foram enviados: {names}',
  'org.pendingInvites': 'Convidados, ainda sem resposta',
  'org.noPendingInvites': 'Ninguém aguardando',
  'org.noPendingInvitesBody': 'Convites enviados aparecem aqui até serem respondidos.',
  'org.invitedRelative': 'convidado {when}',
  'org.revokeInvite': 'Cancelar este convite',

  'org.linkProjects': 'Trazer projetos junto',
  'org.linkProjectsHint':
    'Projetos que você criou e ainda não guardou em outra pasta. Dá para guardar mais a qualquer momento.',

  'org.bannerAdd': 'Adicionar capa',
  'org.bannerReplace': 'Trocar capa',
  'org.bannerRemove': 'Remover capa',
  'org.bannerFailed': 'Não foi possível enviar esta imagem.',
  'org.bannerNotAnImage': 'Escolha um arquivo de imagem.',
  'org.bannerPreviewTitle': 'Como vai ficar',
  'org.bannerPreviewSubtitle':
    'A capa é uma faixa larga, então uma imagem alta mantém só a parte do meio. Confira o corte antes de publicar.',
  'org.bannerAsShown': 'Na página',
  'org.bannerWholeImage': 'A imagem inteira',
  'org.bannerCropHint': 'Tudo fora da faixa acima é cortado.',
  'org.bannerPickAnother': 'Escolher outra',
  'org.bannerUse': 'Usar esta imagem',

  'org.newProject': 'Novo projeto',
  'org.tabTeams': 'Times',
  'org.teamsStaffOnly':
    'Os times são para quem faz parte desta organização. Você chegou aqui por um projeto que está dentro dela.',

  'team.title': 'Times',
  'team.new': 'Novo time',
  'team.newTitle': 'Novo time',
  'team.editTitle': 'Editar time',
  'team.composerSubtitle': 'Um nome, uma cor e quem faz parte.',
  'team.create': 'Criar time',
  'team.name': 'Nome',
  'team.namePlaceholder': 'Back end',
  'team.descriptionPlaceholder': 'O que este grupo faz?',
  'team.members': 'Integrantes',
  'team.noStaffYet': 'Convide pessoas para a organização primeiro.',
  'team.noRosterYet': 'Convide pessoas para o projeto primeiro.',
  'team.headcount': '{count} integrante(s)',
  'team.edit': 'Editar este time',
  'team.delete': 'Excluir este time',
  'team.deleteHint': 'Exclui o grupo. Ninguém perde acesso a nada.',
  'team.none': 'Nenhum time ainda',
  'team.noneBody': 'Os times criados aqui aparecem nesta aba.',
  'team.noneBodyAdmin':
    'Agrupe quem trabalha junto para poder escolher todo mundo de uma vez depois.',
  'team.created': '"{name}" está pronto.',
  'team.createFailed': 'Não foi possível criar este time.',
  'team.updated': 'Time atualizado.',
  'team.deleted': 'Time excluído. Ninguém perdeu acesso a nada.',

  'project.tabTeams': 'Times',
  'project.organization': 'Organização',
  'project.organizationNone': 'Sem organização',
  'project.organizationHint':
    'Apenas organizações que você criou ou administra. Dá para guardar depois também.',
  'project.staffFromTeams': 'Começar com uma equipe',

  'bin.title': 'Lixeira',
  'bin.deletedItems': 'Itens excluídos',
  'bin.autoArchived': 'Arquivado automaticamente',
  'bin.restore': 'Restaurar',
  'bin.emptyNote': 'Nota vazia',
  'bin.opening': 'Abrindo a lixeira',
  'bin.deletedWhen': 'excluída {when}',
  'bin.recently': 'há pouco',
  'bin.notesBoardPage': 'Mural de notas · página {page}',

  'bin.projects': 'Projetos',
  'bin.noDeletedProjects': 'Nenhum projeto excluído',
  'bin.deletedProjectsBody':
    'Um projeto excluído espera aqui por 30 dias com tudo ainda dentro dele. Depois disso ele é destruído, e os arquivos também.',
  'bin.expiresOn': 'Destruído em {date}',
  'bin.expiresToday': 'Destruído hoje',
  'bin.expiresInDays': 'Destruído em {days} dia(s)',
  'bin.projectContents': '{tasks} tarefa(s) · {notes} nota(s) · {messages} mensagem(ns)',
  'bin.projectStillFinished': 'Estava concluído',
  'bin.deleteForever': 'Excluir definitivamente',
  'bin.purgeTaskTitle': 'Excluir esta tarefa definitivamente?',
  'bin.purgeNoteTitle': 'Excluir esta nota definitivamente?',
  'bin.emptyBin': 'Esvaziar lixeira',
  'bin.emptyBinTitle': 'Esvaziar a lixeira?',
  'bin.emptyBinBody': 'Isto exclui definitivamente {tasks} tarefa(s), {notes} nota(s) e {projects} projeto(s). Não é possível desfazer.',
  'bin.emptyBinProjects': 'Projetos só podem ser excluídos com a sua senha. Deixe em branco para mantê-los e esvaziar o restante.',
  'bin.emptied': '{count} item(ns) excluído(s) definitivamente.',
  'bin.emptiedWithSkipped': '{count} item(ns) excluído(s) definitivamente. {skipped} pertencem a colegas e foram mantidos.',
  'bin.purgeProjectTitle': 'Excluir "{name}" definitivamente?',
  'bin.purgeProjectBody':
    'Isso destrói o projeto e tudo que ainda está nele — tarefas, páginas, notas, a lousa, a conversa, as reuniões e todos os arquivos enviados. Não dá para desfazer, e não existe cópia em lugar nenhum. Digite sua senha para confirmar.',
  'bin.purgeProjectLabel': 'Sua senha',
  'bin.purgeProjectAction': 'Excluir definitivamente',

  'invites.title': 'Convites',
  'invites.heading': 'Convites de projeto',
  'invites.subtitle':
    'Aceitar adiciona você à equipe e libera o quadro e a conversa do projeto.',
  'invites.accept': 'Aceitar',
  'invites.decline': 'Recusar',
  'invites.projectExplain': 'Ao entrar, você recebe o quadro, o chat e o trabalho.',
  'invites.orgExplain':
    'Ao entrar, você passa a ver a organização. Os projetos dela convidam você separadamente.',
  'invites.checking': 'Verificando convites',
  'invites.none': 'Nenhum convite pendente',
  'invites.noneBody':
    'Quando alguém convidar você para um projeto, ele aparece aqui e como notificação.',

  'rail.soonestFirst': 'Mais próximos primeiro',
  'rail.noProjects': 'Você ainda não participa de nenhum projeto.',
  'rail.yourCompanies': 'Onde você trabalha',
  'rail.noOrganizations': 'Você ainda não participa de nenhuma organização.',
  'rail.dragToPin': 'Arraste para fixar este projeto em qualquer lugar da tela',
  'rail.pinnedHint': 'Fixado na tela — use a seta de retorno para soltar a cópia',
  'rail.pinnedProject': 'Projeto fixado',
  'rail.hoverHint': 'Passe o ponteiro aqui para ver seus projetos',

  'projectDash.crunching': 'Calculando métricas',
  'projectDash.completion': 'Conclusão',
  'projectDash.doneOfTotal': '{done} de {total} tarefas concluídas',
  'projectDash.completedOnDay': '{count} concluída(s) em {date}',
  'projectDash.perMonth': 'Tarefas concluídas por mês',
  'projectDash.lastMonths': 'Últimos {count} meses',
  'projectDash.completedInMonth': '{count} concluída(s) em {month}',
  'projectDash.monthBest': 'Melhor mês',
  'projectDash.monthAverage': 'Média mensal',
  'projectDash.monthTotal': 'Concluídas no ano',
  'projectDash.noMonthlyData': 'Nada concluído no último ano ainda.',
  'projectDash.nextDeadlines': 'Próximos prazos',
  'projectDash.nothingScheduled': 'Nada agendado',
  'projectDash.noDeadlines': 'Nenhum prazo próximo.',

  'notif.markAllRead': 'Marcar tudo como lido',
  'notif.nothingYet': 'Nada ainda',
  'notif.nothingYetBody': 'Atribuições de tarefas e prazos chegam aqui.',
  'notif.dueAt': 'Prazo {when} · {countdown}',
  'session.blocked':
    'O navegador bloqueou a conexão com o servidor. Se continuar acontecendo, o site pode não ter permissão para acessar a API.',
  'notif.optInTitle': 'Receba avisos no seu computador',
  'notif.optInBody':
    'Saiba na hora quando uma tarefa for atribuída a você ou um prazo estiver perto — mesmo com o Task Studio em outra aba. Você pode desligar quando quiser.',
  'notif.optInAllow': 'Ativar',
  'notif.optInLater': 'Agora não',

  'chat.placeholder': 'Escreva uma mensagem…',
  'chat.offline': 'Offline',
  'chat.offlineQueued': 'Offline — as mensagens serão enviadas ao reconectar',
  'chat.loadEarlier': 'Carregar mensagens anteriores',
  'chat.loadingEarlier': 'Carregando…',
  'chat.retry': 'Não enviada — enviar novamente',
  'chat.send': 'Enviar',
  'chat.empty': 'Nenhuma mensagem ainda. Diga olá para a equipe.',
  'chat.loading': 'Carregando a conversa…',
  'chat.connected': 'Conectado',
  'chat.reconnecting': 'Reconectando…',
  'chat.pinned': 'Fixado',
  'chat.close': 'Fechar conversa',
  'chat.closeTitle': 'Fechar a conversa',
  'chat.typing': '{count} digitando…',
  'chat.sending': 'Enviando',
  'chat.sent': 'Enviada',
  'chat.notSent': 'Não enviada',
  'chat.notSentHelp': 'Esta mensagem não foi enviada. Verifique sua conexão e envie novamente.',

  'theme.toLight': 'Mudar para o tema claro',
  'theme.toDark': 'Mudar para o tema escuro',

  'common.cancel': 'Cancelar',
  'common.save': 'Salvar',
  'common.edit': 'Editar',
  'common.add': 'Adicionar',
  'common.delete': 'Excluir',
  'common.close': 'Fechar',
  'common.retry': 'Tentar novamente',
  'common.loading': 'Carregando…',
  'common.you': 'Você',
  'common.somethingWentWrong': 'Algo deu errado.',
  'error.badRequest': 'Não foi possível concluir. Confira os dados e tente de novo.',
  'error.invalid': 'Alguns dados não são válidos. Confira o formulário e tente de novo.',
  'error.unauthorized': 'Você precisa estar conectado para fazer isso.',
  'error.forbidden': 'Você não tem permissão para fazer isso.',
  'error.notFound': 'Isso não está mais aqui.',
  'error.conflict': 'Isso conflita com algo que já existe.',
  'error.tooLarge': 'Isso é grande demais para enviar.',
  'error.tooMany': 'Tentativas demais. Espere um instante e tente de novo.',
  'error.server': 'Algo deu errado do nosso lado. Tente de novo em instantes.',
  'error.unavailable': 'O servidor está indisponível agora. Tente de novo em instantes.',
  'common.showPassword': 'Mostrar senha',
  'mobile.title': 'A versão para celular ainda está em obras',
  'mobile.body':
    'Por enquanto o Task Studio é uma ferramenta de mesa — quadros que você arrasta, uma lousa, um chat acoplado. A versão para celular está vindo, e preferimos terminá-la a te entregar metade dela.',
  'mobile.desktop': 'Abra em um notebook ou desktop e está tudo lá.',
  'mobile.tablet': 'Tablet também funciona — qualquer tela maior que a de um celular abre normalmente.',
  'mobile.note': 'Sua conta e tudo que há nela seguem intactos.',
  'common.hidePassword': 'Ocultar senha',
  'common.noDescription': 'Sem descrição ainda.',
  'session.unreachable': 'Não foi possível alcançar o servidor. Verifique sua conexão.',
  'session.slowStart':
    'O servidor está demorando demais para responder — talvez esteja iniciando. Tente de novo em instantes.',
  'board.pagesFull': 'Um mural comporta no máximo {max} páginas',
  'board.pagesFullFree': 'Seu plano comporta {max} páginas — faça upgrade para ter mais',
  'board.projectPagesFull':
    'O plano deste projeto comporta {max} páginas no quadro — o dono pode fazer upgrade para ter mais',
  'board.addPage': 'Adicionar uma página',
  'themes.previewFirst': 'Veja qualquer um deles antes de decidir',
  'themes.moreInGallery':
    'mais {count} na galeria — veja qualquer um deles antes de decidir',
  'auth.dragCard.signIn': 'Entrar',
  'auth.dragCard.dragMe': 'Arraste para onde quiser',
  'nav.unpinHint': 'Desafixar — deixar o menu se esconder de novo',
  'nav.pinHint': 'Manter o menu aberto',
  'nav.dragToPin': 'Arraste para fixar isto em qualquer lugar da tela',
  'stage.collapse': 'Voltar para a página',
  'stage.expand': 'Expandir para tela cheia',
  'chat.pinnedHint':
    'Fixado na tela — o chat te acompanha entre as abas. Clique para desafixar.',
  'chat.dragToPin': 'Arraste até o chat para mantê-lo na tela em todo lugar',
  'chat.unpinAria': 'Desafixar o chat da tela',
  'chat.pinAria': 'Fixar o chat na tela',
  'board.moveLocked':
    'Só quem está designado nesta tarefa (ou um admin do projeto) pode movê-la.',
  'common.fullScreen': 'Tela cheia',
  'common.renameNamed': 'Renomear {name}',
  'common.removeNamed': 'Remover {name}',
  'common.openNamed': 'Abrir {name}',
  'common.pageNumber': 'Página {number}',
  'common.useColour': 'Usar {color}',
  'common.textColour': 'Cor do texto {color}',
  'common.decrease': 'Diminuir {label}',
  'common.increase': 'Aumentar {label}',
  'common.value': 'valor',
  'common.createdBy': 'Criado por {name} · {date}',
  'common.createdByWithEmail': 'Criado por {name} ({email}) · {date}',
  'common.lastEditedBy': 'Editado por último por {name} · {date}',
  'common.startsOn': 'Começa {date}',
  'common.dueOn': 'Prazo {date}',
  'common.completedOn': 'Concluída {date}',
  'common.attachedTo': 'Anexado a {title}',
  'common.overdueCount': '{count} atrasada(s)',
  'common.notifications': 'Notificações',
  'common.notificationsUnread': 'Notificações ({count} não lidas)',
  'common.notesFrom': '{count} nota(s) de {names}',
  'common.notesCount': '{count} nota(s)',
  'type.MEGA': 'MegaTarefa',
  'type.MEGA.short': 'Mega',
  'type.MEGA.hint': 'Mais de 2 dias',
  'type.MICRO': 'MicroTarefa',
  'type.MICRO.short': 'Micro',
  'type.MICRO.hint': 'Menos de 8 horas',
  'type.MULTI': 'MultiTarefa',
  'type.MULTI.short': 'Multi',
  'type.MULTI.hint': 'Vários responsáveis',
  'type.STANDARD': 'Tarefa',
  'type.STANDARD.short': 'Tarefa',
  'type.STANDARD.hint': 'Entre 8 horas e 2 dias',

  'status.TODO': 'A fazer',
  'status.IN_PROGRESS': 'Em andamento',
  'status.COMPLETED': 'Concluída',

  'priority.LOW': 'Baixa',
  'priority.NORMAL': 'Normal',
  'priority.HIGH': 'Alta',
  'priority.URGENT': 'Urgente',

  'dates.noDeadline': 'Sem prazo',
  'dates.late': '{amount} de atraso',
  'dates.dueIn': 'em {amount}',
  'dates.windowUnderHour': '< 1 hora',
  'dates.windowHours': 'janela de {count}h',
  'dates.windowDays': 'janela de {count}d',
  'views.lateTag': 'Atrasada',
  'views.doneLateTag': 'Concluída com atraso',
  'views.signedOff': '{done}/{total} aprovaram',
  'views.completedOn': 'Concluída em {date}',
  'views.late': 'Atrasada',
  'views.taskCount': '{count} tarefa(s)',
  'filters.myTasks': 'Minhas tarefas',
  'filters.all': 'Todas',
  'filters.personal': 'Pessoais',
  'filters.status': 'Status',
  'filters.type': 'Tipo',
  'filters.priority': 'Prioridade',
  'filters.time': 'Prazo',
  'filters.late': 'Atrasada',
  'filters.lateHint': 'Passou do prazo e ainda está aberta',
  'filters.lateFinish': 'Concluída com atraso',
  'filters.lateFinishHint': 'Terminada depois do prazo',
  'filters.onTime': 'No prazo',

  'views.board': 'Quadro',
  'views.boardHint': 'Colunas de status para arrastar entre elas',
  'views.sprintHint': 'Colunas de status cruzadas com faixas de prioridade',
  'views.list': 'Lista',
  'views.listHint': 'Uma linha compacta por tarefa',
  'views.calendar': 'Calendário',
  'views.calendarHint': 'Uma grade mensal por prazo',
  'views.agenda': 'Agenda',
  'views.agendaHint': 'Blocos por dia, ordenados por hora',

  'board.expand': 'Expandir',
  'board.shrink': 'Recolher',
  'board.draw': 'Desenhar',
  'board.drawHint': 'Traço livre sobre o mural',
  'board.arrangeNotesHint': 'Arraste as notas, use o laço para selecionar',
  'board.connectNotesHint': 'Desenhe setas entre as notas',
  'board.undo': 'Desfazer',
  'board.redo': 'Refazer',
  'board.nothingToUndo': 'Nada para desfazer',
  'board.nothingToRedo': 'Nada para refazer',
  'board.history.addNote': 'nota adicionada',
  'board.history.delete': 'nota excluída',
  'board.history.edit': 'nota editada',
  'board.history.move': 'nota movida',
  'board.history.connect': 'notas conectadas',
  'board.history.group': 'notas agrupadas',
  'board.history.draw': 'traço desenhado',
  'board.history.erase': 'traço apagado',
  'board.history.clearInk': 'tinta apagada',

  'board.inkSize': 'Espessura',
  'board.eraserNib': 'Ponta',
  'board.sharedWallBody':
    'Cole um post-it, prenda uma imagem ou desenhe direto no mural. Todo mundo no projeto vê enquanto você faz.',

  'notes.selectNote': 'Selecionar esta nota',
  'notes.deselectNote': 'Desmarcar esta nota',
  'notes.noteTitle': 'Título',
  'notes.imageCaption': 'Legenda',
  'notes.boardImage': 'Imagem do mural',
  'notes.useColour': 'Usar {color}',
  'notes.postIts': 'Post-its',
  'notes.textBoardTab': 'Documentos',
  'notes.yourPostItBoard': 'Seu mural de post-its',
  'notes.confirmClearPage': 'Limpar esta página? As notas vão para a lixeira.',
  'notes.confirmRemovePage': 'Remover "{name}"? As notas dela vão para a lixeira.',

  'project.tabChat': 'Conversa',
  'project.tabsLabel': 'Seção do projeto',
  'project.tabBoard': 'Quadro',
  'project.tabMetrics': 'Métricas',
  'project.tabRoster': 'Equipe',
  'project.tabWhiteboard': 'Mural',
  'project.tabText': 'Documentos',
  'project.tabMeetings': 'Reuniões',
  'project.tabLive': 'Ao vivo',
  // ---- Salas ao vivo -----------------------------------------------------
  'live.heading': 'Salas ao vivo',
  'live.subheading': 'Converse, com câmera e tela. Ponto a ponto — nada é gravado.',
  'live.new': 'Nova sala',
  'live.newTitle': 'Abrir uma sala ao vivo',
  'live.editTitle': 'Editar esta sala',
  'live.open': 'Abrir a sala',
  'live.composerHint': 'Tudo aqui pode ser alterado depois, inclusive durante a chamada.',
  'live.emptyTitle': 'Nenhuma sala ao vivo ainda',
  'live.emptyBody':
    'Uma sala ao vivo é uma chamada que qualquer pessoa do projeto pode entrar — voz, câmera e tela, sem gravação. Abra uma quando conversar for mais rápido que escrever.',
  'live.showEnded': 'Mostrar encerradas',

  'live.titleLabel': 'Sobre o que é a chamada?',
  'live.titlePlaceholder': 'Revisão da sprint, apresentação do design…',
  'live.descriptionLabel': 'Recado para quem entrar',
  'live.descriptionPlaceholder': 'Opcional. O que você quer tratar.',

  'live.whenLabel': 'Quando',
  'live.opensNow': 'Abrir agora',
  'live.opensLater': 'Agendar',
  'live.opensAtLabel': 'Abre às',
  'live.closesAtLabel': 'Fecha às (opcional)',
  'live.closesAtHint': 'Deixe vazio para manter aberta até alguém encerrar.',

  'live.whoLabel': 'Quem',
  'live.audienceLabel': 'Quem pode entrar',
  'live.audienceEveryone': 'Todo o projeto',
  'live.audienceEveryoneHint': 'Qualquer pessoa da equipe pode entrar.',
  'live.audienceSelected': 'Somente convidados',
  'live.audienceSelectedHint': 'Pessoas, times e quem estiver nas tarefas abaixo.',
  'live.guestsLabel': 'Quem está convidado',
  'live.notifyLabel': 'Quem avisar',

  'live.subjectLabel': 'Sobre o que é',
  'live.tasksLabel': 'Tarefas na pauta',
  'live.tasksHint':
    'Todo mundo atribuído a uma tarefa escolhida é convidado e avisado — por e-mail também, já que não foi quem marcou a reunião.',
  'live.noTasks': 'Nenhuma tarefa aberta neste projeto ainda.',
  'live.tasksAdds': 'Também convidados e avisados por e-mail:',
  'live.documentLabel': 'Página para abrir junto',
  'live.documentHint': 'Do quadro de textos, para todos chegarem olhando a mesma coisa.',
  'live.noDocument': 'Nenhuma página',

  'live.permissionsLabel': 'O que as pessoas podem fazer',
  'live.talkLabel': 'Falar',
  'live.talkOpen': 'Qualquer um pode falar',
  'live.talkOpenHint': 'Todos entram sem áudio e podem se desmutar.',
  'live.talkLimited': 'Só com permissão',
  'live.talkLimitedHint': 'As pessoas pedem e um moderador passa o microfone.',
  'live.screenLabel': 'Compartilhar tela',
  'live.screenOpen': 'Qualquer um pode compartilhar',
  'live.screenOpenHint': 'Qualquer participante pode apresentar a tela.',
  'live.screenLimited': 'Só com permissão',
  'live.screenLimitedHint': 'Um moderador libera uma pessoa por vez.',
  'live.moderatorHint':
    'Você e qualquer admin do projeto são sempre moderadores — podem falar, apresentar e liberar os dois durante a chamada.',

  'live.stateLive': 'Ao vivo',
  'live.stateScheduled': 'Agendada',
  'live.stateEnded': 'Encerrada',
  'live.join': 'Entrar',
  'live.joinEarly': 'Entrar antes',
  'live.joinHint':
    'Seu navegador vai pedir o microfone e a câmera. Os dois começam desligados — nada é enviado até você ligar.',
  'live.leave': 'Sair',
  'live.end': 'Encerrar sala',
  'live.opensIn': 'Abre {when}',
  'live.openedAt': 'Aberta desde {when}',
  'live.endedAt': 'Encerrada {when}',
  'live.hostedBy': 'Aberta por {name}',
  'live.aboutTasks': '{count} tarefas',
  'live.inRoom': '{count} de {max} aqui',

  'live.requestingDevices': 'Aguardando seu microfone…',
  'live.connecting': 'Conectando à sala…',
  'live.you': 'Você',
  'live.moderator': 'Moderador',
  'live.sharing': 'Compartilhando a tela',
  'live.handRaised': 'Mão levantada',
  'live.micOn': 'Microfone ligado',
  'live.micOff': 'Sem áudio',
  'live.muteSelf': 'Desligar microfone',
  'live.unmuteSelf': 'Ligar microfone',
  'live.cameraOn': 'Ligar câmera',
  'live.cameraOff': 'Desligar câmera',
  'live.share': 'Compartilhar tela',
  'live.stopShare': 'Parar de compartilhar',
  'live.raiseHand': 'Pedir para falar',
  'live.allowSpeak': 'Liberar o microfone',
  'live.denySpeak': 'Tirar o microfone',
  'live.allowPresent': 'Liberar a tela',
  'live.denyPresent': 'Bloquear a tela',
  'live.listenOnly': 'Você está só ouvindo. Peça para falar e um moderador pode passar o microfone.',

  'live.qualityWeak': 'A conexão está sofrendo - {loss}% dos pacotes perdidos, {jitter}ms de jitter. Desligar sua câmera ajuda.',
  'live.qualityBad': 'Conexão ruim - {loss}% dos pacotes perdidos, {jitter}ms de jitter. Talvez só o áudio funcione neste link.',
  'live.relayed': 'Passando pelo retransmissor em vez de ir direto, o que acrescenta um pequeno atraso. Em geral significa que um dos lados está atrás de um firewall restritivo.',

  'live.created': 'Sala aberta.',
  'live.ended': 'Sala encerrada.',
  'live.removed': 'Sala excluída.',
  'live.createFailed': 'Não foi possível abrir a sala',
  'live.updateFailed': 'Não foi possível salvar a sala',
  'live.endFailed': 'Não foi possível encerrar a sala',
  'live.removeFailed': 'Não foi possível excluir a sala',
  'live.grantFailed': 'Não foi possível alterar essa permissão',
  'live.joinFailed': 'Não foi possível entrar na sala',
  'live.noDevices': 'Sem microfone ou câmera — você ainda pode ouvir.',
  'live.mediaBlocked':
    'Não foi possível conectar áudio e vídeo com {name}. Um firewall em uma das redes provavelmente está bloqueando chamadas — tente outra rede.',
  'live.noMicrophone': 'Nenhum microfone disponível.',
  'live.noCamera': 'Nenhuma câmera disponível.',
  'live.askToSpeak': 'Um moderador precisa liberar sua fala nesta sala.',
  'live.askToPresent': 'Um moderador precisa liberar o compartilhamento nesta sala.',
  'live.muteByModerator': 'Um moderador desligou seu microfone.',
  'live.displaced': 'Você entrou nesta sala em outro lugar.',
  'project.tabAssistant': 'Assistente',
  'project.tabChangelog': 'Histórico',

  'task.afterProjectEnd': 'Este projeto termina em {date}. Escolha um prazo anterior.',

  'activity.title': 'Histórico',
  'activity.subtitle': 'Tudo o que aconteceu neste projeto, do mais recente ao mais antigo.',
  'activity.entryCount': '{count} registros',
  'activity.loadMore': 'Carregar registros antigos',
  'activity.empty': 'Nada aconteceu ainda',
  'activity.emptyHint':
    'Tarefas, páginas, reuniões e mudanças na equipe deixam uma linha aqui. A primeira aparece assim que alguém fizer algo.',
  'activity.failed': 'Não foi possível carregar o histórico',
  'activity.failedHint': 'O resto do projeto não foi afetado — tente de novo em instantes.',
  'activity.someone': 'Alguém',
  'activity.somethingUnnamed': 'algo',

  'activity.projectCreated': '{actor} criou este projeto',
  'activity.projectImported': '{actor} importou este projeto do GitHub',
  'activity.projectRenamed': '{actor} renomeou o projeto de “{from}” para “{subject}”',
  'activity.projectCompleted': '{actor} encerrou o projeto',
  'activity.projectReopened': '{actor} reabriu o projeto',
  'activity.projectFiled': '{actor} arquivou o projeto em {target}',
  'activity.projectUnfiled': '{actor} removeu o projeto de {target}',
  'activity.memberInvited': '{actor} convidou {target}',
  'activity.memberJoined': '{actor} agora é membro',
  'activity.memberLeft': '{actor} saiu do projeto',
  'activity.memberRemoved': '{actor} removeu {target} do projeto',
  'activity.memberRoleChanged': '{actor} tornou {target} {role}',
  'activity.taskCreated': '{actor} criou a tarefa “{subject}”',
  'activity.taskCompleted': '{actor} concluiu “{subject}”',
  'activity.taskReopened': '{actor} reabriu “{subject}”',
  'activity.taskDeleted': '{actor} excluiu a tarefa “{subject}”',
  'activity.documentCreated': '{actor} escreveu a página “{subject}”',
  'activity.documentImported': '{actor} importou o arquivo “{subject}”',
  'activity.documentConverted': '{actor} converteu “{subject}” em uma página editável',
  'activity.documentDeleted': '{actor} excluiu a página “{subject}”',
  'activity.meetingScheduled': '{actor} marcou a reunião “{subject}”',
  'activity.actionReverted': '{actor} desfez uma ação de {target}',

  'activity.revert': 'Desfazer',
  'activity.revertConfirm': 'Desfazer',
  'activity.revertHint': 'Reverte esta ação. A linha continua no registro, riscada.',
  'activity.revertedBy': 'desfeito por {actor}',
  'activity.revertFailed': 'Não foi possível desfazer.',
  'project.newTask': 'Nova tarefa',
  'project.newTaskShort': 'Tarefa',
  'project.pinProject': 'Fixar projeto',
  'project.unpinProject': 'Desafixar projeto',

  'task.autoClassified': 'classificada automaticamente',
  'task.subChecklist': 'Subchecklist',

  'bin.confirmPurgeTask': 'Excluir "{title}" definitivamente?',
  'task.noteChecklist': 'Lista de notas',
  'task.noteChecklistEmpty':
    'Nenhum passo ainda. Toque em + para escrever um num post-it — até três por tarefa.',
  'task.noteChecklistFull': 'Uma tarefa guarda no máximo {max} notas. Apague uma para abrir espaço.',
  'task.note': 'Nota',
  'task.newNote': 'Nova nota',
  'task.newNoteHint': 'Um passo, escrito num post-it. Qualquer pessoa na tarefa pode marcá-lo.',
  'task.newNotePlaceholder': 'O que precisa ser feito?',
  'task.noteColour': 'Papel',
  'task.ticked': 'Concluído',
  'task.tickedBy': 'Concluído por {name}',
  'task.suggestSteps': 'Sugerir',
  'task.suggestStepsHint': 'Peça ao assistente até três passos para esta tarefa.',
  'task.stepsFull': 'Uma tarefa comporta três passos iniciais.',
  'task.lateOpen': 'Passou do prazo e ainda está aberta',
  'task.lateDone': 'Concluída depois do prazo',
  'task.pin': 'Fixar tarefa',
  'task.unpin': 'Desafixar tarefa',
  'task.completionNotYours': 'Só quem está designado nesta tarefa pode concluí-la.',
  'task.completionOwnedByAssignees': 'A conclusão pertence aos responsáveis',
  'task.completionTicksOwnBox':
    'Marca apenas a sua parte. A tarefa só fica concluída quando todos os responsáveis marcarem a delas.',
  'task.everyoneTicked': 'Todos já marcaram a parte deles.',
  'task.waitingOn': 'Aguardando {names}',
  'task.sharedBlocked':
    'Tarefa compartilhada — ainda aguardando {people}. Todos os responsáveis precisam marcar a parte deles, ou um admin do projeto pode encerrá-la.',
  'task.sharedBlockedCount': 'mais {count} responsável(is)',
  'task.andMore': ' e mais {count}',
  'task.listJoin': '{list} e {last}',
  'task.noteBy': 'Escrito por {name} — abra para ler o recado inteiro.',

  'project.tabGroups': 'Grupos',
  'groups.tagLabel': 'Grupo',
  'groups.noTag': 'Sem grupo',
  'groups.untagged': 'Sem grupo',
  'groups.untaggedHint': 'Tarefas ainda sem grupo. Arraste uma para uma coluna para arquivá-la.',
  'groups.addColumn': 'Adicionar coluna',
  'groups.newColumn': 'Nova coluna',
  'groups.renameColumn': 'Renomear coluna',
  'groups.columnHint': 'Um rótulo que este projeto inventa para si — não é um status.',
  'groups.columnName': 'Nome',
  'groups.columnNamePlaceholder': 'Wireframe, Back end, Prospecção…',
  'groups.columnColour': 'Cor',
  'groups.rename': 'Renomear coluna',
  'groups.delete': 'Excluir coluna',
  'groups.moveLeft': 'Mover coluna para a esquerda',
  'groups.moveRight': 'Mover coluna para a direita',
  'groups.dropHere': 'Solte uma tarefa aqui',
  'groups.full': 'Este quadro já tem o máximo de {max} colunas.',
  'groups.deleteConfirm': 'Excluir a coluna “{name}”?',
  'groups.deleteConfirmWithTasks':
    'Excluir a coluna “{name}”? As {count} tarefa(s) dela são mantidas e ficam sem grupo.',
  'groups.deleted': 'Coluna excluída. {count} tarefa(s) agora estão sem grupo.',
  'groups.createFailed': 'Não foi possível criar essa coluna.',
  'groups.emptyTitle': 'Nenhuma coluna ainda',
  'groups.emptyBody':
    'Agrupe as tarefas deste projeto do jeito que ele realmente funciona — fases, áreas, clientes. Cada tarefa mantém seu status, mostrado no card.',
  'groups.emptyBodyMember':
    'Um proprietário ou admin pode criar colunas aqui para agrupar as tarefas do projeto.',

  'groups.filterOpen': 'Em aberto',
  'groups.filterDone': 'Concluídas',
  'groups.noneDoneHere': 'Nada concluído nesta coluna ainda',
  'groups.hiddenCount': '{count} oculta(s)',
  'groups.showAllColumns': 'Mostrar todas',
  'groups.hideColumn': 'Ocultar “{name}” na minha tela',
  'groups.showColumn': 'Mostrar “{name}” novamente',
  'groups.previousPage': 'Colunas anteriores',
  'groups.nextPage': 'Mais colunas',
  'groups.allHiddenTitle': 'Todas as colunas estão ocultas',
  'groups.allHiddenBody':
    'Você ocultou todas as colunas deste quadro. Elas ficam ocultas apenas neste dispositivo — traga-as de volta quando quiser.',
  'groups.noneMatchTitle': 'Nada para mostrar aqui',
  'groups.noneOpenBody': 'Não há trabalho em aberto neste quadro agora.',
  'groups.noneDoneBody': 'Nada neste quadro foi concluído ainda.',
  'groups.addTaskHere': 'Nova tarefa em “{name}”',
  'groups.tagLocked': 'Definido pela coluna de onde esta tarefa foi criada.',
  'groups.signOff': '{done} de {total} responsáveis já confirmaram',
  'groups.completeAsAdmin': 'Encerrar esta tarefa em nome da equipe.',
  'groups.completeShared':
    'Marca apenas a sua parte. A tarefa só fica concluída quando todos os responsáveis marcarem a delas.',

  'ai.suggestFailed': 'Não foi possível falar com o assistente.',
  'ai.stepsAdded': '{count} passo(s) adicionado(s).',
  'ai.noStepsAdded': 'Nada a adicionar — esta tarefa já está cheia.',
  'ai.needsTitleAndBody':
    'Escreva um título e uma descrição primeiro — é o que o assistente lê.',


  // ---- Theme gallery ------------------------------------------
  'skin.STUDIO.tagline': 'O padrão discreto',
  'skin.STUDIO.body':
    'Superfícies suaves, um único acento azul-petróleo e nada que peça sua atenção — o visual do qual todos os outros são um desvio.',
  'skin.PAPER.tagline': 'Ilustrado e barulhento',
  'skin.PAPER.body':
    'Contornos grossos de tinta, cantos robustos e sombras que parecem papel recortado pousado sobre a página, em vez de um borrão.',
  'skin.TERMINAL.tagline': 'Fósforo e scanlines',
  'skin.TERMINAL.body':
    'Monoespaçada em tudo, cantos retos, linhas de neon e uma leve varredura de scanlines sobre o tubo inteiro.',
  'skin.VINTAGE.tagline': 'Latão e couro',
  'skin.VINTAGE.body':
    'Cantos usinados, filetes gravados e controles que capturam um brilho na borda superior e afundam quando pressionados.',
  'skin.PIXEL.tagline': 'Cartucho de 8 bits',
  'skin.PIXEL.body':
    'Ladrilhos nine-slice com os pixels dos cantos removidos, dithering ordenado como material e movimento quantizado em quadros inteiros.',
  'skin.SPACE.tagline': 'O campo profundo',
  'skin.SPACE.body':
    'Instrumentos de vidro iluminados sobre um campo de estrelas à deriva, com uma singularidade afundada em cada borda da tela onde um menu se esconde.',
  'skin.HAZARD.tagline': 'Falha de contenção',
  'skin.HAZARD.body':
    'Avisos estampados, fita de perigo em cada emenda e lodo tóxico acumulado no fundo de cada card — que balança junto.',
  'skin.NEWSPAPER.tagline': 'A edição da manhã',
  'skin.NEWSPAPER.body':
    'Papel em meio-tom, colunas com filetes, manchetes sob um fio duplo e fotografias impressas do único jeito que a prensa permitia — em preto.',
  'skin.ELDRITCH.tagline': 'Algo está lendo isto',
  'skin.ELDRITCH.body':
    'Cantos que cresceram em vez de serem cortados, luz sem origem, capitulares inscritas — e um olho que se abre em algum ponto da página enquanto você olha para outro lado.',
  'skin.AUTUMN.tagline': 'Papel quente e folhas caindo',
  'skin.AUTUMN.body':
    'Papel de algodão prensado sob um sol baixo, cantos arredondados por inteiro, uma serifada em que dá para ler um romance — e folhas descendo sobre tudo isso.',
  'skin.RUNIC.tagline': 'Escrito a tinta em papel antigo',
  'skin.RUNIC.body':
    'Papel de algodão manchado pelo tempo, com as marcas da fôrma ainda visíveis, margens gastas em alguns trechos e letras cor de sangue-de-boi que penetraram na fibra — nas barras, nos ícones, sob o seu cursor.',
  'skin.UNDERWATER.tagline': 'Mantido sob a água',
  'skin.UNDERWATER.body':
    'Luz quebrada vinda de uma superfície lá em cima, cantos que a água teve muito tempo para arredondar, cores que escorrem para o azul conforme a profundidade — e bolhas subindo por tudo.',
  'skin.VIBECODED.tagline': 'Igualzinho a todo outro app',
  'skin.VIBECODED.body': 'não escrevi uma única linha para esse aqui',
  'skin.HALLOWEEN.tagline': 'Algo se mexeu no canto',
  'skin.HALLOWEEN.body':
    'Escuridão à luz de vela, com uma lanterna esculpida como marca. Teias se formam nas portas que você não abriu, e abrir qualquer coisa assusta o que dormia lá dentro.',
  'skin.VOLCANO.tagline': 'A crosta é uma tampa',
  'skin.VOLCANO.body':
    'Placas cinza-cinza rachadas por fraturas, uma costura quente sob cada borda onde o magma aparece, tipografia golpeada em vez de composta — e brasas subindo de tudo isso.',
  'skin.DRAGON.tagline': 'Algo comprido passa pela janela',

  'theme.light': 'Claro',
  'theme.dark': 'Escuro',

  'editor.sizeSmall': 'Pequeno',
  'editor.sizeNormal': 'Normal',
  'editor.sizeLarge': 'Grande',
  'editor.sizeHuge': 'Enorme',

  'ai.startsToday': 'começa hoje',
  'ai.startsInDays': 'começa em {count}d',
  'ai.lastsHours': '{count}h de trabalho',
  'ai.lastsDays': '{count}d de trabalho',

  'meetings.new': 'Nova reunião',
  'meetings.newTitle': 'Publicar reunião',
  'meetings.editTitle': 'Editar reunião',
  'meetings.composerSubtitle':
    'Toda a equipe pode ler. Só donos e administradores podem alterar.',
  'meetings.post': 'Publicar reunião',
  'meetings.saveChanges': 'Salvar alterações',
  'meetings.nameLabel': 'Nome da reunião',
  'meetings.namePlaceholder': 'Revisão da sprint',
  'meetings.roomLabel': 'Sala',
  'meetings.roomPlaceholder': 'Sala de reunião 2',
  'meetings.roomElsewhere': 'Em outro lugar',

  'rooms.title': 'Salas',
  'rooms.manage': 'Salas',
  'rooms.manageHint': 'Cadastre as salas que esta agenda pode reservar',
  'rooms.add': 'Adicionar sala',
  'rooms.nameLabel': 'Nome',
  'rooms.namePlaceholder': 'Sala de reunião 2',
  'rooms.locationLabel': 'Onde fica',
  'rooms.locationPlaceholder': '3º andar, ala leste',
  'rooms.capacityLabel': 'Lugares',
  'rooms.capacityInvalid': 'Entre 1 e 10000.',
  'rooms.none': 'Nenhuma sala cadastrada',
  'rooms.noneBody':
    'Cadastre uma e ela passa a ser reservável — duas reuniões nunca dividem a mesma.',
  'rooms.inherited': 'Da organização',
  'rooms.remove': 'Remover sala',
  'rooms.removeHint': 'As reuniões já marcadas nela mantêm o nome.',
  'rooms.created': 'Sala adicionada.',
  'rooms.saved': 'Sala salva.',
  'rooms.saveFailed': 'Não foi possível salvar a sala.',
  'rooms.deleted': 'Sala removida.',
  'rooms.deleteFailed': 'Não foi possível remover a sala.',
  'meetings.starts': 'Começa',
  'meetings.ends': 'Termina',
  'meetings.windowInvalid': 'A reunião precisa terminar depois de começar.',
  'meetings.descriptionLabel': 'Pauta',
  'meetings.descriptionPlaceholder': 'Para que é a reunião e o que levar.',
  'meetings.participants': 'Participantes',
  'meetings.linkProject': 'Sobre um projeto',
  'meetings.linkProjectNone': 'A organização como um todo',
  'meetings.linkProjectHint':
    'Escolha um e a reunião também é publicada no quadro daquele projeto. Isso não pode ser alterado depois.',
  'meetings.companyWide': 'Toda a organização',
  'meetings.wholeRoster': 'Toda a equipe',
  'meetings.until': 'até',
  'meetings.liveNow': 'Acontecendo agora',
  'meetings.edit': 'Editar reunião',
  'meetings.markComplete': 'Marcar como concluída',
  'meetings.markCompleteHint': 'Marcar como concluída — ela sai do quadro.',
  'meetings.delete': 'Excluir reunião',
  'meetings.confirm': 'Confirmar',
  'meetings.search': 'Buscar reuniões',
  'meetings.searchPlaceholder': 'Buscar pelo nome',
  'meetings.viewUpcoming': 'Próximas',
  'meetings.viewByDay': 'Por dia',
  'meetings.viewCalendar': 'Calendário',
  'meetings.count': '{count} reunião(ões)',
  'meetings.none': 'Nada agendado',
  'meetings.noneBody':
    'Quando um dono ou administrador publicar uma reunião, ela aparece aqui.',
  'meetings.noneBodyAdmin': 'Publique a primeira — toda a equipe vai ver.',
  'meetings.noMatch': 'Nenhuma reunião com esse nome',
  'meetings.noMatchBody': 'Nada agendado corresponde a “{query}”.',
  'meetings.today': 'Hoje',
  'meetings.previousDay': 'Dia anterior',
  'meetings.nextDay': 'Próximo dia',
  'meetings.pickDay': 'Ir para um dia',
  'meetings.previousMonth': 'Mês anterior',
  'meetings.nextMonth': 'Próximo mês',
  'meetings.nothingOnDay': 'Nada neste dia',
  'meetings.nothingOnDayBody': 'A agenda está livre.',
  'meetings.jumpPrevious': 'Dia agendado anterior',
  'meetings.jumpNext': 'Próximo dia agendado',
  'meetings.created': 'Reunião publicada.',
  'meetings.updated': 'Reunião atualizada.',
  'meetings.completed': 'Reunião marcada como concluída.',
  'meetings.deleted': 'Reunião excluída.',
  'meetings.createFailed': 'Não foi possível publicar a reunião.',
  'meetings.saveFailed': 'Não foi possível salvar a reunião.',
  'meetings.deleteFailed': 'Não foi possível excluir a reunião.',

  'ai.noCreditsLeft': 'Sem chamadas do assistente neste mês',
  'ai.noCreditsLeftBody': 'Você usou todas as {limit} do seu plano. Elas renovam em {date} — ou faça upgrade nas Configurações para ter mais.',


  'landing.nav.pricing': 'Preços',
  'landing.pricing.title': 'Escolha seu plano',
  'landing.pricing.subtitle': 'Comece grátis. Suba de plano quando o teto começar a aparecer.',
  'landing.pricing.monthly': 'Mensal',
  'landing.pricing.yearly': 'Anual',
  'landing.pricing.save': '2 meses grátis',
  'landing.pricing.popular': 'Popular',
  'landing.pricing.free': 'Grátis',
  'landing.pricing.perMonth': '/mês',
  'landing.pricing.perYear': '/ano',
  'landing.pricing.billedYearly': '{amount} por mês, cobrado anualmente',
  'landing.pricing.ctaFree': 'Começar',
  'landing.pricing.ctaPaid': 'Começar',
  'landing.pricing.unavailable': 'Ainda não vendido aqui',
  'landing.pricing.feat.projects': '{count} projetos por vez',
  'landing.pricing.feat.orgOne': '1 organização por vez',
  'landing.pricing.feat.orgMany': '{count} organizações por vez',
  'landing.pricing.feat.members': '{count} pessoas por projeto',
  'landing.pricing.feat.membersUnlimited': 'Pessoas ilimitadas por projeto',
  'landing.pricing.feat.tasks': '{count} tarefas por vez em cada projeto',
  'landing.pricing.feat.docs': '{size} por quadro de documentos',
  'landing.pricing.feat.ai': '{count} chamadas do assistente por mês',
  'landing.pricing.feat.connectionsFree': 'Conexões com Discord e Figma',
  'landing.pricing.feat.themesAll': 'Todos os temas da galeria',
  'landing.pricing.feat.themesFree': 'Temas Studio e Paper',
  'landing.pricing.feat.connectionsAll': 'Todas as conexões — Slack, Discord, Figma, webhooks',

  'ai.steer': 'Oriente',
  'ai.steerOptional': 'Oriente o assistente (opcional)',
  'ai.steerPlaceholder': 'Foque no onboarding — vamos lançar em duas semanas.',
  'ai.steerHint': 'Uma nota sobre o que propor. Ela orienta o assunto, não o funcionamento do assistente.',
  'ai.steerTooLong': 'Mantenha a nota abaixo de {max} caracteres.',

  // ---- Planos e cobrança --------------------------------------------
  'billing.section': 'Plano',
  'billing.sectionHint': 'O que sua conta pode guardar, e quanto custa.',
  'billing.plan.FREE': 'Gratuito',
  'billing.plan.STARTUP': 'Startup',
  'billing.plan.BARON': 'Baron',
  'billing.currentPlan': 'Você está no {plan}',
  'billing.freeBlurb': 'Tudo o que você precisa para tocar um projeto de verdade, sozinho.',
  'billing.startupBlurb': 'O triplo do espaço, e todas as conexões liberadas.',
  'billing.baronBlurb': 'Cinco vezes o espaço, equipes sem limite e 30 chamadas do assistente para cada pessoa.',
  'billing.perMonth': '/mês',
  'billing.perYear': '/ano',
  'billing.monthly': 'Mensal',
  'billing.yearly': 'Anual',
  'billing.yearlySave': '2 meses grátis',
  'billing.choosePlan': 'Escolher {plan}',
  'billing.currentBadge': 'Plano atual',
  'billing.upgrade': 'Fazer upgrade',
  'billing.comparePlans': 'Comparar planos',
  'billing.hidePlans': 'Ocultar planos',
  'billing.manage': 'Gerenciar cobrança',
  'billing.manageHint': 'Troque o cartão, mude a frequência de pagamento ou cancele.',
  'billing.renewsOn': 'Renova em {date}',
  'billing.endsOn': 'Termina em {date}',
  'billing.pastDue': 'Um pagamento não foi aprovado',
  'billing.pastDueBody': 'Seu plano continua ativo enquanto o cartão é tentado de novo. Atualize-o para não perdê-lo.',
  'billing.grantedByAdmin': 'Aplicado diretamente à sua conta — não há nada a pagar.',
  'billing.grantedByAdminHint': 'Fale com o suporte para alterar.',
  'billing.notConfigured': 'Os pagamentos não estão configurados nesta instância.',
  'billing.notConfiguredBody': 'Todas as contas se comportam como Gratuito. Os limites abaixo continuam valendo.',
  'billing.checkoutFailed': 'Não foi possível abrir o checkout.',
  'billing.portalFailed': 'Não foi possível abrir o portal de cobrança.',
  'billing.checkoutDone': 'Pagamento recebido — estamos ativando seu plano.',
  'billing.checkoutCancelled': 'Checkout cancelado. Nada foi cobrado.',
  'billing.usage': 'O que você está usando',
  'billing.usage.projects': 'Projetos',
  'billing.usage.organizations': 'Organizações',
  'billing.usage.ai': 'Chamadas do assistente neste mês',
  'billing.usage.personalBoard': 'Seu quadro pessoal',
  'billing.usage.personalBoardNote': 'O quadro de um projeto tem o próprio limite, no plano de quem é dono dele',
  'billing.usage.resets': 'Renova em {date}',
  'billing.limit.unlimited': 'Ilimitado',
  'billing.limit.ofLimit': 'de {limit}',
  'planSoon.title': 'Esta sala ainda está em obras',
  'planSoon.body': 'Os planos pagos ainda não estão ativos. Estamos terminando o resto do estúdio primeiro.',
  'planSoon.bodyNamed': 'O {plan} ainda não está ativo. Estamos terminando o resto do estúdio antes de começar a cobrar.',
  'planSoon.reassure': 'Nada do que você já tem muda, e nada foi cobrado.',
  'planSoon.back': 'Voltar às configurações',
  'planSoon.backHome': 'Voltar para a página inicial',
  'planSoon.notify': 'Avise-me quando abrir',
  'planSoon.note': 'Tudo no seu plano gratuito continua funcionando exatamente como agora.',
  'billing.feature.projects': 'Projetos por vez',
  'billing.feature.organizations': 'Organizações por vez',
  'billing.feature.membersPerProject': 'Pessoas por projeto',
  'billing.feature.membersPerOrganization': 'Pessoas por organização',
  'billing.feature.tasksPerProject': 'Tarefas por vez, em cada projeto',
  'billing.feature.boardPages': 'Páginas no seu quadro de recados',
  'billing.feature.whiteboardPages': 'Páginas em cada quadro de projeto',
  'billing.feature.documentBoard': 'Armazenamento por quadro de documentos',
  'billing.feature.ai': 'Chamadas do assistente, por pessoa, por mês',
  'billing.feature.discord': 'Discord',
  'billing.feature.broadcast': 'Slack e endpoints próprios',
  'billing.feature.figma': 'Arquivos do Figma',
  'billing.feature.customThemes': 'Todos os temas',
  'billing.feature.yes': 'Incluído',
  'billing.feature.no': 'Fora deste plano',
  'billing.currency': 'Moeda',
  // Whiteboard picture folders on the Documents board — see `FolderDocument`.
  'folder.rowCount': '{count} imagens · do quadro branco',
  'folder.chip': 'Pasta do quadro branco',
  'folder.chipHint':
    'Preenchida automaticamente com cada imagem fixada nesta página do quadro branco.',
  'folder.summary': '{count} imagens · {size}',
  'folder.explain':
    'Toda imagem fixada nesta página do quadro branco também chega aqui. Abra uma para ver, ou salve uma ou todas.',
  'folder.detached':
    'A página do quadro branco foi removida. As imagens ficam aqui, e as novas vão para uma nova pasta.',
  'folder.downloadAll': 'Baixar tudo (.zip)',
  'folder.download': 'Baixar',
  'folder.downloadOne': 'Baixar {name}',
  'folder.remove': 'Remover {name} desta pasta',
  'folder.removeHint':
    'Remover da pasta — libera espaço no quadro de documentos. Uma imagem ainda fixada no quadro branco continua lá.',
  'folder.removeConfirm': 'Remover?',
  'folder.removeFailed': 'Não foi possível remover essa imagem.',
  'folder.saveFailed': 'Não foi possível baixar isso.',
  'folder.zipPartial':
    'Salvas {count} de {total} imagens — o resto era grande demais para um zip ou não está mais no armazenamento.',
  'folder.empty': 'Nenhuma imagem nesta pasta',
  'folder.emptyBody': 'As imagens fixadas na página do quadro branco aparecerão aqui.',
  'folder.loadFailed': 'Não foi possível abrir esta pasta',
  'folder.loadFailedBody': 'Verifique sua conexão e tente novamente.',
  'folder.filed': 'Também salva em Documentos › {title}',
  'folder.full.title': 'O quadro de documentos está cheio',
  'folder.full.subtitle': 'Sua imagem está no quadro branco — só falta a cópia em Documentos.',
  'folder.full.bodyBytes':
    'O quadro de documentos deste projeto não tem mais espaço para guardar esta imagem na pasta da página do quadro branco.',
  'folder.full.bodyPages':
    'O quadro de documentos deste projeto atingiu o limite de páginas, então não foi possível criar uma nova pasta para esta página do quadro branco.',
  'folder.full.storageLabel': 'Armazenamento',
  'folder.full.pagesLabel': 'Páginas',
  'folder.full.pictureSize': 'Esta imagem: {size}',
  'folder.full.hintTitle': 'Limpe arquivos sem uso',
  'folder.full.hintBody':
    'Abra o quadro de documentos e remova imagens, pastas ou páginas de que não precisa mais. Remover uma imagem de uma pasta nunca a tira do quadro branco.',
  'folder.full.askOwner':
    'O tamanho do quadro segue o plano do dono do projeto — peça a ele um upgrade para ter mais espaço.',
  'folder.full.onBaron':
    'Este projeto já está no Baron, o maior plano — liberar espaço é o caminho para abrir lugar.',
  'folder.full.openDocuments': 'Abrir quadro de documentos',
  'folder.full.upgrade': 'Fazer upgrade do plano',
  // Leaving a project from its settings — see `ProjectSettingsDialog`.
  'project.settingsSubtitleMember':
    'Você é integrante deste projeto. Só o dono e os administradores podem alterá-lo.',
  'project.leaveTitle': 'Sair deste projeto',
  'project.leaveExplain':
    'Você perde o acesso ao quadro, às páginas e ao chat, e suas atribuições voltam para a equipe. Nada é apagado — um convite traz você de volta.',
  'project.leaveExplainOwner':
    'Um projeto sempre tem um dono, então escolha quem assume. Essa pessoa vira a dona — e os limites passam a seguir o plano dela — e você sai.',
  'project.leaveOnlyMember':
    'Você é a única pessoa neste projeto, então não há para quem passá-lo. Convide alguém antes, ou finalize ou exclua abaixo.',
  'project.leaveSuccessor': 'Novo dono',
  'project.leave': 'Sair do projeto',
  'project.leaveConfirm': 'Clique de novo para sair',
  'project.leaveOwner': 'Passar e sair',
  'project.leaveConfirmOwner': 'Passar para {name} e sair',
  'project.leftToast': 'Você saiu do projeto.',
  'project.leaveFailed': 'Não foi possível sair do projeto.',
  // The empty card at the foot of To do — see `AddTaskSlot`.
  'board.addTask': 'Adicionar uma tarefa',
};

export const DICTIONARIES: Record<Locale, Record<TranslationKey, string>> = {
  en,
  'pt-BR': ptBR,
};
